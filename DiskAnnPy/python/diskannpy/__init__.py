...
- `build_async_index` - To build an index that can fully fit into memory when searching (async memory index building)
...
from typing import Any, Literal, NamedTuple, Type, Union, Dict

import numpy as np
from numpy import typing as npt

DistanceMetric = Literal["l2", "cosine", "cosinenormalized", "innerproduct"]
""" Type alias for one of {"l2", "cosine", "cosinenormalized", "innerproduct"} """
...
class QueryResponseBatchWithStats(NamedTuple):
    """
    A named tuple containing identifiers, distances, and search statistics. The identifiers and distances are 2D arrays,
    where rows correspond to the number of queries made, and columns correspond to the k neighbors requested. The two
    2D arrays have an implicit, position-based relationship. The search statistics are a dictionary of statistics.
    """
    ...
    *k-th* neighbor 
    ...
    """
    A dictionary containing search statistics. The keys are strings representing the names of the statistics, and the
    values can be of any type, representing the values of the statistics.
    """

class RangeQueryResponseBatchWithStats(NamedTuple):
    """
    A named tuple containing limits, identifiers, distances, and search statistics. The identifiers and distances are 1D arrays,
    where rows correspond to the number of queries made, and columns correspond to the k neighbors requested. The two
    arrays have an implicit, position-based relationship. The limits denote the boundaries between results for the different queries.
    The search statistics are a dictionary of statistics.
    """
    lims: npt.NDArray[np.uint32]
    """
    A `numpy.typing.NDArray[np.uint32]` array of limits, 1 dimensional. The 'lims' array indicates the boundaries between 
    results for different queries.
    """
    identifiers: npt.NDArray[VectorIdentifier]
    """ 
    A `numpy.typing.NDArray[VectorIdentifier]` array of vector identifiers, 1 dimensional. The 'lims' array indicates 
    the boundaries between results for different queries.
    """
    distances: np.ndarray[np.float32]
    """  
    A `numpy.typing.NDAarray[numpy.float32]` of distances as calculated by the distance metric function, 1 dimensional. 
    The 'lims' array indicates 
    the boundaries between results for different queries.
    """
    search_stats: Dict[str, Any]
    """
...
from ._async_diskann import AsyncDiskIndex
from ._bftree_diskann import BfTreeIndex
from ._builder import build_disk_index, build_async_index
__all__ = [
    "build_disk_index",
    "StaticDiskIndex",
    "build_async_index",
