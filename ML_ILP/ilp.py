...
# Try to import an ILP solver; fall back to pulp if GEKKO not available
do_lp = False
try:
...
def lp_soft_method(distances, matches, eps=1e-4):
    Q, N = distances.shape
    # Build LP
    if do_lp:
        # Using GEKKO
        ...
        w_d = pulp.LpVariable('w_d', lowBound=0)
...
def main():
    ...
    parser.add_argument('--method', choices=['ratio', 'lp'], default='ratio')
    ...
    if args.method == 'ratio':
        ...
        w_d, w_m, total_pairs, violations = lp_soft_method(distances, filter_matches, args.eps)
