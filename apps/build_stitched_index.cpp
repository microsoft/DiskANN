...
void handle_args(int argc, char **argv, std::string &data_type,
  ...
  try {
    ...
    required_configs.add_options()(
        "data_path", po::value<std::string>(&input_data_path)->required(),
    ...
    optional_configs.add_options()(
        "stitched_R", po::value<uint32_t>(&stitched_R)->default_value(100),
...
void save_full_index(path final_index_path_prefix, path input_data_path,
                     uint64_t final_index_size,
                     std::vector<std::vector<uint32_t>> stitched_graph,
                     tsl::robin_map<std::string, uint32_t> entry_points,
  ...
  for (auto &point_neighbors : stitched_graph) {
    index_max_observed_degree =
        std::max(index_max_observed_degree, (uint32_t)point_neighbors.size());
  }
  ...
  stitched_graph_writer.write((char *)&index_num_frozen_points,
                              sizeof(uint64_t));

  size_t bytes_written = METADATA;
  for (uint32_t node_point = 0; node_point < stitched_graph.size();
       node_point++) {
    uint32_t current_node_num_neighbors =
        (uint32_t)stitched_graph[node_point].size();
...
stitch_indices_return_values stitch_label_indices(
    path final_index_path_prefix, uint32_t total_number_of_points,
  ...
  for (const auto &lbl : all_labels) {
    ...
    std::tie(curr_label_index, curr_label_index_size) =
        diskann::load_label_index(curr_label_index_path,
...
void prune_and_save(path final_index_path_prefix, path full_index_path_prefix,
  ...
  diskann::Index<T> index(diskann::Metric::L2, dimension,
                          number_of_label_points, nullptr, nullptr, 0, false,
...
int main(int argc, char **argv) {
  ...
  if (data_type == "uint8")
    label_id_to_orig_id_map =
        diskann::generate_label_specific_vector_files<uint8_t>(
            input_data_path, labels_to_number_of_points, point_ids_to_labels,
            all_labels);
  else if (data_type == "int8")
    label_id_to_orig_id_map =
        diskann::generate_label_specific_vector_files<int8_t>(
            input_data_path, labels_to_number_of_points, point_ids_to_labels,
            all_labels);
  else if (data_type == "float")
    label_id_to_orig_id_map =
        diskann::generate_label_specific_vector_files<float>(
            input_data_path, labels_to_number_of_points, point_ids_to_labels,
            all_labels);
  else
    throw;
  ...
  if (data_type == "uint8")
    diskann::generate_label_indices<uint8_t>(
        input_data_path, final_index_path_prefix, all_labels, R, L, alpha,
        num_threads);
  else if (data_type == "int8")
    diskann::generate_label_indices<int8_t>(input_data_path,
                                            final_index_path_prefix, all_labels,
                                            R, L, alpha, num_threads);
  else if (data_type == "float")
    diskann::generate_label_indices<float>(input_data_path,
                                           final_index_path_prefix, all_labels,
                                           R, L, alpha, num_threads);
  else
    throw;
