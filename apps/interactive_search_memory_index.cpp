...
void load_raw_txt_file(std::string filename, std::vector<std::string> &contents) {
...
int search_memory_index(diskann::Metric &metric, const std::string &index_path, const std::string &result_path_prefix,
                        ...
                        const std::vector<label_set> &query_filters, const float fail_if_recall_below, std::string raw_query_labels_file, std::string raw_base_txt_file)
{
    using TagT = uint32_t;
    // Load the query file
    T *query = nullptr;
    ...
    std::vector<std::string> raw_query_labels, raw_base_data;
    if (raw_query_labels_file != "") {
        load_raw_txt_file(raw_query_labels_file, raw_query_labels);
    }

    if (raw_base_txt_file != "") {
        load_raw_txt_file(raw_base_txt_file, raw_base_data);
    }
...
*/
    ...
    while (!done_flag)
    {
        std::cout << "Enter query number to run (-1 to quit)" << std::endl;
        int qno;
        std::cin >> qno;
        ...
        std::cout << "Enter L_search to use" << std::endl;
        uint32_t L;
        std::cin>> L;
...
//        for (int64_t i = 0; i < (int64_t)query_num; i++)
        {
            ...
            if (filtered_search)
            {
                auto raw_filters = query_filters.size() == 1 ? query_filters[0] : query_filters[qno];

                if (raw_filters.size() == 1) {
                auto raw_filter = *(raw_filters.begin());
                auto retval = index->search_with_filters(query + qno * query_aligned_dim, raw_filter, recall_at, L,
                                                         query_result_ids.data() + qno * recall_at,
                                                         query_result_dists.data() + qno * recall_at);
//                cmp_stats[i] = retval.second;
                } else {
                auto retval = index->conjunctive_search_by_postprocessing(query + qno * query_aligned_dim, raw_filters, recall_at, L,
                                                         query_result_ids.data() + qno * recall_at,
                                                         query_result_dists.data() + qno * recall_at);
//                cmp_stats[i] = retval.second;
                }
...
/*            else if (metric == diskann::FAST_L2)
            ...
            {
                index->search(query + qno * query_aligned_dim, recall_at, L,
                                            query_result_ids.data() + qno * recall_at);;
            }
...
//        double displayed_qps = query_num / diff.count();
        ...
        if (calc_recall_flag)
        {
            ...
            for (uint32_t curr_recall = first_recall; curr_recall <= recall_at; curr_recall++)
            {
                recalls.push_back(diskann::calculate_recall(1, gt_ids + qno*gt_dim, gt_dists + qno*gt_dim, (uint32_t)gt_dim,
                                                            query_result_ids.data() + qno*recall_at, recall_at, curr_recall));
            }
        }
...
int main(int argc, char **argv)
{
    std::string data_type, dist_fn, index_path_prefix, result_path, query_file, gt_file, filter_label, label_type,
        query_filters_file, raw_query_labels, raw_base_txt;
    ...
    try
    {
        ...
        optional_configs.add_options()("raw_query_labels",
                                       po::value<std::string>(&raw_query_labels)->default_value(std::string("")),
                                       program_options_utils::FILTERS_FILE_DESCRIPTION);
        optional_configs.add_options()("raw_base_txt",
                                       po::value<std::string>(&raw_base_txt)->default_value(std::string("")),
                                       program_options_utils::FILTERS_FILE_DESCRIPTION);
        ...
        if (!query_filters.empty() && label_type == "ushort")
        {
            if (data_type == std::string("int8"))
            {
                return search_memory_index<int8_t, uint16_t>(
                    ...
                    Lvec, dynamic, tags, show_qps_per_thread, query_filters, fail_if_recall_below, raw_query_labels, raw_base_txt);
            }
            ...
            else if (data_type == std::string("float"))
            {
                return search_memory_index<float, uint16_t>(metric, index_path_prefix, result_path, query_file, gt_file,
                                                            ...
                                                            show_qps_per_thread, query_filters, fail_if_recall_below, raw_query_labels, raw_base_txt);
            }
            else
            {
