...
int main(int argc, char **argv)
{
    std::string data_type, dist_fn, data_path, index_path_prefix, codebook_prefix, label_file, universal_label,
        label_type;
    uint32_t num_threads, R, L, disk_PQ, build_PQ, QD, Lf, filter_threshold, num_parts;
    float B, M;
    ...
    try
    {
        ...
        optional_configs.add_options()("num_parts", po::value<uint32_t>(&num_parts)->default_value(0),
                                       program_options_utils::NUM_PARTS_DESCRIPTION);
    ...
    try
    {
        ...
        {
            if (data_type == std::string("int8"))
                return diskann::merge_subgraph_index<int8_t>(data_path.c_str(), index_path_prefix.c_str(), params.c_str(),
                                                         metric, use_opq, codebook_prefix, use_filters, label_file,
                                                         universal_label, filter_threshold, Lf, num_parts);
            //else if (data_type == std::string("uint8"))
