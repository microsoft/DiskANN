...
int search_disk_index(diskann::Metric &metric,
                      ...
                      const uint32_t num_threads, const float search_range,
...
#pragma omp parallel for schedule(dynamic, 1)
    for (int64_t i = 0; i < (int64_t)query_num; i++) {
      ...
      uint32_t res_count = _pFlashIndex->range_search(
    ...
    if (calc_recall_flag) {
      recall = diskann::calculate_range_search_recall(
          (uint32_t)query_num, groundtruth_ids, query_result_ids[test_id]);
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    required_configs.add_options()("range_threshold,K",
