...
int search_disk_index(
  ...
  diskann::cout << "Caching " << num_nodes_to_cache << " nodes around medoid(s)"
                << std::endl;
    ...
    for (int64_t i = 0; i < (int64_t)query_num; i++) {
      if (!filtered_search) {
        ...
        if (query_filters.size() == 1) { // one label for all queries
          label_for_search =
              _pFlashIndex->get_converted_label(query_filters[0]);
        } else { // one label for each query
          label_for_search =
              _pFlashIndex->get_converted_label(query_filters[i]);
        }
        _pFlashIndex->cached_beam_search(
            query + (i * query_aligned_dim), recall_at, L,
            query_result_ids_64.data() + (i * recall_at),
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    required_configs.add_options()(
        "result_path", po::value<std::string>(&result_path_prefix)->required(),
    ...
    optional_configs.add_options()("beamwidth,W",
    ...
    optional_configs.add_options()(
        "use_reorder_data", po::bool_switch()->default_value(false),
