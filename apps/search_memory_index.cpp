...
int search_memory_index(diskann::Metric &metric, const std::string &index_path,
                        ...
                        const std::vector<uint32_t> &Lvec, const bool dynamic,
  ...
  const size_t num_frozen_pts =
      diskann::get_graph_num_frozen_points(index_path);
  ...
  if (calc_recall_flag) {
    for (uint32_t curr_recall = first_recall; curr_recall <= recall_at;
         curr_recall++) {
...
#pragma omp parallel for schedule(dynamic, 1)
    for (int64_t i = 0; i < (int64_t)query_num; i++) {
      ...
      if (filtered_search && !tags) {
        ...
        auto retval = index->search_with_filters(
            query + i * query_aligned_dim, raw_filter, recall_at, L,
  ...
  for (auto L : Lvec) {
    ...
    std::string cur_result_path_prefix =
        result_path_prefix + "_" + std::to_string(L);

    std::string cur_result_path = cur_result_path_prefix + "_idx_uint32.bin";
    diskann::save_bin<uint32_t>(cur_result_path,
...
int main(int argc, char **argv) {
  ...
  po::options_description desc{program_options_utils::make_program_description(
      "search_memory_index", "Searches in-memory DiskANN indexes")};
  try {
    ...
    required_configs.add_options()(
        "recall_at,K", po::value<uint32_t>(&K)->required(),
    ...
    optional_configs.add_options()(
        "query_filters_file",
        po::value<std::string>(&query_filters_file)
    ...
    optional_configs.add_options()(
        ...
        "tags.  Default false.");
    optional_configs.add_options()(
        "tags", po::value<bool>(&tags)->default_value(false),
    ...
    output_controls.add_options()(
        "print_all_recalls", po::bool_switch(&print_all_recalls),
        "Print recalls at all positions, from 1 up to specified "
        "recall_at value");
    output_controls.add_options()(
        "print_qps_per_thread", po::bool_switch(&show_qps_per_thread),
