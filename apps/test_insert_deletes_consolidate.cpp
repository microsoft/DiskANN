...
void insert_till_next_checkpoint(
    diskann::AbstractIndex &index, size_t start, size_t end,
...
void delete_from_beginning(diskann::AbstractIndex &index,
  ...
  try {
    ...
    for (size_t i = points_to_skip;
      ...
      index.lazy_delete(
          static_cast<TagT>(i + 1)); // Since tags are data location + 1
...
void build_incremental_index(
    const std::string &data_path, diskann::IndexWriteParameters &params,
    size_t points_to_skip, size_t max_points_to_insert,
  ...
  auto index_search_params =
      diskann::IndexSearchParams(params.search_list_size, params.num_threads);
  ...
  if (points_to_delete_from_beginning > max_points_to_insert) {
    points_to_delete_from_beginning =
        static_cast<uint32_t>(max_points_to_insert);
  ...
  if (concurrent) {
    ...
    for (size_t start = current_point_offset; start < last_point_threshold;
      ...
      auto insert_task = std::async(std::launch::async, [&]() {
        ...
        insert_till_next_checkpoint<T, TagT, LabelT>(
            *index, start, end, sub_threads, data, aligned_dim,
            location_to_labels);
      });
      insert_task.wait();
      ...
      if (!delete_launched && end >= start_deletes_after &&
        ...
        diskann::IndexWriteParameters delete_params =
            diskann::IndexWriteParametersBuilder(params)
    ...
    for (size_t start = current_point_offset; start < last_point_threshold;
      ...
      load_aligned_bin_part(data_path, data, start, end - start);
      insert_till_next_checkpoint<T, TagT, LabelT>(
          *index, start, end, (int32_t)params.num_threads, data, aligned_dim,
          location_to_labels);
    ...
    if (points_to_delete_from_beginning > 0) {
      delete_from_beginning<T, TagT>(*index, params, points_to_skip,
                                     points_to_delete_from_beginning);
    }
...
int main(int argc, char **argv) {
  ...
  po::options_description desc{program_options_utils::make_program_description(
      "test_insert_deletes_consolidate", "Test insert deletes & consolidate")};
  try {
    ...
    required_configs.add_options()(
        "points_to_skip", po::value<uint64_t>(&points_to_skip)->required(),
    ...
    optional_configs.add_options()(
        ...
        "points_to_skip");
    optional_configs.add_options()(
        "do_concurrent", po::value<bool>(&concurrent)->default_value(false),
        "");
    optional_configs.add_options()(
        "start_deletes_after",
