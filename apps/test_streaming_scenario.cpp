...
void insert_next_batch(diskann::AbstractIndex &index, size_t start, size_t end,
...
#pragma omp parallel for num_threads((int32_t)insert_threads) schedule(dynamic) reduction(+ : num_failed)
    for (int64_t j = start; j < (int64_t)end; j++) {
      ...
      if (pts_to_labels.size() > 0) {
        insert_result = index.insert_point(&data[(j - start) * aligned_dim],
...
void delete_and_consolidate(diskann::AbstractIndex &index,
  ...
  try {
    ...
    std::cout << "#active   points: " << report._active_points << std::endl
              ...
              << "Deletion rate: " << deletion_rate / delete_params.num_threads
              << "/thread/sec   " << std::endl;
  } catch (std::system_error &e) {
...
void build_incremental_index(
    const std::string &data_path, const uint32_t L, const uint32_t R,
    const float alpha, const uint32_t insert_threads,
    const uint32_t consolidate_threads, size_t max_points_to_insert,
    size_t active_window, size_t consolidate_interval,
    const float start_point_norm, uint32_t num_start_pts,
    const std::string &save_path, const std::string &label_file,
  ...
  if (has_labels) {
    ...
    auto parse_result =
        diskann::parse_formatted_label_file<LabelT>(labels_file_to_use);
    pts_to_labels = std::get<0>(parse_result);
  }

  diskann::get_bin_metadata(data_path, num_points, dim);
  diskann::cout << "metadata: file " << data_path << " has " << num_points
                << " points in " << dim << " dims" << std::endl;
  ...
  if (num_points < max_points_to_insert)
    throw diskann::ANNException(std::string("num_points(") +
  ...
  diskann::alloc_aligned((void **)&data,
                         std::max(consolidate_interval, active_window) *
  ...
  auto insert_task = std::async(std::launch::async, [&]() {
    ...
    insert_next_batch<T, TagT, LabelT>(*index, (size_t)0, active_window,
                                       params.num_threads, data, aligned_dim,
                                       pts_to_labels);
  });
  insert_task.wait();
  ...
  for (size_t start = active_window;
    ...
    auto insert_task = std::async(std::launch::async, [&]() {
      ...
      insert_next_batch<T, TagT, LabelT>(*index, start, end, params.num_threads,
                                         data, aligned_dim, pts_to_labels);
    });
    insert_task.wait();
    ...
    if (start >= active_window + consolidate_interval) {
      ...
      delete_tasks.emplace_back(std::async(std::launch::async, [&]() {
        delete_and_consolidate<T, TagT, LabelT>(
            *index, delete_params, (size_t)start_del, (size_t)end_del);
      }));
    }
  }
  if (delete_tasks.size() > 0)
...
int main(int argc, char **argv) {
  ...
  po::options_description desc{program_options_utils::make_program_description(
      "test_streaming_scenario", "Test insert deletes & consolidate")};
  try {
    ...
    required_configs.add_options()(
        "active_window", po::value<uint64_t>(&active_window)->required(),
    ...
    optional_configs.add_options()(
        "num_start_points",
        po::value<uint32_t>(&num_start_pts)
            ->default_value(diskann::defaults::NUM_FROZEN_POINTS_DYNAMIC),
  ...
  if (label_type != std::string("ushort") &&
    ...
    std::cerr << "Invalid label type. Supported types are uint and ushort"
              << std::endl;
    return -1;
  }
  ...
  if (data_type != std::string("int8") && data_type != std::string("uint8") &&
    ...
    std::cerr << "Invalid data type. Supported types are int8, uint8 and float"
              << std::endl;
    return -1;
  }
  ...
  if (dist_fn != std::string("l2") && dist_fn != std::string("mips")) {
    std::cerr
        << "Invalid distance function. Supported functions are l2 and mips"
        << std::endl;
    return -1;
  }
    ...
    if (data_type == std::string("uint8")) {
      if (label_type == std::string("ushort")) {
        build_incremental_index<uint8_t, uint32_t, uint16_t>(
            ...
            start_point_norm, num_start_pts, index_path_prefix, label_file,
            universal_label, Lf);
