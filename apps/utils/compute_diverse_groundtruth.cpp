...
void exact_knn(const size_t dim, const size_t k,
               ...
               size_t nqueries, float *queries_in, uint32_t kPerSeller, std::vector<diskann::NeighborPriorityQueueExtendColor> &running_results,
...
#pragma omp parallel for schedule(dynamic, 16)
        ...
        {
            diskann::NeighborPriorityQueueExtendColor& cur_query_best_results = running_results[q];
//            for (size_t p = 0; p < k; p++)
//                point_dist.emplace(p, dist_matrix[(ptrdiff_t)p + (ptrdiff_t)(q - q_b) * (ptrdiff_t)npoints]);
            for (size_t p = 0; p < npoints; p++)
            {
                uint32_t cur_id = static_cast<std::uint32_t>(p + start_id);
                float cur_dist = dist_matrix[(ptrdiff_t)p + (ptrdiff_t)(q - q_b) * (ptrdiff_t)npoints];
                cur_query_best_results.insert(diskann::Neighbor(cur_id, cur_dist));
            }
        }
...
std::vector<std::vector<std::pair<uint32_t, float>>> processUnfilteredParts(const std::string &base_file,
                                                                            ...
                                                                            std::vector<uint32_t> &location_to_tag, std::vector<uint32_t> &location_to_seller, uint32_t kperseller)
{
    float *base_data = nullptr;
    int num_parts = get_num_parts<T>(base_file.c_str());
    std::vector<std::vector<std::pair<uint32_t, float>>> res(nqueries);
    std::vector<diskann::NeighborPriorityQueueExtendColor> running_results(nqueries, diskann::NeighborPriorityQueueExtendColor(k, kperseller, location_to_seller));
    for (int p = 0; p < num_parts; p++)
    {
        ...
        {
            std::vector<diskann::Neighbor> beset_L_nodes;
            auto & cur_results = running_results[i];
            cur_results.get_data(beset_L_nodes);
            //            for (size_t j = 0; j < part_k; j++)
            for (uint32_t x = 0; x < beset_L_nodes.size(); x++)
            {
                auto &nbr = beset_L_nodes[x];
...
template <typename T>
int aux_main(const std::string &base_file, const std::string &query_file, const std::string &seller_file, const std::string &gt_file, size_t k, size_t kperseller, 
             const diskann::Metric &metric, const std::string &tags_file = std::string(""))
{
    ...
    std::vector<uint32_t> location_to_seller;
    ...
    std::vector<std::vector<std::pair<uint32_t, float>>> results =
        processUnfilteredParts<T>(base_file, nqueries, npoints, dim, k, query_data, metric, location_to_tag, location_to_seller, (std::uint32_t)kperseller);
...
{
    std::string data_type, dist_fn, base_file, query_file, gt_file, tags_file, seller_file;
    uint64_t K, KperSeller;
    ...
    try
    {
        ...
        desc.add_options()("seller_file", po::value<std::string>(&seller_file)->required(),
                           "File containing the seller per point");
        ...
        desc.add_options()("KperSeller", po::value<uint64_t>(&KperSeller)->required(),
                           "Number of ground truth nearest neighbors to compute per Seller");
