...
#pragma omp parallel for schedule(static, 65536)
  for (int64_t d = 0; d < num_points; ++d)
    points_l2sq[d] =
        cblas_sdot((int64_t)dim, matrix + (ptrdiff_t)d * (ptrdiff_t)dim, 1,
...
processUnfilteredParts(const std::string &base_file, size_t &nqueries,
                       size_t &npoints, size_t &dim, size_t &k,
                       float *query_data, const diskann::Metric &metric,
...
std::vector<std::vector<std::pair<uint32_t, float>>> processFilteredParts(
    ...
    const std::string &filter_label, const std::string &universal_label,
    size_t &nqueries, size_t &npoints, size_t &dim, size_t &k,
    float *query_data, const diskann::Metric &metric,
    std::vector<uint32_t> &location_to_tag) {
...
int aux_main(const std::string &base_file, const std::string &label_file,
             ...
             size_t k, const std::string &universal_label,
             const diskann::Metric &metric, const std::string &filter_label,
  ...
  std::vector<uint32_t> location_to_tag =
      diskann::loadTags(tags_file, base_file);
  ...
  if (filter_label == "") {
    results = processUnfilteredParts<T>(base_file, nqueries, npoints, dim, k,
                                        query_data, metric, location_to_tag);
  } else {
    results = processFilteredParts<T>(base_file, label_file, filter_label,
                                      universal_label, nqueries, npoints, dim,
...
void load_truthset(const std::string &bin_file, uint32_t *&ids, float *&dists,
...
int main(int argc, char **argv) {
  std::string data_type, dist_fn, base_file, query_file, gt_file, tags_file,
      label_file, filter_label, universal_label, filter_label_file;
  uint64_t K;
  ...
  try {
    ...
    desc.add_options()("filter_label_file",
                       po::value<std::string>(&filter_label_file)
  ...
  if (filter_label != "" && filter_label_file != "") {
    std::cerr
        << "Only one of filter_label and query_filters_file should be provided"
        << std::endl;
    return -1;
  }
  ...
  if (filter_labels.size() == 1) {
    try {
      if (data_type == std::string("float"))
        aux_main<float>(base_file, label_file, query_file, gt_file, K,
                        universal_label, metric, filter_labels[0], tags_file);
    ...
    for (size_t i = 0; i < filter_labels.size(); i++) {
      ...
      if (labels_to_number_of_queries.find(label) ==
          labels_to_number_of_queries.end()) {
    ...
    for (size_t i = 0; i < point_to_labels.size(); i++) {
      for (size_t j = 0; j < point_to_labels[i].size(); j++) {
        ...
        if (all_labels.find(label) != all_labels.end()) {
          ...
          if (labels_to_number_of_points.find(label) ==
              labels_to_number_of_points.end()) {
    ...
    tsl::robin_map<std::string, std::vector<uint32_t>>
        label_query_id_to_orig_id;

    if (data_type == std::string("float")) {
      label_id_to_orig_id =
          diskann::generate_label_specific_vector_files_compat<float>(
      ...
      label_id_to_orig_id =
          diskann::generate_label_specific_vector_files_compat<int8_t>(
              base_file, labels_to_number_of_points, point_ids_to_labels,
              all_labels);
      ...
      label_id_to_orig_id =
          diskann::generate_label_specific_vector_files_compat<uint8_t>(
              base_file, labels_to_number_of_points, point_ids_to_labels,
              all_labels);
    ...
    try {
      for (const auto &label : all_labels) {
        ...
        if (data_type == std::string("float"))
          aux_main<float>(filtered_base_file, "", filtered_query_file,
                          filtered_gt_file, K, "", metric, "");
        if (data_type == std::string("int8"))
          aux_main<int8_t>(filtered_base_file, "", filtered_query_file,
                           filtered_gt_file, K, "", metric, "");
        if (data_type == std::string("uint8"))
          aux_main<uint8_t>(filtered_base_file, "", filtered_query_file,
                            filtered_gt_file, K, "", metric, "");
      }
    } catch (const std::exception &e) {
      std::cout << std::string(e.what()) << std::endl;
    ...
    for (const auto &lbl : all_labels) {
      ...
      for (uint32_t i = 0; i < labels_to_number_of_queries[lbl]; i++) {
        ...
        for (uint64_t j = 0; j < K; j++) {
          final_gt_ids[orig_query_id][j] =
              label_id_to_orig_id[lbl][gt_ids[i * K + j]];
