...
public:
  ...
  std::unordered_map<uint32_t, uint32_t> createDistributionMap() {
    ...
    uint32_t primary_label_freq =
        (uint32_t)ceil(num_points * distribution_factor);
  ...
  int writeDistribution(std::ofstream &outfile) {
    ...
    for (uint32_t i{0}; i < num_points; i++) {
      ...
      for (auto it = distribution_map.cbegin(); it != distribution_map.cend();
           it++) {
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    desc.add_options()("num_labels,L",
                       po::value<uint32_t>(&num_labels)->required(),
