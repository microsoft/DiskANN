...
int block_write_float(std::ofstream &writer, size_t ndims, size_t npts,
                      bool normalization, float norm, float rand_scale) {
...
int block_write_int8(std::ofstream &writer, size_t ndims, size_t npts,
                     float norm) {
  auto vec = new float[ndims];
...
int block_write_uint8(std::ofstream &writer, size_t ndims, size_t npts,
                      float norm) {
  auto vec = new float[ndims];
  auto vec_T = new int8_t[ndims];
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    desc.add_options()("npts,N", po::value<uint64_t>(&npts)->required(),
                       "Number of vectors");
    desc.add_options()(
        "norm", po::value<float>(&norm)->default_value(-1.0f),
        "Norm of the vectors (if not specified, vectors are not normalized)");
    desc.add_options()(
        "rand_scaling", po::value<float>(&rand_scaling)->default_value(1.0f),
  ...
  if (data_type == std::string("int8") || data_type == std::string("uint8")) {
    ...
    if (rand_scaling > 1.0) {
      std::cout << "Data scaling only supported for floating point data."
                << std::endl;
      return -1;
    }
  }
  ...
  try {
    ...
    for (size_t i = 0; i < nblks; i++) {
      ...
      if (data_type == std::string("float")) {
        ret = block_write_float(writer, ndims, cblk_size, normalization, norm,
                                rand_scaling);
