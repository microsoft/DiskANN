...
pub struct Registry {
    ...
    ///
    /// The keys are generated by [`Reflection::type_name`]. We store the associated
    /// [`Relflection`] and [`TypeId`] to ensure that registered type-names are unique.
    name_map: HashMap<String, (Reflection, TypeId)>,
        ...
        T: Input + 'static,
    {
        ...
        match self.inputs.entry(tag) {
            Entry::Vacant(v) => {
                ...
                //
                // TODO: Enable roll-back of any newly registered type.
                Reflection::new::<T::Raw>()
                    .visit_with(|reflection: Reflection| -> Result<bool, ()> {
                        let type_id = reflection.type_id();

                        // If the type doesn't exist - try to add it via type-name.
                        // Only if all that succeeds to we commit the type-id.
                        if let hash_set::Entry::Vacant(vacant) = self.type_ids.entry(type_id) {
                            let type_name = reflection.type_name().to_string();
                            match self.name_map.entry(type_name) {
                                Entry::Vacant(v) => {
                                    v.insert((reflection, type_id));
                                }
                                Entry::Occupied(o) => {
                                    if o.get().1 != type_id {
                                        panic!("eventually we will fail gracetully");
                                    }
                                }
                            }
