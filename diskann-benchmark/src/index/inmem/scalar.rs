...
pub(crate) fn register_benchmarks(benchmarks: &mut Registry) -> anyhow::Result<()> {
    ...
    // `ScalarQuantized<NBITS, T>` via the `impl_sq_build!` macro,
    ...
    //      ScalarQuantized::<N, T>::new().search(Topk));` call here.
    //   2. Add a matching `impl_sq_build!(N, T);` invocation at the bottom.
    //
    ...
    benchmarks.register(
        ...
        ScalarQuantized::<8, f32>::new().search(plugins::Topk),
    )?;
    benchmarks.register(
        "graph-index-sq-4-bit-f32",
        ScalarQuantized::<4, f32>::new().search(plugins::Topk),
    )?;
    benchmarks.register(
        "graph-index-sq-1-bit-f32",
        ScalarQuantized::<1, f32>::new().search(plugins::Topk),
    )?;
...
where
    T: VectorRepr,
{
    quant_search: plugins::Plugins<SQProvider<NBITS, T>, SearchPhase, Strategy<common::Quantized>>,
    full_search:
