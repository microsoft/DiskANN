...
pub(crate) struct SearchResults {
    ...
    pub(crate) query_latencies: Vec<Vec<MicroSeconds>>,
    pub(crate) p90_latencies: Vec<MicroSeconds>,
    pub(crate) p99_latencies: Vec<MicroSeconds>,
    pub(crate) p999_latencies: Vec<MicroSeconds>,
    ...
    F: Fn(usize) -> String,
{
    ...
    results.iter().enumerate().for_each(|(i, r)| {
        ...
        row.insert(
            format!(
                "{:.1}us ({:.1})",
                MaybeDisplay(percentiles::mean(&r.p90_latencies), "missing"),
                MaybeDisplay(r.p90_latencies.iter().max(), "missing"),
            ),
            col_idx + 6,
        );
        row.insert(
        ...
        row.insert(
            format!(
                "{:.1}us ({:.1})",
                MaybeDisplay(percentiles::mean(&r.p999_latencies), "missing"),
                MaybeDisplay(r.p999_latencies.iter().max(), "missing"),
            ),
            col_idx + 8,
        );
        row.insert(format!("{:3}", r.recall.average), col_idx + 9);
        row.insert(r.num_tasks, col_idx + 10);
    });
