...
fn run_ab(run: &Run) -> anyhow::Result<QuantRunResult> {
    ...
    // Path B — the coarse tiler rebuild (same math + tiling, `Tiler`/`Tile` handles,
    // kernel pinned on the panel types).
    let mut tiled_query = QuantTiledQuery::build(data.queries.as_view())
    ...
    // Timed adjacent to `staged` so the staged-vs-tiled ratio is trustworthy
    // despite this box's cross-run clock variance.
