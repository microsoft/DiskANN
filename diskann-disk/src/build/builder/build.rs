...
use crate::{
    build::builder::{
        ...
        vamana::{
            determine_build_strategy, IndexBuildStrategy, MergedVamanaBuilder, OneShotVamanaBuilder,
        },
    },
    storage::{
    ...
    <StorageProvider as StorageReadProvider>::Reader: std::marker::Send,
{
    ...
    async fn build_internal(&mut self) -> ANNResult<()> {
        ...
        self.build_vamana_index(pool.as_ref()).await?;
    ...
    async fn build_vamana_index(&mut self, pool: RayonThreadPoolRef<'_>) -> ANNResult<()> {
        let strategy = determine_build_strategy::<Data>(
        ...
        match strategy {
            ...
            IndexBuildStrategy::Merged => {
                MergedVamanaBuilder::<Data, _>::new(
                    &self.index_configuration,
