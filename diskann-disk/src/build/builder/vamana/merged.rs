...
use crate::{
    ...
    utils::{
        instrumentation::{BuildMergedVamanaIndexCheckpoint, PerfLogger},
        partition_with_ram_budget,
    },
    DiskIndexBuildParameters,
};

use super::{one_shot::OneShotVamanaBuilder, strategy::estimate_build_index_ram_usage};
...
pub(in crate::build::builder) struct MergedVamanaBuilder<'a, Data, StorageProvider>
where
...
impl<'a, Data, StorageProvider> MergedVamanaBuilder<'a, Data, StorageProvider>
where
    Data: GraphDataType<VectorIdType = u32>,
    ...
    <StorageProvider as StorageReadProvider>::Reader: Send,
{
    ...
    pub(in crate::build::builder) async fn build(
        mut self,
    ...
    async fn build_shard_index(
        ...
        OneShotVamanaBuilder::<Data::VectorDataType, _>::new(
            &index_config,
