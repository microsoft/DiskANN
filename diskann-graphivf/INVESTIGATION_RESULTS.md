...
Benchmark JSONs live in the untracked `_results/configs/sweeps/enron/graph-ivf/`:
`graph-ivf-enron-build*.json` / `graph-ivf-enron-search*.json`. Analysis tooling:
`diskann-graphivf/scripts/analysis/centroid_graph_ablation.rs` (ablation 5),
a since-removed `refine_centroids_full_corpus` example (ablation 8; recover it from
git history), and `_results/scripts/analyze_ivf_lists.py` (ablation 7).
...
**Method** (a `refine_centroids_full_corpus` example, since removed — recover it from
git history): read the
...
Configs in `_results/configs/sweeps/enron/disk-index/`:
`disk-index-enron-t1-beamsweep.json` (beam sweep),
`disk-index-enron-t1-recall1000-bw32.json`,
