...
What is left here are the two tools the harness does not cover: preparing a corpus,
and a centroid-graph quality diagnostic. Each is a standalone `cargo` example (binary
name == file stem, kept stable via explicit `[[example]]` targets in
