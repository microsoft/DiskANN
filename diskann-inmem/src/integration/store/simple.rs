...
boilerplate!(
    store::simple::Simple => Store,
    for<'a> store::simple::Reader<'a> => Reader,
    for<'a> store::simple::Exclusive<'a> => Writer,
);
...
impl Store {
    ...
    pub fn new(config: Config) -> Self {
        ...
        let slots_config = store::simple::Simple::config(Bytes::new(config.entry_bytes));
