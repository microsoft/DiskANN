    ...
    DP: DataProvider,
{
    ...
    where
        ...
        DP: Delete + Send + Sync,
    {
        self.handle.block_on(self.inner.multi_inplace_delete(
            strategy,
            context,
            ids,
            num_to_replace,
    ...
    {
        self.handle.block_on(self.inner.search_recorded(
            strategy,
            context,
            query,
            search_params,
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn flat_search<'a, S, T, O, OB, I>(
        &'a self,
        strategy: &'a S,
    ...
    where
        ...
        I: Iterator<Item = DP::InternalId>,
        O: Send,
        OB: search_output_buffer::SearchOutputBuffer<O> + Send,
    {
        self.handle.block_on(self.inner.flat_search(
            strategy,
            context,
            query,
            vector_filter,
    ...
    ) -> ANNResult<(SearchStats, Vec<O>, Vec<f32>)>
    where
        T: Sync + ?Sized,
        S: SearchStrategy<DP, T, O>,
        O: Send + Default + Clone,
    {
        self.handle.block_on(
            self.inner
                .range_search(strategy, context, query, search_params),
        )
    }
