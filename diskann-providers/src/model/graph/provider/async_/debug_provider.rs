...
use diskann::{
    ...
    graph::{
        AdjacencyList,
        glue::{
            self, ExpandBeam, FilterStartPoints, InplaceDeleteStrategy, InsertStrategy, Pipeline,
...
impl<B> glue::MultiInsertStrategy<DebugProvider, B> for FullPrecision
where
    B: glue::Batch,
    Self: for<'a> InsertStrategy<DebugProvider, B::Element<'a>, PruneStrategy = Self>,
{
    ...
    fn finish<Itr>(&self, _batch: &Arc<B>, _ids: Itr) -> Self::Seed
    where
