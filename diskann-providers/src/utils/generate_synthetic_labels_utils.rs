...
mod test {
    ...
    fn generate_label_test() {
        let rng = &mut crate::utils::create_rnd_in_tests();
        let label_file1: &str = "rand_labels_50_10K_zipf.txt";
        generate_labels(label_file1, "zipf", 10000, 50, rng).unwrap();
        ...
        generate_labels(label_file2, "random", 10000, 50, rng).unwrap();
        ...
        generate_labels(label_file3, "one_per_point", 10000, 50, rng).unwrap();
