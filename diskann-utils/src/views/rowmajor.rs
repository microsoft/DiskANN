...
use std::{marker::PhantomData, mem::ManuallyDrop, num::NonZeroUsize, ops::Range, ptr::NonNull};
use thiserror::Error;
...
mod tests {
    ...
    fn try_from_error_misc() {
        let error = Ref::<f32>::try_from_data(&[], 1, 2).unwrap_err();
    ...
    fn test_get_row_panics() {
        let m = Owned::<usize>::copied(0, 3, 7).unwrap();
        m.row(3);
    }
    ...
    fn test_get_row_mut_panics() {
        let mut m = Owned::<usize>::copied(0, 3, 7).unwrap();
        m.row_mut(3);
    }
    ...
    fn test_par_window_iter_panics() {
        let m = Owned::<usize>::copied(0, 4, 4).unwrap();
    ...
    fn test_par_window_iter_mut_panics() {
        let mut m = Owned::<usize>::copied(0, 4, 4).unwrap();
    ...
    fn test_box_slice_dense_data_impls() {
        ...
        let ptr = data.as_ptr();
        let len = data.len();
        let owned = Owned::try_from_data(data, 2, 3).unwrap();
        test_dense_data_repr(ptr, len, owned, &"Owned");

        let mut owned = Owned::copied(0.0, 2, 3).unwrap();
        set_mut_dense_data_repr(&mut owned, 1.0, 2.0);
        for (i, &v) in owned.as_slice().iter().enumerate() {
            assert_eq!(
                v,
                1.0 + 2.0 * (i as f32),
    ...
    fn test_unsafe_get_unchecked_methods() {
        ...
        // Safety: derives from known size of matrix and access element ids
        unsafe {
            *m.element_mut_unchecked(0, 0) = 100;
            *m.element_mut_unchecked(1, 2) = 200;
        }
        ...
        // Safety: derives from known size of matrix and access element ids
        unsafe {
            let row1 = m.row_mut_unchecked(1);
            row1[0] = 300;
        }
    ...
    fn test_generator_trait_impls() {
        let cloned = Owned::cloned(42, 1, 2).unwrap();
        assert_eq!(cloned.as_slice(), &[42, 42]);

        let mut counter = 0;
        let generated = Owned::from_fn(1, 3, || {
            counter += 1;
            counter
        })
        .unwrap();
        assert_eq!(generated.as_slice(), &[1, 2, 3]);
    }
    ...
    fn test_matrix_from_conversions() {
        ...
        // Test immutable view slice access.
        let view = m.as_view();
    ...
    fn test_matrix_construction_edge_cases() {
        // Test 1x1 matrix
        let m = Owned::cloned(42, 1, 1).unwrap();
        assert_eq!(m.nrows(), 1);
        assert_eq!(m.ncols(), 1);
        ...
        let m = Owned::cloned(7, 1, 5).unwrap();
        assert_eq!(m.nrows(), 1);
        assert_eq!(m.ncols(), 5);
        assert!(m.as_slice().iter().all(|&x| x == 7));

        // Test single column matrix
        let m = Owned::cloned(9, 5, 1).unwrap();
        assert_eq!(m.nrows(), 5);
        assert_eq!(m.ncols(), 1);
    ...
    #[test]
    fn test_get_element() {
        let m = Owned::try_from_data(vec![1, 2, 3, 4, 5, 6].into(), 2, 3).unwrap();
    ...
    fn test_parallel_methods_edge_cases() {
        ...
        let mut m2 = Owned::cloned(0, 4, 3).unwrap();
        ...
        let mut m3 = Owned::cloned(0, 4, 3).unwrap();
    ...
    fn test_matrix_pointers() {
        let mut m = Owned::cloned(42, 3, 4).unwrap();
    ...
    fn test_matrix_init_generator_various_types() {
        ...
        let m = Owned::from_fn(2, 3, || counter.fetch_add(1, Ordering::SeqCst)).unwrap();
    ...
    fn test_debug_error_formatting() {
        ...
        assert!(debug_str.contains("data: \"<hidden>\""));
        ...
        let non_debug_err = Owned::try_from_data(non_debug_data, 1, 3).unwrap_err();
        ...
        for nrows in [1, 2, 3, 5, 8, 10] {
            for ncols in [1, 3, 4] {
                for batchsize in [1, 2, 3, 7] {
                    ...
                    let mut m = Owned::cloned(0usize, nrows, ncols).unwrap();
    ...
    fn test_par_row_iter_mut_comprehensive() {
        ...
        let nrows = 6;
        let ncols = 4;
        let mut m = Owned::cloned(0u32, nrows, ncols).unwrap();
    ...
    fn test_rayon_trait_bounds_validation() {
        ...
        let mut m = Owned::cloned(0u64, 4, 5).unwrap();
