...
use diskann_vector::{distance::SquaredL2, DistanceFunction};
use half::f16;
...
pub(crate) fn benchmark_f32(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("bulk-l2-f32");
...
pub(crate) fn benchmark_f16(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("bulk-l2-f16");
    let args = DatasetArgs::new(
        0,
        NonZeroUsize::new(dim.into()).unwrap(),
...
pub(crate) fn benchmark_i8(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("bulk-l2-i8");
    let args = DatasetArgs::new(
        0,
        NonZeroUsize::new(dim.into()).unwrap(),
        NonZeroUsize::new(128).unwrap(),
...
pub(crate) fn benchmark_u8(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("bulk-l2-u8");
    let args = DatasetArgs::new(
        0,
        NonZeroUsize::new(dim.into()).unwrap(),
        NonZeroUsize::new(128).unwrap(),
