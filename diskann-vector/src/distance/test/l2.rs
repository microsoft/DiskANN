...
mod distance_test {
    ...
    fn test_dist_cosine_normalized_float_turing() {
        ...
        assert_abs_diff_eq!(distance, got, epsilon = 1e-4);
    }
    ...
    fn distance_test() {
        ...
        for seed in 0..100 {
            ...
            let distance = compare::<f32, 256>(256, Metric::L2, &vec1.v);
            let expected = no_vector_compare_f32_as_f64(&vec2.0, &vec3.0);
            assert_relative_eq!(
                distance as f64,
                expected,
                epsilon = 1e-4f64,
                max_relative = 1e-4f64,
            );
        }
    }
