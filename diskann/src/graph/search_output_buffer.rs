...
impl<I> SearchOutputBuffer<I> for IdDistance<'_, I> {
    ...
    fn push(&mut self, neighbor: Neighbor<I>) -> BufferState {
        ...
        Itr: IntoIterator<Item = Neighbor<I>>,
    {
        ...
        .zip(itr)
        .for_each(|((i_out, d_out), neighbor)| {
            let (i_in, d_in) = neighbor.as_tuple();
            i += 1;
            *i_out = i_in;
            *d_out = d_in;
        });
    ...
    for IdDistanceAssociatedData<'_, I, A>
{
    ...
    fn push(&mut self, neighbor: Neighbor<(I, A)>) -> BufferState {
        if self.position == self.capacity() {
            return BufferState::Full;
        }

        let ((id, assoc), distance) = neighbor.as_tuple();
        self.ids[self.position] = id;
        self.distances[self.position] = distance;
    ...
    fn extend<Itr>(&mut self, itr: Itr) -> usize
    where
        Itr: IntoIterator<Item = Neighbor<(I, A)>>,
    {
        let mut i = 0;
        let p = self.position;
        for (((id_out, dist_out), assoc_out), neighbor) in self
            .ids
            .iter_mut()
            .skip(p)
            ...
            .zip(itr)
        {
            let ((id, assoc), dist) = neighbor.as_tuple();
            *id_out = id;
            *dist_out = dist;
            *assoc_out = assoc;
            i += 1;
...
mod tests {
    ...
    fn test_id_distance() {
        ...
        // All `push`.
        {
            ...
            assert!(buffer.push(Neighbor::new(5, 5.0)).is_full());
            assert_eq!(buffer.current_len(), 5);
            assert_eq!(buffer.size_hint(), Some(0));

            assert!(buffer.push(Neighbor::new(6, 6.0)).is_full());
            assert_eq!(buffer.current_len(), 5);
            assert_eq!(buffer.size_hint(), Some(0));
        ...
        // All `iterator`.
        {
            ...
            assert!(buffer.push(Neighbor::new(7, 7.0)).is_full());

            let set = buffer.extend([(10, 10.0), (20, 20.0)].map(Neighbor::from_tuple));
        ...
        // Mixture
        {
            ...
            let set = buffer.extend([(2, 2.0), (3, 3.0)].map(Neighbor::from_tuple));
            ...
            let set = buffer.extend([(5, 5.0), (6, 6.0)].map(Neighbor::from_tuple));
            assert_eq!(
                set, 1,
    ...
    fn test_id_distance_associated() {
        ...
        // All `push`.
        {
            ...
            assert!(buffer.push(Neighbor::new((1, 10), 1.0)).is_available());
            assert_eq!(buffer.current_len(), 1);
            assert_eq!(buffer.size_hint(), Some(MAX_LENGTH - 1));

            assert!(buffer.push(Neighbor::new((2, 20), 2.0)).is_available());
            assert_eq!(buffer.current_len(), 2);
            assert_eq!(buffer.size_hint(), Some(MAX_LENGTH - 2));

            assert!(buffer.push(Neighbor::new((3, 30), 3.0)).is_available());
            assert_eq!(buffer.current_len(), 3);
            assert_eq!(buffer.size_hint(), Some(MAX_LENGTH - 3));

            assert!(buffer.push(Neighbor::new((4, 40), 4.0)).is_available());
            assert_eq!(buffer.current_len(), 4);
            assert_eq!(buffer.size_hint(), Some(MAX_LENGTH - 4));

            // This should error since further attempts will not work.
            assert!(buffer.push(Neighbor::new((5, 50), 5.0)).is_full());
            assert_eq!(buffer.current_len(), 5);
            assert_eq!(buffer.size_hint(), Some(0));

            assert!(buffer.push(Neighbor::new((6, 60), 6.0)).is_full());
            assert_eq!(buffer.current_len(), 5);
            assert_eq!(buffer.size_hint(), Some(0));
        ...
        // All `iterator`.
        {
            ...
            // Ensure that `pushing` respects the limit.
            assert!(buffer.push(Neighbor::new((7, 70), 7.0)).is_full());

            let set =
                buffer.extend([((10, 100), 10.0), ((20, 200), 20.0)].map(Neighbor::from_tuple));
            assert_eq!(set, 0, "no more items can be added");
        ...
        // Mixture
        {
            ...
            let set = buffer.extend([((2, 20), 2.0), ((3, 30), 3.0)].map(Neighbor::from_tuple));
            assert_eq!(set, 2, "only two items were pushed");
            ...
            let set = buffer.extend([((5, 50), 5.0), ((6, 60), 6.0)].map(Neighbor::from_tuple));
            assert_eq!(
                set, 1,
                "there should only be room for one more item in the buffer"
            );
