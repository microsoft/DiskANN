...
template <typename label_type> class AbstractFilterStore
{
  public:
    ...
    DISKANN_DLLEXPORT virtual void set_universal_labels(const std::vector<std::string> &raw_universal_label) = 0;
    // DISKANN_DLLEXPORT virtual const label_type get_universal_label() const = 0;
    DISKANN_DLLEXPORT virtual void set_universal_labels_with_category(
        const std::unordered_map<std ::string, std::string> category_label_map) = 0;
  ...
  protected:
    ...
    DISKANN_DLLEXPORT virtual void load_universal_labels(const std::string &universal_labels) = 0;
