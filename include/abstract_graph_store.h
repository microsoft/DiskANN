...
public:
  ...
  load(const std::string &index_path_prefix, const size_t num_points) = 0;
  virtual int store(const std::string &index_path_prefix,
                    const size_t num_points, const size_t num_fz_points,
                    const uint32_t start) = 0;

  // not synchronised, user should use lock when necvessary.
  virtual const std::vector<location_t> &
  get_neighbours(const location_t i) const = 0;
  ...
  virtual void set_neighbours(const location_t i,
                              std::vector<location_t> &neighbours) = 0;
  ...
  size_t get_total_points() { return _capacity; }
...
protected:
  ...
  size_t get_reserve_graph_degree() { return _reserve_graph_degree; }
