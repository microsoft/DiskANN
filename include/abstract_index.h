...
struct consolidation_report {
  ...
  consolidation_report(status_code status, size_t active_points,
      ...
      : _status(status), _active_points(active_points), _max_points(max_points),
...
public:
  ...
  virtual void build(const std::string &data_file,
...
#ifdef EXEC_ENV_OLS
  virtual void load(AlignedFileReader &reader, uint32_t num_threads,
                    uint32_t search_l) = 0;
#else
  virtual void load(const char *index_file, uint32_t num_threads,
                    uint32_t search_l) = 0;
#endif

  // For FastL2 search on optimized layout
  template <typename data_type>
  void search_with_optimized_layout(const data_type *query, size_t K, size_t L,
                                    uint32_t *indices);

  // Initialize space for res_vectors before calling.
  template <typename data_type, typename tag_type>
  size_t search_with_tags(const data_type *query, const uint64_t K,
  ...
  std::pair<uint32_t, uint32_t> search(const data_type *query, const size_t K,
  ...
  search_with_filters(const DataType &query, const std::string &raw_label,
  ...
  void lazy_delete(const std::vector<tag_type> &tags,
...
private:
  ...
  virtual void _set_start_points_at_random(DataType radius,
                                           uint32_t random_seed = 0) = 0;
  virtual int _get_vector_by_tag(TagType &tag, DataType &vec) = 0;
  virtual size_t _search_with_tags(const DataType &query, const uint64_t K,
                                   const uint32_t L, const TagType &tags,
