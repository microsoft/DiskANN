...
class Distance {
 public:
  DISKANN_DLLEXPORT Distance(diskann::Metric dist_metric)
      : _distance_metric(dist_metric) {}
  ...
  DISKANN_DLLEXPORT virtual float compare(const T *a, const T *b,
                                          const float normA, const float normB,
  ...
  DISKANN_DLLEXPORT virtual uint32_t post_normalization_dimension(
  ...
  DISKANN_DLLEXPORT virtual void preprocess_base_points(
      T *original_data, const size_t orig_dim, const size_t num_points);
  ...
  DISKANN_DLLEXPORT virtual void preprocess_query(const T *query_vec,
...
class AVXNormalizedCosineDistanceFloat : public Distance<float> {
 ...
 public:
  AVXNormalizedCosineDistanceFloat()
      : Distance<float>(diskann::Metric::COSINE) {}
  DISKANN_DLLEXPORT virtual float compare(const float *a, const float *b,
                                          uint32_t length) const {
  ...
  DISKANN_DLLEXPORT virtual uint32_t post_normalization_dimension(
      uint32_t orig_dimension) const override;

  DISKANN_DLLEXPORT virtual bool preprocessing_required() const;

  DISKANN_DLLEXPORT virtual void preprocess_base_points(
      float *original_data, const size_t orig_dim,
      const size_t num_points) override;

  DISKANN_DLLEXPORT virtual void preprocess_query(
      const float *query_vec, const size_t query_dim,
