...
namespace diskann {
  ...
  class Distance {
   public:
    ...
    virtual float compare(const T *a, const T *b, uint32_t length,
  ...
  class DistanceFastL2
   ...
   public:
    ...
    inline float compare(const T *a, const T *b, float norm, unsigned size,
                  float break_distance) const {
      return compare(a, b, norm, size);
    }
  };
