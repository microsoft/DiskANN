...
public:
  ...
  DISKANN_DLLEXPORT InMemFilterStore<LabelT> &
  operator=(const InMemFilterStore<LabelT> &v) = delete;
  DISKANN_DLLEXPORT
  ...
  DISKANN_DLLEXPORT inline location_t
  get_real_point_for_dummy(location_t dummy_id) {
  ...
  DISKANN_DLLEXPORT inline bool
  point_has_label_or_universal_label(location_t id,
                                     const LabelT filter_label) const {
    return point_has_label(id, filter_label) ||
           (_use_universal_label &&
            point_has_label(id, _universal_filter_label));
  }

  DISKANN_DLLEXPORT inline LabelT
