...
inline double estimate_ram_usage(size_t size, uint32_t dim, uint32_t datasize,
  ...
  double size_of_graph =
      ((double)size) * degree * sizeof(uint32_t) * defaults::GRAPH_SLACK_FACTOR;
  ...
  return OVERHEAD_FACTOR *
         (size_of_data + size_of_graph + size_of_locks + size_of_outer_vector);
}
...
public:
  ...
  DISKANN_DLLEXPORT static size_t
  get_graph_num_frozen_points(const std::string &graph_file);
  ...
  detect_common_filters(uint32_t point_id, bool search_invocation,
  ...
  DISKANN_DLLEXPORT void build(const std::string &data_file,
                               const size_t num_points_to_load,
  ...
  DISKANN_DLLEXPORT void set_start_points_at_random(T radius,
  ...
  search(const T *query, const size_t K, const uint32_t L, IDType *indices,
         float *distances = nullptr);

  // Initialize space for res_vectors before calling.
  DISKANN_DLLEXPORT size_t search_with_tags(
  ...
  DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t>
  search_with_filters(const T *query, const LabelT &filter_label,
  ...
  consolidate_deletes(const IndexWriteParameters &parameters);

  DISKANN_DLLEXPORT void prune_all_neighbors(const uint32_t max_degree,
                                             const uint32_t max_occlusion,
                                             const float alpha);
  ...
  DISKANN_DLLEXPORT void reposition_points(uint32_t old_location_start,
...
protected:
  ...
  virtual void _build(const DataType &data, const size_t num_points_to_load,
                      TagVector &tags) override;

  virtual std::pair<uint32_t, uint32_t>
  _search(const DataType &query, const size_t K, const uint32_t L,
          std::any &indices, float *distances = nullptr) override;
  virtual std::pair<uint32_t, uint32_t>
  _search_with_filters(const DataType &query,
                       const std::string &filter_label_raw, const size_t K,
                       const uint32_t L, std::any &indices,
                       float *distances) override;

  virtual int _insert_point(const DataType &data_point,
                            const TagType tag) override;
  virtual int _insert_point(const DataType &data_point, const TagType tag,
                            Labelvector &labels) override;
  ...
  virtual void _set_start_points_at_random(DataType radius,
                                           uint32_t random_seed = 0) override;

  virtual int _get_vector_by_tag(TagType &tag, DataType &vec) override;

  virtual void _search_with_optimized_layout(const DataType &query, size_t K,
                                             size_t L,
                                             uint32_t *indices) override;

  virtual size_t
  _search_with_tags(const DataType &query, const uint64_t K, const uint32_t L,
                    const TagType &tags, float *distances,
                    DataVector &res_vectors, bool use_filters = false,
  ...
  load_label_map(const std::string &map_file);
  ...
  iterate_to_fixed_point(InMemQueryScratch<T> *scratch, const uint32_t Lindex,
  ...
  void
  occlude_list(const uint32_t location, std::vector<Neighbor> &pool,
               const float alpha, const uint32_t degree, const uint32_t maxc,
               std::vector<uint32_t> &result, InMemQueryScratch<T> *scratch,
  ...
  void inter_insert(uint32_t n, std::vector<uint32_t> &pruned_list,
                    const uint32_t range, InMemQueryScratch<T> *scratch);
