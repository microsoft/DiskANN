...
inline double estimate_ram_usage(size_t size, uint32_t dim, uint32_t datasize,
  ...
  double size_of_graph =
      ((double)size) * degree * sizeof(uint32_t) * GRAPH_SLACK_FACTOR;
...
struct consolidation_report {
  ...
  consolidation_report(status_code status, size_t active_points,
      ...
      : _status(status),
        ...
        _slots_released(slots_released),
        _delete_set_size(delete_set_size),
...
class Index {
 ...
 public:
  ...
  DISKANN_DLLEXPORT Index(
      Metric m, const size_t dim, const size_t max_points = 1,
  ...
  DISKANN_DLLEXPORT Index(
      ...
      const bool dynamic_index, const IndexWriteParameters &indexParameters,
  ...
  DISKANN_DLLEXPORT static size_t get_graph_num_frozen_points(
  ...
  DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> search(
      const T *query, const size_t K, const uint32_t L, IDType *indices,
 ...
 protected:
  ...
  void build_with_data_populated(IndexWriteParameters &parameters,
                                 const std::vector<TagT> &tags);
  ...
  std::pair<uint32_t, uint32_t> iterate_to_fixed_point(
      const T *node_coords, const uint32_t Lindex,
      const std::vector<uint32_t> &init_ids, InMemQueryScratch<T> *scratch,
