...
private:
  IndexFilterParams(const std::string &save_path_prefix,
...
public:
  ...
  IndexFilterParamsBuilder &
  with_universal_label(const std::string &univeral_label) {
    this->_universal_label = univeral_label;
    return *this;
  }

  IndexFilterParamsBuilder &
  with_filter_threshold(const std::uint32_t &filter_threshold) {
  ...
  IndexFilterParamsBuilder &
  operator=(const IndexFilterParamsBuilder &) = delete;
