...
// compute l2-squared norms of data stored in row major num_points * dim, needs
// to be pre-allocated
...
void rotate_data_randomly(float *data, size_t num_points, size_t dim, float* rot_mat, float* &new_mat, bool transpose_rot = false);
...
void compute_closest_centers_in_block(const float* const data, const size_t num_points,
			     const size_t dim, const float* const centers,
			     const size_t num_centers,
			     const float* const docs_l2sq,
			     const float* const centers_l2sq,
...
// not null, then it will assume that point norms are pre-computed and use those
// values
...
//if to_subtract is 1, will subtract nearest center from each row. Else will add. Output will be in data_load iself.
...
namespace kmeans {
	...
	// And squared lengths of data points, output the closest center to each data point, update centers, and also return inverted index.
...
//If you pass NULL for closest_docs and closest_center, it will NOT return the results, else it will assume appriate allocation as closest_docs = new vector<size_t> [num_centers], and closest_center = new size_t[num_points]
//Final centers are output in centers as row major num_centers * dim
//
float run_lloyds(float* data, size_t num_points, size_t dim, float* centers, const size_t num_centers, const size_t max_reps, std::vector<size_t>* closest_docs,  uint32_t* closest_center); 

//assumes already memory allocated for pivot_data as new float[num_centers*dim] and select randomly num_centers points as pivots
