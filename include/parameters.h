...
class IndexSearchParams
{
  public:
    IndexSearchParams(const uint32_t initial_search_list_size, const uint32_t num_search_threads,
                      ...
                      const uint32_t clustering_threshold = 0, const uint32_t dist_cmp_cutoff=defaults::DIST_CMP_CUTOFF)
        : initial_search_list_size(initial_search_list_size), num_search_threads(num_search_threads),
          ...
          clustering_threshold(clustering_threshold), dist_cmp_cutoff(dist_cmp_cutoff)
    {
    }
    ...
    const uint32_t dist_cmp_cutoff{defaults::DIST_CMP_CUTOFF};
};
