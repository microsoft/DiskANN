...
void aggregate_coords(const std::vector<unsigned> &ids,
...
generate_pq_pivots(const float *const train_data, size_t num_train,
...
DISKANN_DLLEXPORT int
generate_opq_pivots(const float *train_data, size_t num_train, unsigned dim,
                    unsigned num_centers, unsigned num_pq_chunks,
                    std::string opq_pivots_path, bool make_zero_mean = false);

DISKANN_DLLEXPORT int
generate_pq_pivots_simplified(const float *train_data, size_t num_train,
...
DISKANN_DLLEXPORT int generate_pq_data_from_pivots_simplified(
    const float *data, const size_t num, const float *pivot_data,
...
void generate_quantized_data(const std::string &data_file_to_use,
                             ...
                             const double p_val, const uint64_t num_pq_chunks,
                             const bool use_opq,
