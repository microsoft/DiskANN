...
class AbstractPQDistance  // FixedChunkPQTable
{
 public:
  ...
  // Has to be < ndim
  virtual uint32_t get_num_chunks();
  ...
  // Workhorse
  virtual float compare(PQScratch<data_t> *pq_scratch,
                        const std::vector<uint8_t> &data,
                        std::vector<float> &dists_out) = 0;

  virtual float distance(const float *query_vec, uint8_t *base_vec) = 0;
...
template <typename T>
struct PQScratch : public AbstractScratch {
  ...
  PQScratch(size_t graph_degree, size_t aligned_dim) {
    ...
    diskann::alloc_aligned((void **)&aligned_dist_scratch,
                           (size_t)graph_degree * sizeof(float), 256);
    diskann::alloc_aligned((void **)&aligned_query_float,
                           aligned_dim * sizeof(float), 8 * sizeof(float));
    diskann::alloc_aligned((void **)&rotated_query, aligned_dim * sizeof(float),
                           8 * sizeof(float));
