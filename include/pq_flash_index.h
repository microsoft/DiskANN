...
template <typename T, typename LabelT = uint32_t> class PQFlashIndex
{
  public:
    ...
    DISKANN_DLLEXPORT std::unique_ptr<ANNErrorCode> cached_beam_search(const T *query, const uint64_t k_search, const uint64_t l_search,
    ...
    DISKANN_DLLEXPORT std::pair<std::unique_ptr<ANNErrorCode>, LabelT> get_converted_label(const std::string &filter_label);
