...
#ifdef EXEC_ENV_OLS
        diskann::load_bin<_u32>(files, rearrangement_file, rearrangement, numr, numc);
#else
...
#ifdef EXEC_ENV_OLS
        diskann::load_bin<_u32>(files, chunk_offset_file, chunk_offsets, numr, numc);
#else
      ...
      for (_u64 chunk = 0; chunk < n_chunks; chunk++) {
        ...
        for (_u64 j = chunk_offsets[chunk]; j < chunk_offsets[chunk + 1]; j++) {
          ...
          for (_u64 idx = 0; idx < 256; idx++) {
            ...
            double diff = centers_dim_vec[idx] -
