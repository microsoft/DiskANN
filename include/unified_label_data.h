...
struct filter_resolution_result
{
    // True when at least one requested label string resolved through the
    // dictionary. Callers use this to report invalid-label telemetry.
    bool label_valid = false;

    // True when search can proceed through a resolved label or the universal
    // label. False means the caller should return a successful empty result.
...
class unified_label_data_base
{
  public:
    ...
    // medoids in a single dictionary probe per string. Unknown labels are
    // ignored, matching the legacy filtered-search path. If no requested label
    // resolves and a universal label with a dictionary medoid exists, both are
    // inserted into the outputs. Otherwise the caller returns a successful
    // empty result.
    filter_resolution_result resolve_filters(const std::vector<std::string> &filter_label_strings,
    ...
    virtual std::unique_ptr<filter_match_proxy> make_match_proxy(const std::vector<uint32_t> &filter_label_ints) = 0;
...
class unified_label_data_bitmask final : public unified_label_data_base
{
  public:
    ...
    std::unique_ptr<filter_match_proxy> make_match_proxy(const std::vector<uint32_t> &filter_label_ints) override;
