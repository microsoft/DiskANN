...
template<typename T>
void save_Tvecs_plain(const char *filename, T *data, size_t npts,
                      size_t ndims) {
...
template<typename T>
void load_Tvecs_plain(const char *filename, T *&data, size_t &npts,
                      size_t &ndims) {
  typedef uint64_t _u64;
  typedef uint32_t _u32;
  std::string      fname(filename);
  ...
  std::cout << "filesize: " << fsize << ", vec_size: " << vec_size
            << ", npts : " << npts << ", ndims : " << dims_u32 << '\n';
...
// plain loads data as npts X ndims array from filename -> data (after
// conversion)
template<typename InType, typename OutType>
void load_Tvecs_plain(const char *filename, OutType *&data, size_t &npts,
                      size_t &ndims) {
  typedef uint64_t _u64;
  typedef uint32_t _u32;
  std::string      fname(filename);
...
// conversion)
template<typename InType, typename OutType>
void load_bin_plain(const char *filename, OutType *&data, size_t &npts,
                    size_t &ndims) {
  typedef uint64_t _u64;
  typedef uint32_t _u32;
  std::string      fname(filename);
...
unsigned load_file_into_data(const char *filepath, Datatype *&datapath,
                             size_t &num_rows, size_t &num_dims) {
  ...
  if (cur_file.find(fvecs) != std::string::npos) {
    ...
    load_Tvecs_plain<uint32_t, Datatype>(filepath, datapath, num_rows,
                                         num_dims);
    return_type = 3;
  } else {
    return_type = -1;
