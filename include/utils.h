...
#endif
    auto message = std::string("Failed to open file") + filename +
                   " for write because " + buff + ", ret=" + ret;
...
inline void convert_labels_string_to_int(const std::string &inFileName,
  ...
  if (unv_label != "")
    string_int_map[unv_label] =
        0; // if universal label is provided map it to 0 always
...
DISKANN_DLLEXPORT void get_bin_metadata(AlignedFileReader &reader, size_t &npts,
                                        size_t &ndim, size_t offset = 0);
template <typename T>
DISKANN_DLLEXPORT void load_bin(AlignedFileReader &reader, T *&data,
                                size_t &npts, size_t &ndim, size_t offset = 0);
template <typename T>
...
copy_aligned_data_from_file(AlignedFileReader &reader, T *&data, size_t &npts,
                            size_t &dim, const size_t &rounded_dim,
                            size_t offset = 0);
...
inline void
prune_truthset_for_range(const std::string &bin_file, float range,
...
inline void load_range_truthset(const std::string &bin_file,
  ...
  size_t expected_file_size = 2 * sizeof(uint32_t) + gt_num * sizeof(uint32_t) +
  ...
  for (uint32_t i = 0; i < gt_num; i++) {
    ...
    if (gt_count[i] != 0)
      reader.read((char *)groundtruth[i].data(),
                  sizeof(uint32_t) * gt_count[i]);
  }
}
...
DISKANN_DLLEXPORT double
calculate_range_search_recall(unsigned num_queries,
                              std::vector<std::vector<uint32_t>> &groundtruth,
...
inline void load_aligned_bin_impl(std::basic_istream<char> &reader,
  ...
  diskann::cout << "allocating aligned memory of " << allocSize << " bytes... "
                << std::flush;
...
template <typename T>
inline size_t save_data_in_base_dimensions(const std::string &filename, T *data,
...
inline std::string get_tag_string(const tag_uint128 &tag) {
  std::string str =
      std::to_string(tag._data2) + "_" + std::to_string(tag._data1);
  return str;
}
...
read_file_to_vector_of_strings(const std::string &filename,
                               bool unique = false) {
  ...
  if (filename != "") {
    ...
    throw diskann::ANNException(
        std::string("Failed to open file. filename can not be blank"), -1);
  }
  return result;
}

inline void clean_up_artifacts(tsl::robin_set<std::string> paths_to_clean,
                               tsl::robin_set<std::string> path_suffixes) {
  try {
    for (const auto &path : paths_to_clean) {
      for (const auto &suffix : path_suffixes) {
        ...
        if (std::remove(curr_path_to_clean.c_str()) != 0)
          diskann::cout << "Warning: Unable to remove file :"
                        << curr_path_to_clean << std::endl;
      }
    }
    diskann::cout << "Cleaned all artifacts" << std::endl;
  } catch (const std::exception &e) {
    diskann::cout << "Warning: Unable to clean all artifacts " << e.what()
                  << std::endl;
  }
}

template <typename T> inline const char *diskann_type_to_name() = delete;
template <> inline const char *diskann_type_to_name<float>() { return "float"; }
template <> inline const char *diskann_type_to_name<uint8_t>() {
  return "uint8";
}
template <> inline const char *diskann_type_to_name<int8_t>() { return "int8"; }
