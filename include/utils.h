                                        ...
                                        uint32_t& unv_label_id)
{
    ...
    //   2. `std::istringstream new_iss(line)` per row: replaced with a manual
    //      pointer scan over the line for ','.
    //   3. `token.erase(remove(...))` per token (three scans): replaced with a
    //      single strip of a trailing '\r' at the end of each line
    //      (that's the only place '\r' can survive std::getline on Windows).
    //   4. `label_writer << lbls[j]` per int (stdio format): replaced with a
    //      manual to-decimal writer.
    //
    ...
    auto append_uint32 = [](std::vector<char> &dst, uint32_t v) {
        // Manual decimal formatter — avoids stdio locale overhead.
        if (v == 0)
        {
            dst.push_back('0');
            return;
        }
        char tmp[11]; // uint32 max: 10 digits
        size_t n = 0;
        while (v > 0)
        {
    ...
    {
        // Strip a single trailing '\r' if present (Windows CRLF from source).
        if (!line.empty() && line.back() == '\r')
        {
            line.pop_back();
        }

        // Manual scan for ',' — avoids istringstream construction per row.
        lbls.clear();
        ...
        while (p < end)
        {
            ...
            size_t tok_len = comma ? static_cast<size_t>(comma - p) : static_cast<size_t>(end - p);

            std::string token(p, tok_len);
            auto it = string_int_map.find(token);
            uint32_t id;
            if (it == string_int_map.end())
            {
                id = static_cast<uint32_t>(string_int_map.size()) + 1; // never 0
                string_int_map.emplace(std::move(token), id);
            }
            else
            {
                id = it->second;
