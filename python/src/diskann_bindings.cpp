...
struct DiskANNIndex {
  ...
  int load_index(const std::string &index_path_prefix, const int num_threads,
    ...
    int load_success =
        pq_flash_index->load(num_threads, index_path_prefix.c_str());
    if (load_success != 0) {
  ...
  auto search(py::array_t<T, py::array::c_style | py::array::forcecast> &query,
              const uint64_t knn, const uint64_t l_search,
    ...
    for (uint64_t i = 0; i < knn; ++i) r(i) = (unsigned)u64_ids[i];
...
template <class T>
struct DynamicInMemIndex {
  Index<T, IdT, filterT> *_index;
  const IndexWriteParameters write_params;

  DynamicInMemIndex(Metric m, const size_t dim, const size_t max_points,
                    const IndexWriteParameters &index_parameters,
                    const uint32_t initial_search_list_size,
                    const uint32_t search_threads,
  ...
  ~DynamicInMemIndex() { delete _index; }

  int insert(py::array_t<T, py::array::c_style | py::array::forcecast> &vector,
             const IdT id) {
    return _index->insert_point(vector.data(), id);
  }

  int mark_deleted(const IdT id) { return _index->lazy_delete(id); }
  ...
  auto search(py::array_t<T, py::array::c_style | py::array::forcecast> &query,
    ...
    _index->search_with_tags(query.data(), knn, l_search, ids.mutable_data(),
                             dists.mutable_data(), empty_vector);
    return std::make_pair(ids, dists);
  }
...
#pragma omp parallel for schedule(dynamic, 1)
    for (int64_t i = 0; i < (int64_t)num_queries; i++) {
      _index->search_with_tags(queries.data(i), knn, l_search,
                               ids.mutable_data(i), dists.mutable_data(i),
                               empty_vector);
    }
...
template <class T>
struct StaticInMemIndex {
  Index<T, IdT, filterT> *_index;

  StaticInMemIndex(Metric m, const std::string &data_path,
                   IndexWriteParameters &index_parameters) {
  ...
  ~StaticInMemIndex() { delete _index; }
  ...
  auto search(py::array_t<T, py::array::c_style | py::array::forcecast> &query,
    ...
    py::array_t<float> dists(knn);
    std::vector<T *> empty_vector;
    _index->search(query.data(), knn, l_search, ids.mutable_data(),
                   dists.mutable_data());
    return std::make_pair(ids, dists);
  }
...
#pragma omp parallel for schedule(dynamic, 1)
    for (int64_t i = 0; i < (int64_t)num_queries; i++) {
      _index->search(queries.data(i), knn, l_search, ids.mutable_data(i),
                     dists.mutable_data(i));
    }
...
PYBIND11_MODULE(_diskannpy, m) {
  ...
  py::class_<StaticInMemIndex<float>>(m, "DiskANNStaticInMemFloatIndex")
      ...
      .def("search", &StaticInMemIndex<float>::search, py::arg("query"),
           py::arg("knn"), py::arg("l_search"))
      .def("batch_search", &StaticInMemIndex<float>::batch_search,
           py::arg("queries"), py::arg("num_queries"), py::arg("knn"),
           py::arg("l_search"), py::arg("num_threads"));
  ...
  py::class_<StaticInMemIndex<int8_t>>(m, "DiskANNStaticInMemInt8Index")
      ...
      .def("search", &StaticInMemIndex<int8_t>::search, py::arg("query"),
           py::arg("knn"), py::arg("l_search"))
      .def("batch_search", &StaticInMemIndex<int8_t>::batch_search,
           py::arg("queries"), py::arg("num_queries"), py::arg("knn"),
           py::arg("l_search"), py::arg("num_threads"));
  ...
  py::class_<StaticInMemIndex<uint8_t>>(m, "DiskANNStaticInMemUint8Index")
      ...
      .def("search", &StaticInMemIndex<uint8_t>::search, py::arg("query"),
           py::arg("knn"), py::arg("l_search"))
      .def("batch_search", &StaticInMemIndex<uint8_t>::batch_search,
           py::arg("queries"), py::arg("num_queries"), py::arg("knn"),
           py::arg("l_search"), py::arg("num_threads"));

  py::class_<DynamicInMemIndex<float>>(m, "DiskANNDynamicInMemFloatIndex")
      .def(py::init(
          [](diskann::Metric metric, const size_t dim, const size_t max_points,
             const IndexWriteParameters &index_parameters,
             const uint32_t initial_search_list_size,
             const uint32_t search_threads, const bool concurrent_consolidate) {
            return std::unique_ptr<DynamicInMemIndex<float>>(
                new DynamicInMemIndex<float>(
                    metric, dim, max_points, index_parameters,
                    initial_search_list_size, search_threads,
                    concurrent_consolidate));
          }))
      .def("search", &DynamicInMemIndex<float>::search, py::arg("query"),
           py::arg("knn"), py::arg("l_search"))
      .def("batch_search", &DynamicInMemIndex<float>::batch_search,
           py::arg("queries"), py::arg("num_queries"), py::arg("knn"),
           py::arg("l_search"), py::arg("num_threads"))
      .def("insert", &DynamicInMemIndex<float>::insert, py::arg("vector"),
           py::arg("id"))
      .def("mark_deleted", &DynamicInMemIndex<float>::mark_deleted,
           py::arg("id"))
      ...
      .def(py::init(
          [](diskann::Metric metric, const size_t dim, const size_t max_points,
            ...
            return std::unique_ptr<DynamicInMemIndex<int8_t>>(
                new DynamicInMemIndex<int8_t>(
                    metric, dim, max_points, index_parameters,
                    initial_search_list_size, search_threads,
                    concurrent_consolidate));
          }))
      .def("search", &DynamicInMemIndex<int8_t>::search, py::arg("query"),
           py::arg("knn"), py::arg("l_search"))
      .def("batch_search", &DynamicInMemIndex<int8_t>::batch_search,
           py::arg("queries"), py::arg("num_queries"), py::arg("knn"),
           py::arg("l_search"), py::arg("num_threads"))
      .def("insert", &DynamicInMemIndex<int8_t>::insert, py::arg("vector"),
           py::arg("id"))
      .def("mark_deleted", &DynamicInMemIndex<int8_t>::mark_deleted,
           py::arg("id"))
      .def("consolidate_delete",
           &DynamicInMemIndex<int8_t>::consolidate_delete);
      ...
      .def(py::init(
          [](diskann::Metric metric, const size_t dim, const size_t max_points,
            ...
            return std::unique_ptr<DynamicInMemIndex<uint8_t>>(
                new DynamicInMemIndex<uint8_t>(
                    metric, dim, max_points, index_parameters,
                    initial_search_list_size, search_threads,
                    concurrent_consolidate));
          }))
      .def("search", &DynamicInMemIndex<uint8_t>::search, py::arg("query"),
           py::arg("knn"), py::arg("l_search"))
      .def("batch_search", &DynamicInMemIndex<uint8_t>::batch_search,
           py::arg("queries"), py::arg("num_queries"), py::arg("knn"),
           py::arg("l_search"), py::arg("num_threads"))
      .def("insert", &DynamicInMemIndex<uint8_t>::insert, py::arg("vector"),
           py::arg("id"))
      .def("mark_deleted", &DynamicInMemIndex<uint8_t>::mark_deleted,
           py::arg("id"))
      .def("consolidate_delete",
           &DynamicInMemIndex<uint8_t>::consolidate_delete);
