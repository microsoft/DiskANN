...
StaticDiskIndex<DT>::StaticDiskIndex(const diskann::Metric metric, const std::string &index_path_prefix, const uint32_t num_threads,
              const size_t num_nodes_to_cache, const uint32_t cache_mechanism) :
      _reader(std::make_shared<PlatformSpecificAlignedFileReader>()),
      _index(_reader, metric)
    {
...
template <typename DT>
void StaticDiskIndex<DT>::cache_bfs_levels(const size_t num_nodes_to_cache)
{
...
void StaticDiskIndex<DT>::cache_sample_paths(const size_t num_nodes_to_cache, const std::string &warmup_query_file,
                            const uint32_t num_threads)
    {
        ...
        std::vector<uint32_t> node_list;
        _index.generate_cache_list_from_sample_queries(warmup_query_file, 15, 4, num_nodes_to_cache,
...
NeighborsAndDistances<StaticIdType> StaticDiskIndex<DT>::search(
        py::array_t<DT, py::array::c_style | py::array::forcecast> &query, const uint64_t knn,
        const uint64_t complexity, const uint64_t beam_width)
    {
        ...
        _index.cached_beam_search(query.data(), knn, complexity, u64_ids.data(), dists.mutable_data(),
...
NeighborsAndDistances<StaticIdType> StaticDiskIndex<DT>::batch_search(
        ...
        const uint64_t knn, const uint64_t complexity, const uint64_t beam_width, const uint32_t num_threads)
    {
...
#pragma omp parallel for schedule(dynamic, 1) default(none) shared(num_queries, queries, knn, complexity, u64_ids, dists, beam_width)
        for (int64_t i = 0; i < (int64_t)num_queries; i++)
        {
            _index.cached_beam_search(queries.data(i), knn, complexity, u64_ids.data() + i * knn,
