...
fn bench_original_parser(c: &mut Criterion) {
    ...
    // Benchmark Pest parser with simple filter
    group.bench_function("pest_simple", |b| {
        b.iter(|| {
            let ast = pest_parser::parse_query_filter(&simple_filter);
            assert!(ast.is_some());
        })
    });

    // Benchmark Pest parser with complex filter
    group.bench_function("pest_complex", |b| {
        b.iter(|| {
            let ast = pest_parser::parse_query_filter(&complex_filter);
            assert!(ast.is_some());
        })
    });

    // Benchmark the string parsing specific to Pest
    let simple_filter_str = serde_json::to_string(&simple_filter).unwrap();
    let complex_filter_str = serde_json::to_string(&complex_filter).unwrap();

    // Benchmark Pest parser string parsing (without JSON conversion)
    group.bench_function("pest_str_simple", |b| {
        b.iter(|| {
            let ast = pest_parser::parse_query_filter_str(&simple_filter_str);
            assert!(ast.is_ok());
        })
    });

    group.bench_function("pest_str_complex", |b| {
        b.iter(|| {
            let ast = pest_parser::parse_query_filter_str(&complex_filter_str);
            assert!(ast.is_ok());
        })
    });
...
fn bench_parsers_comparison(c: &mut Criterion) {
    let filters = vec![
        ...
        // Logical operators
        ("logical", json!({"$and": [{"a": {"$eq": 1}}, {"b": {"$eq": 2}}]})),
    ...
    for (name, filter) in filters {
        ...
        group.bench_with_input(BenchmarkId::new("optimized", name), &filter, |b, filter| {
            b.iter(|| {
                let ast = optimized::parse_query_filter(filter);
                assert!(ast.is_some());
            })
        });
        
        group.bench_with_input(BenchmarkId::new("pest", name), &filter, |b, filter| {
            b.iter(|| {
                let ast = pest_parser::parse_query_filter(filter);
                assert!(ast.is_some());
            })
        });
    }
...
fn generate_random_filter() -> serde_json::Value {
    let mut rng = rand::thread_rng();
    let fields = ["a", "b", "c", "price", "in_stock", "category", "brand", "specs.cpu", "specs.memory.size"];
    let categories = ["laptop", "desktop", "tablet", "phone"];
    let brands = ["Apple", "Dell", "Lenovo", "HP"];
    let cpus = ["i5", "i7", "i9"];
    ...
    let value = match field {
        ...
        "price" => {
            let price = rng.gen_range(1000.0..2000.0);
            json!({"$lt": price})
        },
        "in_stock" => {
            let is_in_stock = rng.gen_bool(0.5);
            json!({"$eq": is_in_stock})
        },
        "category" => {
...
fn bench_dynamic(c: &mut Criterion) {
    ...
    group.bench_function("dynamic_optimized", |b| {
        b.iter_batched(
            || (generate_random_label(), generate_random_filter()),
            |(label, filter)| {
                let _ast = optimized::parse_query_filter(&filter).unwrap();
                eval_query_expr(&_ast, &label);
            },
            criterion::BatchSize::SmallInput,
        )
    ...
    group.bench_function("dynamic_focused", |b| {
        b.iter_batched(
            || (generate_random_label(), generate_random_filter()),
            |(label, filter)| {
                let _ast = parse_query_filter(&filter).unwrap();
    ...
    group.bench_function("dynamic_pest", |b| {
        b.iter_batched(
            || (generate_random_label(), generate_random_filter()),
            |(label, filter)| {
                let _ast = pest_parser::parse_query_filter(&filter).unwrap();
                eval_query_expr(&_ast, &label);
            },
            criterion::BatchSize::SmallInput,
        )
