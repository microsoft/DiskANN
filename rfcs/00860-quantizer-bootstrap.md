...
Indexes that use certain quantizers must have a minimum number of vectors before
they can build quantization tables and start inserting vectors. Bootstrapping is
the process of incrementally building an index that starts empty, operates on
non-quantized vectors until enough vectors are present to build quantization
tables, and then transitions to quantized operation.

## Motivation

### Background

Some of DiskANN's quantizers require statistical information in order to build
...
quantization tables are calculated and a graph index is built with DiskANN. This
is a lot of non-trivial infrastructure to provide in order to integrate DiskANN
into a working system.

### Problem Statement

This RFC proposes creating DiskANN DataProviders that operate in a quantization
bootstrap mode where they operate on full precision vectors until sufficient
vectors exist to create quantization tables, and then seamlessly transitions to
a quantized index.

By moving this into DataProviders, implementation burden can be reduced
significantly. By documenting and having examples of this design, DataProvider
implementors avoid having to re-invent the wheel and focus on how to achieve the
bootstrap data management in the best way for their own systems.

The index will operate in three different phases. In Phase 1, the index operates
in full precision mode only until sufficient vectors exist to build quantization
tables. During Phase 2, quantization tables will be built and vectors will be
quantized on insert; pre-existing unquantized vectors will be quantized in the
background. Once all vectors are quantized, Phase Three begins the normal
operation of the quantized index. In every phase, the index is fully operations,
capable of servicing inserts and queries.
...
1. **Switching Strategies Behind the Scenes**: DiskANN needs to start by using
   a full precision only strategy during Phase One, and switching to a
   quantized-only or hybrid strategy for Phase Two. This should not require 
   the caller to explicitly pass different strategies.
2. **Quantization Backfill**: During Phase Two, previously inserted full
   precision vectors will need to be quantized. As background jobs are a
   performance concern, how exactly this is accomplished must be customizable by
   the data provider.
...
Instead of passing different strategies to `insert()` and `search()` based on the 
current phase, instead a single strategy will be used that changes behavior based
...
impl SetElement<&[f32]> for ExampleProvider {
     type SetError = ExampleProviderError;
...
Since vector data may be in one of two representations, the accessor exposes its
elements as raw bytes (an `ElementRef<'a>` of `&'a [u8]`, with owned values held
as an over-aligned `Poly<[u8], AlignToEight>`), and the data provider interprets
based on data size. An alternative would be to use an enum type with two
variants for more type safety perhaps with more runtime branching. The distance
and query computers will also need modifications to accept both vector
representations, and in the case of query computer the representation much match
that of the query.
...
missing quantized representations so that it can generate them once a quantizer
is trained.
...
One possibility is record the range of possible unquantized vectors when phase
two starts, and then during asynchronous backfill, scan vectors in that range
and quantize any missing ones. For example, in diskann-garnet, internal IDs
start from 0 and cannot be reused until phase 3. So when phase two begins, it
needs to set `max_id_for_backfill` to the last ID created so far. Then when
backfill happens, it can check each ID in the range and quantize any unquantized
vectors. Once all vectors in that range are quantized, it then enables ID reuse.
...
worth the cost. This was previously the direction of the caching provider, which
proved very unwieldy.
...
impact during Phase Three to performance. Specifically, during phase one, the
index should have the same performance profile as a full precision index. In
phase 3, the index should have the same performance profile as a fully quantized
index. Phase 2 will had minor overhead to `insert()` over the normal full
precision index because it must quantize and write a quantized term in addition
to its normal functions, as well as the extra compute of the backfill scan and
quantization.
