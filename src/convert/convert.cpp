...
int aux_main(int argc, char** argv) {
  ...
  diskann::cout << "Loading source vector from file " << argv[2] << std::endl;
  ...
  diskann::convert_types<source_type, dest_type>(sourceVec, destVec, num_points,
                                             num_dims);
  diskann::cout << "Converted vector from type " << argv[1] << "to type " << argv[3]
            << std::endl;
  diskann::save_bin<dest_type>(argv[4], destVec, num_points, num_dims);
  diskann::cout << "Saved converted vector to file " << argv[4] << std::endl;
  return 0;
}
...
int main(int argc, char** argv) {
  ...
  if (sourceType == destType) {
    diskann::cout << "Nothing to do. source and destination types are the same."
              << std::endl;
    return 0;
  }
  ...
  if (sourceType == "int8" && destType == "uint8") {
    ...
    diskann::cout << "One of the source or destination types is unknown: "
