...
void breakup_dense_points(const std::string data_file,
                          const std::string labels_file, uint32_t density,
  ...
  if (dummy_pt_ids.size() != 0) {
    ...
    if (ptr == nullptr) {
      ...
      free(data);
      data = nullptr;
      throw new diskann::ANNException("Realloc failed while expanding data.",
                                      -1, __FUNCTION__, __FILE__, __LINE__);
    } else {
      data = ptr;
    }
...
int build_merged_vamana_index(
  ...
  for (int p = 0; p < num_parts; p++) {
    ...
    if (!use_filters) {
      ...
      _index.build_filtered_index(shard_base_file.c_str(), shard_labels_file,
                                  shard_base_pts);
    }
  ...
  diskann::merge_shards(merged_index_prefix + "_subshard-", "_mem.index",
                        ...
                        num_parts, R, mem_index_path, medoids_file, use_filters,
...
void create_disk_layout(const std::string base_file,
  ...
  if (nnodes_per_sector > 0) { // Write multiple nodes per sector
    for (uint64_t sector = 0; sector < n_sectors; sector++) {
      ...
      for (uint64_t sector_node_id = 0;
        ...
        *(uint32_t *)(node_buf.get() + ndims_64 * sizeof(T)) =
        ...
        // write nhood next
        memcpy(node_buf.get() + ndims_64 * sizeof(T) + sizeof(uint32_t),
    ...
    for (uint64_t i = 0; i < npts_64; i++) {
      ...
      memset(multisector_buf.get(), 0,
             nsectors_per_node * defaults::SECTOR_LEN);
  ...
  diskann::save_bin<uint64_t>(output_file, output_file_meta.data(),
