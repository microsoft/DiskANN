...
//
template <typename T>
float Distance<T>::compare(const T *a, const T *b, const float normA,
                           const float normB, uint32_t length) const {
  throw std::logic_error("This function is not implemented.");
}

template <typename T>
uint32_t Distance<T>::post_normalization_dimension(
    uint32_t orig_dimension) const {
  return orig_dimension;
}

template <typename T>
diskann::Metric Distance<T>::get_metric() const {
...
template <typename T>
void Distance<T>::preprocess_base_points(T *original_data,
                                         const size_t orig_dim,
                                         const size_t num_points) {}

template <typename T>
void Distance<T>::preprocess_query(const T *query_vec, const size_t query_dim,
                                   T *scratch_query) {
...
template <typename T>
float SlowDistanceL2<T>::compare(const T *a, const T *b,
                                 uint32_t length) const {
  float result = 0.0f;
  for (uint32_t i = 0; i < length; i++) {
...
uint32_t AVXNormalizedCosineDistanceFloat::post_normalization_dimension(
    uint32_t orig_dimension) const {
  return orig_dimension;
}
...
void AVXNormalizedCosineDistanceFloat::preprocess_base_points(
    float *original_data, const size_t orig_dim, const size_t num_points) {
  for (uint32_t i = 0; i < num_points; i++) {
    normalize((float *)(original_data + i * orig_dim), orig_dim);
  }
}

void AVXNormalizedCosineDistanceFloat::preprocess_query(const float *query_vec,
                                                        const size_t query_dim,
                                                        float *query_scratch) {
  normalize_and_copy(query_vec, query_dim, query_scratch);
}

void AVXNormalizedCosineDistanceFloat::normalize_and_copy(
    const float *query_vec, const uint32_t query_dim,
...
diskann::Distance<uint8_t> *get_distance_function(diskann::Metric m) {
  if (m == diskann::Metric::L2) {
    ...
    std::stringstream stream;
    stream << "Only L2 and cosine supported for uint32_t byte vectors."
           << std::endl;
    diskann::cerr << stream.str() << std::endl;
    throw diskann::ANNException(stream.str(), -1, __FUNCSIG__, __FILE__,
                                __LINE__);
