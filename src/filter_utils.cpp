...
void generate_label_indices(path input_data_path, path final_index_path_prefix,
  ...
  diskann::IndexWriteParameters label_index_build_parameters =
  ...
  for (const auto &lbl : all_labels) {
    ...
    diskann::Index<T> index(diskann::Metric::L2, dimension,
                            number_of_label_points,
                            std::make_shared<diskann::IndexWriteParameters>(
                                label_index_build_parameters),
                            nullptr, 0, false, false, false, false, 0, false);
...
load_label_index(path label_index_path, uint32_t label_number_of_points) {
  ...
  while (bytes_read != index_file_size) {
    ...
    label_index_stream.read((char *)&current_node_num_neighbors,
                            sizeof(uint32_t));
    nodes_read++;
...
std::tuple<std::vector<std::vector<LabelT>>, tsl::robin_set<LabelT>>
parse_formatted_label_file(std::string label_file) {
  ...
  diskann::cout << "Identified " << labels.size() << " distinct label(s)"
                << std::endl;
...
template DISKANN_DLLEXPORT
    std::tuple<std::vector<std::vector<uint16_t>>, tsl::robin_set<uint16_t>>
    parse_formatted_label_file(path label_file);

template DISKANN_DLLEXPORT void generate_label_indices<float>(
    path input_data_path, path final_index_path_prefix, label_set all_labels,
    uint32_t R, uint32_t L, float alpha, uint32_t num_threads);
template DISKANN_DLLEXPORT void generate_label_indices<uint8_t>(
    path input_data_path, path final_index_path_prefix, label_set all_labels,
    uint32_t R, uint32_t L, float alpha, uint32_t num_threads);
template DISKANN_DLLEXPORT void generate_label_indices<int8_t>(
    path input_data_path, path final_index_path_prefix, label_set all_labels,
    uint32_t R, uint32_t L, float alpha, uint32_t num_threads);
