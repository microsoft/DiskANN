 ...
 */
parse_label_file_return_values parse_raw_label_file(path label_data_path, std::string universal_label)
{
...
std::tuple<std::vector<std::vector<LabelT>>, tsl::robin_set<LabelT>> parse_formatted_label_file(path label_file)
{
...
parse_formatted_label_file<uint16_t>(path label_file);

template DISKANN_DLLEXPORT std::tuple<std::vector<std::vector<uint32_t>>, tsl::robin_set<uint32_t>>
parse_formatted_label_file<uint32_t>(path label_file);
