...
namespace NSG {
  ...
  template<typename T, typename NhoodType>
  void FlashNSG<T, NhoodType>::cache_bfs_levels(_u64 nlevels) {
    assert(nlevels > 1);
  ...
  template<typename T, typename NhoodType>
  void FlashNSG<T, NhoodType>::load(const char *filename) {
  ...
  template<typename T, typename NhoodType>
  std::pair<int, int> FlashNSG<T, NhoodType>::beam_search(
  ...
  template<typename T, typename NhoodType>
  std::pair<int, int> FlashNSG<T, NhoodType>::cached_beam_search(
      const float *query, const _u64 k_search, const _u64 l_search,
      _u32 *indices, const _u64 beam_width, QueryStats *stats) {
    if (nbrs_cache == nullptr || coords_cache == nullptr) {
      std::cerr << "Run FlashNSG<T, NhoodType>::cache_bfs_levels before "
  ...
  template class FlashNSG<int8_t, DiskNhood<int8_t>>;
}
