...
#ifdef EXEC_ENV_OLS
template <typename data_t>
location_t Index<data_t>::load_impl(AlignedFileReader &reader) {
...
void InMemDataStore<data_t>::prefetch_vector(const location_t loc) {
  diskann::prefetch_vector((const char *)_data + _aligned_dim * (size_t)loc,
...
template <typename data_t>
void InMemDataStore<data_t>::get_distance(const data_t *query,
                                          const location_t *locations,
                                          const uint32_t location_count,
                                          float *distances) const {
  for (location_t i = 0; i < location_count; i++) {
    distances[i] = _distance_fn->compare(
        query, _data + locations[i] * _aligned_dim, this->_aligned_dim);
  }
}
...
float InMemDataStore<data_t>::get_distance(const location_t loc1,
  ...
  return _distance_fn->compare(_data + loc1 * _aligned_dim,
                               _data + loc2 * _aligned_dim, this->_aligned_dim);
}
