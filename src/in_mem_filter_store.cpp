...
namespace diskann {
  ...
  DISKANN_DLLEXPORT std::unique_ptr<char[]> get_file_content(const std::string &filename,
                                           uint64_t &         file_size);

  template<typename LabelT>
  InMemFilterStore<LabelT>::~InMemFilterStore() {
  ...
  const std::vector<location_t> &InMemFilterStore<LabelT>::get_medoids_of_label(
    ...
    if (_filter_to_medoid_ids.find(label) != _filter_to_medoid_ids.end()) {
      ...
      std::stringstream ss;
      ss << "Could not find " << label << " in filters_to_medoid_ids map."
         << std::endl;
      diskann::cerr << ss.str();
  ...
  template<typename LabelT>
  inline location_t InMemFilterStore<LabelT>::get_real_point_for_dummy(
      location_t dummy_id) {
  ...
  inline bool InMemFilterStore<LabelT>::point_has_label_or_universal_label(
    ...
    return point_has_label(id, filter_label) ||
           (_use_universal_label &&
            point_has_label(id, _universal_filter_label));
  }
  ...
  void InMemFilterStore<LabelT>::load_label_file(
    ...
    while (cur_pos < file_size && cur_pos != std::string_view::npos) {
      ...
      while (lbl_pos < next_pos && lbl_pos != std::string_view::npos) {
        ...
        if (next_lbl_pos ==
            std::string_view::npos)  // the last label in the whole file
        {
        ...
        label_str.assign(label_file_content.data() + lbl_pos,
  ...
  template<typename LabelT>
  void InMemFilterStore<LabelT>::load_labels_to_medoids(
      std::basic_istream<char> &medoid_stream) {
  ...
  template<typename LabelT>
  void InMemFilterStore<LabelT>::parse_universal_label(
  ...
  template<typename LabelT>
  void InMemFilterStore<LabelT>::generate_random_labels(
      std::vector<LabelT> &labels, const uint32_t num_labels,
      const uint32_t nthreads) {
    std::random_device rd;
    labels.clear();
    labels.resize(num_labels);

    uint64_t num_total_labels = _pts_to_label_offsets[_num_points - 1] +
                                _pts_to_label_counts[_num_points - 1];
    std::mt19937 gen(rd());
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(nthreads)
  ...
  template<typename LabelT>
  void InMemFilterStore<LabelT>::reset_stream_for_reading(
      std::basic_istream<char> &infile) {
    infile.clear();
    infile.seekg(0);
  }

  template<typename LabelT>
  void InMemFilterStore<LabelT>::get_label_file_metadata(
      const std::string_view &fileContent, uint32_t &num_pts,
      uint32_t &num_total_labels) {
    num_pts = 0;
    num_total_labels = 0;
  ...
  template<typename LabelT>
  bool InMemFilterStore<LabelT>::parse_stream(
