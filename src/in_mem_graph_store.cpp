...
namespace diskann {
InMemGraphStore::InMemGraphStore(const size_t total_pts,
...
InMemGraphStore::load(const std::string &index_path_prefix,
...
InMemGraphStore::get_neighbours(const location_t i) const {
  return _graph.at(i);
}

void InMemGraphStore::add_neighbour(const location_t i,
                                    location_t neighbour_id) {
...
void InMemGraphStore::set_neighbours(const location_t i,
...
void InMemGraphStore::clear_graph() { _graph.clear(); }

#ifdef EXEC_ENV_OLS
std::tuple<uint32_t, uint32_t, size_t>
InMemGraphStore::load_impl(AlignedFileReader &reader,
...
std::tuple<uint32_t, uint32_t, size_t>
InMemGraphStore::load_impl(const std::string &filename,
                           size_t expected_num_points) {
  size_t expected_file_size;
  size_t file_frozen_pts;
  uint32_t start;
  ...
  while (bytes_read != expected_file_size) {
    ...
    if (k == 0) {
      diskann::cerr << "ERROR: Point found with no out-neighbours, point#"
                    << nodes_read << std::endl;
    }
...
int InMemGraphStore::save_graph(const std::string &index_path_prefix,
  ...
  for (uint32_t i = 0; i < num_points; i++) {
    ...
    max_degree =
        _graph[i].size() > max_degree ? (uint32_t)_graph[i].size() : max_degree;
...
size_t InMemGraphStore::get_max_range_of_graph() { return _max_range_of_graph; }
