...
#pragma omp parallel for schedule(static, 65536)
  ...
  void Index<T, TagT>::batch_inter_insert(
      ...
      std::vector<unsigned> &indegree_counts, unsigned indegree_threshold,
      unsigned rise_factor, bool count) {
    ...
    for (auto des : pruned_list) {
...

      {
        ...
        if (std::find(_final_graph[des].begin(), _final_graph[des].end(), n) ==
          ...
          if (count) {
            indegree_counts[des] += 1;
            if (_final_graph[des].size() > (unsigned) (range * SLACK_FACTOR)) {
              need_to_sync[des] = 1;
            }
          } else {
            if (indegree_counts[des] > indegree_threshold) {
  ...
  void Index<T, TagT>::link(Parameters &parameters) {
    ...
    unsigned              indegree_threshold = 3;
    bool                  count = true;
    diskann::Parameters local_params;
    std::vector<unsigned> indegree_counts(_max_points + _num_frozen_pts, 0);
    for (uint32_t rnd_no = 0; rnd_no < NUM_RNDS; rnd_no++) {
      ...
      if (rnd_no == NUM_RNDS - 1) {
        ...
        std::vector<unsigned> vector_sorted(indegree_counts.size());
        partial_sort_copy(begin(indegree_counts), end(indegree_counts),
                          begin(vector_sorted), end(vector_sorted));
        indegree_threshold =
            vector_sorted[(_max_points + _num_frozen_pts) * 0.95];
        std::cout << "Threshold: " << indegree_threshold << std::endl;
      }
...
#pragma omp parallel for schedule(dynamic)
        for (_s64 node_ctr = (_s64) start_id; node_ctr < (_s64) end_id;
          ...
          if (!count && indegree_counts[node] > indegree_threshold) {
...
#pragma omp parallel for schedule(dynamic, 64)
        for (_s64 node_ctr = start_id; node_ctr < (_s64) end_id; ++node_ctr) {
          ...
          batch_inter_insert(node, pruned_list, parameters, need_to_sync,
                             indegree_counts, indegree_threshold, rise_factor,
                             count);
