...
Index<T, TagT, LabelT>::Index(Metric m, const size_t dim, const size_t max_points,
    ...
    : Index(
          IndexConfigBuilder()
              ...
              .build(),
          IndexFactory::construct_mem_datastore<T>((max_points == 0 ? (size_t)1 : max_points),
                                               dim, m),
    ...
    if (!_save_as_one_file)
    {
        ...
        if (_filtered_index)
        {
            ...
            if (_use_integer_labels)
            {
                ...
                if (_label_vector.write_to_file(integer_label_file))
                {
                    diskann::cout << "Integer labels saved to " << integer_label_file << std::endl;
                }
                else
                {
                    diskann::cerr << "Failed to save integer labels to " << integer_label_file << std::endl;
                    throw diskann::ANNException(std::string("Failed to save integer labels to ") + integer_label_file,
                                                -1);
                }
            }
        }
...
void Index<T, TagT, LabelT>::load(const IndexLoadParams & load_params)
{
    ...
    {
        uint64_t nrows_seller_file;
        parse_seller_file(old_index_seller_file, nrows_seller_file, _location_to_seller, _num_unique_sellers);
        ...
        || file_exists(integer_label_file))
    {
        ...
        {
            // load integer labels from file
            if (_label_vector.initialize_from_file(integer_label_file, label_num_pts))
            {
                diskann::cout << "Integer labels loaded from " << integer_label_file << std::endl;
                _use_integer_labels = true;
            }
            else
            {
                diskann::cerr << "Failed to load integer labels from " << integer_label_file << std::endl;
                throw diskann::ANNException(std::string("Failed to load integer labels from ") + integer_label_file,
                                            -1);
            }
    ...
    while (cur_alpha <= alpha && result.size() < degree)
    {
        ...
        for (auto iter = pool.begin(); result.size() < degree && iter != pool.end(); ++iter)
        {
            ...
            if (occlude_factor[cur_index] > cur_alpha)
            {
                ...
                if (blockers[cur_index].size() >= _num_diverse_build)
                {
                    continue;
                }
                ...
                {
                    if (blockers[cur_index].find(std::numeric_limits<uint32_t>::max()) != blockers[cur_index].end())
                    {
                        continue;
                    }
                }
          ...
          //  occlude_factor[iter - pool.begin()] = std::numeric_limits<float>::max();
            ...
            for (auto iter2 = iter + 1; iter2 != pool.end(); iter2++)
            {
                ...
                if (occlude_factor[t] > alpha)
                {
                    ...
                    if (blockers[t].size() >= _num_diverse_build)
                    {
                        continue;
                    }
                ...
                if (_filtered_index)
                {
                    ...
                    {
                        prune_allowed = _label_vector.check_label_full_contain(a, b);
                    }
                ...
                if (_dist_metric == diskann::Metric::L2 || _dist_metric == diskann::Metric::COSINE)
                {
                    ...
                    if (djk == 0 || (iter2->distance / djk) > cur_alpha)
                    {
                        if (_diverse_index)
                        {
                            auto iter_seller = _location_to_seller[iter->id];
                        ...
                        if (_attribute_diversity)
                        {
                            ...
                            if (attr_dist > _attr_dist_threshold)
...
template <typename T, typename TagT, typename LabelT>
double Index<T, TagT, LabelT>::attribute_distance(const std::vector<uint32_t> &a, const std::vector<uint32_t> &b)
{
...
void Index<T, TagT, LabelT>::build_with_data_populated(const std::vector<TagT> &tags)
{
    ...
    if (_diverse_index) {
        uint64_t nrows;
        parse_seller_file(_seller_file, nrows, _location_to_seller, _num_unique_sellers);
...
template <typename T, typename TagT, typename LabelT>
void Index<T, TagT, LabelT>::enable_integer_label()
{
    _use_integer_labels = true;
}
                                                       ...
                                                       tsl::robin_set<ValueT>* unique_values)
{
    ...
    if (infile.fail())
    {
        throw diskann::ANNException(std::string("Failed to open file ") + file_path, -1);
    }
    ...
    if (_dynamic_index)
    {
        location_to_values.resize(_max_points, std::vector<ValueT>());
    }
    else
    {
        location_to_values.resize(line_cnt, std::vector<ValueT>());
    }
    ...
    while (std::getline(infile, line))
    {
        ...
        while (getline(new_iss, token, ','))
        {
            ...
            ValueT token_as_num = (ValueT)std::stoul(token);
            vals.push_back(token_as_num);
            if (unique_values != nullptr)
            {
                unique_values->insert(token_as_num);
            }
        }
    ...
    if (unique_values != nullptr)
    {
        diskann::cout << "Identified " << unique_values->size() << " distinct value(s)" << std::endl;
    }
}
...
template <typename T, typename TagT, typename LabelT>
void Index<T, TagT, LabelT>::convert_pts_label_to_integer_vector(
    std::vector<std::vector<LabelT>>& pts_to_labels,
    integer_label_vector& int_label_vector, size_t total_labels)
{
    int_label_vector.initialize(pts_to_labels.size(), total_labels);
    for (size_t i = 0; i < pts_to_labels.size(); i++)
    {
        int_label_vector.add_labels(static_cast<std::uint32_t>(i), pts_to_labels[i]);
    }
}
                                               ...
                                               std::vector<uint32_t>& location_to_seller, uint32_t& num_unique_sellers)
{
    ...
    num_unique_sellers = static_cast<uint32_t>(sellers.size());
    num_points = (size_t)line_cnt;
    diskann::cout << "Identified " << sellers.size() << " distinct seller(s) across " << num_points << " points." << std::endl;
}

template <typename T, typename TagT, typename LabelT>
void Index<T, TagT, LabelT>::aggregate_points_by_bitmask_label(
...
template <typename T, typename TagT, typename LabelT>
void Index<T, TagT, LabelT>::aggregate_points_by_integer_label(
    std::unordered_map<LabelT, std::vector<uint32_t>>& label_to_points,
    size_t num_points_to_load)
{
    for (int lbl = 0; lbl < _labels.size(); lbl++)
    {
        ...
        for (uint32_t point_id = 0; point_id < num_points_to_load; point_id++)
        {
            if (_label_vector.check_label_exists(point_id, x)
                || (_use_universal_label && _label_vector.check_label_exists(point_id, _universal_label)))
            {
                labeled_points.emplace_back(point_id);
            }
        }
                                                  ...
                                                  const size_t num_points_to_load, const std::vector<TagT> &tags)
{
    ...
    parse_integer_string_file<LabelT>(label_file,
                     num_points_labels, total_labels, _location_to_labels, &_labels); // determines medoid for each label and identifies
    ...
    {
        convert_pts_label_to_integer_vector(_location_to_labels, _label_vector, total_labels);
    }

    std::unordered_map<LabelT, std::vector<uint32_t>> label_to_points;
    if (!_use_integer_labels)
    {
        aggregate_points_by_bitmask_label(label_to_points, num_points_to_load);
    }
    else
    {
        aggregate_points_by_integer_label(label_to_points, num_points_to_load);
    }
                                                              ...
                                                              std::function<float(const std::uint8_t*, size_t)> rerank_fn)
{
    try
    {
        ...
        if (typeid(uint32_t *) == indices.type())
        {
            ...
            return this->search(typed_query, K, L, u32_ptr, distances, 0, std::move(rerank_fn));
        }
        else if (typeid(uint64_t *) == indices.type())
        {
            auto u64_ptr = std::any_cast<uint64_t *>(indices);
            return this->search(typed_query, K, L, u64_ptr, distances, 0, std::move(rerank_fn));
        }
        else
        {
...
std::pair<uint32_t, uint32_t> Index<T, TagT, LabelT>::_diverse_search(const DataType& query, const size_t K, const uint32_t L, const uint32_t maxLperSeller,
    std::any& indices, float* distances, std::function<float(const std::uint8_t*, size_t)> rerank_fn)
{
    try
    {
        auto typed_query = std::any_cast<const T*>(query);
        ...
        {
            auto u32_ptr = std::any_cast<uint32_t*>(indices);
            return this->search(typed_query, K, L, u32_ptr, distances, maxLperSeller, rerank_fn);
        }
        else if (typeid(uint64_t*) == indices.type())
        {
            auto u64_ptr = std::any_cast<uint64_t*>(indices);
            return this->search(typed_query, K, L, u64_ptr, distances, maxLperSeller, rerank_fn);
        }
        else
        {
            throw ANNException("Error: indices type can only be uint64_t or uint32_t.", -1);
...
std::pair<uint32_t, uint32_t> Index<T, TagT, LabelT>::_search_with_filters(const DataType &query,
                                                                           const std::vector<std::string> &raw_labels, const size_t K,
                                                                           ...
                                                                           std::function<float(const std::uint8_t*, size_t)> rerank_fn)
{
    ...
    if (typeid(uint64_t *) == indices.type())
    {
        ...
        return this->search_with_filters(std::any_cast<const T *>(query), converted_labels, K, L, maxLperSeller, ptr, distances, rerank_fn);
    }
...
template <typename T, typename TagT, typename LabelT>
template <typename IDType>
void Index<T, TagT, LabelT>::post_process_search_results(InMemQueryScratch<T>* scratch, const size_t K, IDType* indices, float* distances,
    std::function<float(const std::uint8_t*, size_t)> rerank_fn)
{
    ...
    // If reorder_index is enabled and rerank function is provided, rerank the results
    if (_reorder_index && rerank_fn != nullptr)
    {
        // Use pre-allocated buffer from scratch to avoid repeated allocations
        auto& reranked_results = scratch->reranked_results();
        reranked_results.clear();
        
        auto reorder_data_store = dynamic_cast<const InMemReorderDataStore<T>*>(_data_store.get());

        // Rerank each candidate using the provided rerank function
        for (size_t i = 0; i < best_L_nodes->size(); ++i)
        {
            if ((*best_L_nodes)[i].id < _max_points)
            {
                // Get the original full-dimensional vector for this point
                const uint8_t* data_ptr = reinterpret_cast<const uint8_t*>(reorder_data_store->get_reorder_vector((*best_L_nodes)[i].id));
                
                // Calculate the reranked distance using the full-dimensional data
                float reranked_distance = rerank_fn(data_ptr, reorder_data_store->get_reorder_aligned_dim() * sizeof(T));
                
                reranked_results.emplace_back((*best_L_nodes)[i].id, reranked_distance);
            }
        }
        
        // Sort by reranked distance
        std::sort(reranked_results.begin(), reranked_results.end());
        
        // Fill the output arrays with top K results
        size_t num_results = std::min(K, reranked_results.size());
        for (size_t i = 0; i < num_results; ++i)
        {
            indices[i] = static_cast<IDType>(reranked_results[i].id);
            if (distances != nullptr)
            {
#ifdef EXEC_ENV_OLS
                // DLVS expects negative distances
                distances[i] = reranked_results[i].distance;
#else
                distances[i] = _dist_metric == diskann::Metric::INNER_PRODUCT ? -1 * reranked_results[i].distance
                                                                                : reranked_results[i].distance;
#endif
            }
        }
        ...
        if (num_results < K)
        {
            diskann::cerr << "Found fewer than K elements for query after reranking" << std::endl;
        }
    }
    else
    {
        // Original process: copy results from best_L_nodes
        size_t pos = 0;
        for (size_t i = 0; i < best_L_nodes->size(); ++i)
        {
            if ((*best_L_nodes)[i].id < _max_points)
            {
                indices[pos] = static_cast<IDType>((*best_L_nodes)[i].id);
...
template <typename T, typename TagT, typename LabelT> void Index<T, TagT, LabelT>::compact_data()
{
    ...
    if (_use_integer_labels)
    {
            throw ANNException("Compaction not supported for integer label based filtered index", -1, __FUNCSIG__, __FILE__,
                        __LINE__);
    }
...
int Index<T, TagT, LabelT>::insert_point(const T *point, const TagT tag, const std::vector<LabelT> &labels)
{
    ...
    if (_filtered_index)
    {
        ...
        if (_use_integer_labels)
        {
            std::cerr << "Error: integer labels isn't support in streaming " << std::endl;
            return -1;
        }
    }
