...
Index<T, TagT, LabelT>::Index(
    ...
    const bool dynamic_index, const IndexWriteParameters &indexParams,
    ...
    : Index(m, dim, max_points, dynamic_index, enable_tags,
            concurrent_consolidate, pq_dist_build, num_pq_chunks, use_opq,
            indexParams.num_frozen_points) {
  ...
  initialize_query_scratch(num_scratch_spaces, initial_search_list_size,
...
Index<T, TagT, LabelT>::Index(Metric m, const size_t dim,
  ...
  if (_pq_dist) {
    ...
    if (m == diskann::Metric::INNER_PRODUCT)
      throw ANNException(
          "ERROR: Inner product metrics not yet supported "
          "with PQ distance "
          "base index",
          -1, __FUNCSIG__, __FILE__, __LINE__);
  }
  ...
  if (m == diskann::Metric::COSINE && std::is_floating_point<T>::value) {
    ...
    this->_distance.reset(
        (Distance<T> *)new AVXNormalizedCosineDistanceFloat());
  ...
  _data_store = std::make_unique<diskann::InMemDataStore<T>>(
      (location_t)total_internal_points, _dim, this->_distance);
...
size_t Index<T, TagT, LabelT>::load_graph(std::string filename,
  ...
  file_frozen_pts = *((size_t *)(header.get() + sizeof(size_t) +
  ...
  if (_max_points < expected_max_points) {
    diskann::cout << "Number of points in data: " << expected_max_points
                  ...
                  << " Setting max points to: " << expected_max_points
                  << std::endl;
  ...
  while (bytes_read != expected_file_size) {
    ...
    if (nodes_read % 10000000 == 0) diskann::cout << "." << std::flush;
    if (k > _max_range_of_loaded_graph) {
      _max_range_of_loaded_graph = k;
    }
  }
...
std::pair<uint32_t, uint32_t> Index<T, TagT, LabelT>::iterate_to_fixed_point(
    const T *query, const uint32_t Lsize, const std::vector<uint32_t> &init_ids,
...
//    pq_coord_scratch = scratch->pq_scratch()->aligned_pq_coord_scratch;
  ...
  auto compute_dists = [this, scratch, pq_dists](
  ...
  for (auto id : init_ids) {
    ...
    if (use_filter) {
      if (!detect_common_filters(id, search_invocation, filter_label)) continue;
    }
  ...
  while (best_L_nodes.has_unexpanded_node()) {
    ...
    dist_scratch.clear();
    {
      if (_dynamic_index) _locks[n].lock();
      ...
      if (_dynamic_index) _locks[n].unlock();
    }
...
void Index<T, TagT, LabelT>::search_for_point_and_prune(
  ...
  if (!use_filter) {
    ...
    iterate_to_fixed_point(scratch->aligned_query(), Lindex, init_ids, scratch,
                           false, unused_filter_label, false);
  } else {
    ...
    _data_store->get_vector(location, scratch->aligned_query());
    iterate_to_fixed_point(scratch->aligned_query(), filteredLindex,
...
void Index<T, TagT, LabelT>::occlude_list(
  ...
  if (pool.size() > maxc) pool.resize(maxc);
      ...
      for (auto iter2 = iter + 1; iter2 != pool.end(); iter2++) {
        ...
        if (_filtered_index) {
          ...
          for (auto &x : _pts_to_labels[b]) {
            ...
            if (!prune_allowed) break;
          }
        }
        if (!prune_allowed) continue;
...
void Index<T, TagT, LabelT>::prune_neighbors(
  ...
  if (_saturate_graph && alpha > 1) {
    for (const auto &node : pool) {
      if (pruned_list.size() >= range) break;
...
#pragma omp parallel for schedule(dynamic, 2048)
  for (int64_t node_ctr = 0; node_ctr < (int64_t)(visit_order.size());
    ...
    if (_final_graph[node].size() > _indexingRange) {
      ...
      for (auto id : new_out_neighbors) _final_graph[node].emplace_back(id);
    }
  }
  if (_nd > 0) {
...
void Index<T, TagT, LabelT>::build_filtered_index(
  ...
  for (typename tsl::robin_set<LabelT>::size_type lbl = 0; lbl < _labels.size();
       lbl++) {
...
std::pair<uint32_t, uint32_t> Index<T, TagT, LabelT>::search(const T *query,
  ...
  auto retval =
      iterate_to_fixed_point(scratch->aligned_query(), L, init_ids, scratch,
                             false, unused_filter_label, true);
...
std::pair<uint32_t, uint32_t> Index<T, TagT, LabelT>::search_with_filters(
  ...
  _distance->preprocess_query(query, _data_store->get_dims(),
                              scratch->aligned_query());
  auto retval = iterate_to_fixed_point(scratch->aligned_query(), L, init_ids,
                                       scratch, true, filter_vec, true);
...
template <typename T, typename TagT, typename LabelT>
size_t Index<T, TagT, LabelT>::search_with_tags(const T *query,
                                                const uint64_t K,
                                                const uint32_t L, TagT *tags,
                                                float *distances,
  ...
  _distance->preprocess_query(query, _data_store->get_dims(),
                              scratch->aligned_query());
  iterate_to_fixed_point(scratch->aligned_query(), L, init_ids, scratch, false,
  ...
  for (size_t i = 0; i < best_L_nodes.size(); ++i) {
    ...
    if (_location_to_tag.try_get(node.id, tag)) {
      ...
      if (pos == K || pos == res_vectors.size()) break;
    }
  }
...
inline void Index<T, TagT, LabelT>::process_delete(
  ...
  if (modify) {
    if (expanded_nodes_set.size() <= range) {
      ...
      for (auto &ngh : expanded_nodes_set) _final_graph[loc].push_back(ngh);
    } else {
...
consolidation_report Index<T, TagT, LabelT>::consolidate_deletes(
  ...
  std::unique_lock<std::shared_timed_mutex> update_lock(_update_lock,
                                                        std::defer_lock);
  if (!_conc_consolidate) update_lock.lock();
...
size_t Index<T, TagT, LabelT>::release_locations(
  ...
  for (auto location : locations) {
    if (_empty_slots.is_in_set(location))
      throw ANNException(
          "Trying to release location, but location "
...
void Index<T, TagT, LabelT>::reposition_points(uint32_t old_location_start,
  ...
  if (new_location_start < old_location_start) {
    ...
    for (uint32_t loc_offset = 0; loc_offset < num_locations; loc_offset++) {
      ...
      _final_graph[new_location_start + loc_offset].swap(
          _final_graph[old_location_start + loc_offset]);
    }
...
int Index<T, TagT, LabelT>::insert_point(const T *point, const TagT tag) {
  ...
  if (tag == static_cast<TagT>(0)) {
    throw diskann::ANNException(
        "Do not insert point with tag 0. That is "
  ...
  if (location == -1) {
    ...
    if (location == -1) {
      throw diskann::ANNException(
          "Cannot reserve location even after "
  ...
  }
  {
    ...
    if (_conc_consolidate) tlock.lock();
...
void Index<T, TagT,
  ...
  for (uint32_t i = 0; i < _nd; i++) {
    ...
    std::memcpy(cur_node_offset + sizeof(uint32_t), _final_graph[i].data(),
                k * sizeof(uint32_t));
