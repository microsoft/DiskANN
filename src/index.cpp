...
std::vector<uint32_t> Index<T, TagT, LabelT>::get_init_ids() {
  ...
  for (uint32_t frozen = _max_points; frozen < _max_points + _num_frozen_pts;
       frozen++) {
    if (frozen != _start) {
...
std::pair<uint32_t, uint32_t> Index<T, TagT, LabelT>::iterate_to_fixed_point(
  ...
  for (auto id : init_ids) {
    ...
    if (use_filter) {
      ...
      std::set_intersection(filter_label.begin(), filter_label.end(), x.begin(),
                            x.end(), std::back_inserter(common_filters));
      if (_use_universal_label) {
      ...
      if (common_filters.size() == 0) continue;
    }
...
void Index<T, TagT, LabelT>::prune_neighbors(
  ...
  if (pool.begin()->distance == 0) {
    diskann::cerr
        << "Warning: a candidate with distance 0 found in prune_neighbors"
        << std::endl;
  }
...
void Index<T, TagT, LabelT>::build(const T *data,
                                   ...
                                   IndexWriteParameters &parameters,
  ...
  if (modify) {
    if (expanded_nodes_set.size() <= range) {
      ...
      for (auto &ngh : expanded_nodes_set) {
        expanded_nghrs_vec.emplace_back(ngh,
                                        _data_store->get_distance(loc, ngh));
      }
...
void Index<T, TagT,
  ...
  for (uint32_t i = 0; i < _nd; i++) {
    ...
    float cur_norm =
        dist_fast->norm((T *)cur_vec, _data_store->get_aligned_dim());
