...
void Index<T, TagT, LabelT>::load(const char *filename, uint32_t num_threads, uint32_t search_l)
{
    ...
    if (file_exists(labels_file))
    {
       ...
       // _clusters_to_labels_to_points.resize(_ivf_clusters->get_num_clusters());
        ...
        for (size_t i = 0; i < _ivf_clusters->get_num_clusters(); i++)
        {
...
std::pair<uint32_t, uint32_t> Index<T, TagT, LabelT>::iterate_to_fixed_point(
    ...
    const std::vector<LabelT> &filter_labels, bool search_invocation, bool penalty_approach, bool beta_approach)
{
    ...
    // if(beta_approach && !penalty_approach) {
    //     std::cout<<"[iterate_to_fixed_point] beta_approach is true, penalty_approach is false"<<std::endl;
    // }
    ...
    if (!inserted_into_pool_bs.isEmpty() || inserted_into_pool_rs.size() > 0)
    {
        ...
        // std::cout<< "Inserted_into_pool_rs :";

        for (auto const &id : inserted_into_pool_rs)
        {
            // std::cout<<id<<", ";
        ...
        // std::cout<< "Inserted_into_pool_bs :" << "Size: " << inserted_into_pool_bs.cardinality() << std::endl;
        for (auto const &id : inserted_into_pool_bs)
        {
            // std::cout<<id<<", ";
        }
        // std::cout<<std::endl;
    ...
    {
        // std::cout<<"[iterate_to_fixed_point]starting search with id: "<<id<<std::endl;
        ...
        if (is_not_visited(id))
        {
            ...
            if (distances[0] < 0)
            {
                distance = distances[0] / beta + penalty; // Reduce the magnitude of the negative distance
            }
            else
            {
                distance = distances[0] * beta + penalty; // Increase the magnitude of the positive distance
            }
            ...
            if (print_qstats)
                {
                    ...
                    out << "prev distance:" << distances[0] << " distance " << distance << ", penalty " << penalty << ", beta " << beta << std::endl;
                    // out << std::endl;
                    out.close();
                }

            Neighbor nn = Neighbor(id, distance);
            // std::cout<<"[iterate_to_fixed_point]Added to best_l_nodes: "<<id<<std::endl;
            best_L_nodes.insert(nn);
        }
    }

    // std::cout<<"[iterate_to_fixed_point]best_L_nodes: "<<std::endl;
    for (size_t i = 0; i < best_L_nodes.size(); ++i) {
        auto nbr = best_L_nodes[i];
...
/*        for (uint32_t rnr = 0; rnr < best_L_nodes.size(); rnr++) {
        ...
        if (_dynamic_index)
        {
            ...
            for (auto id : _graph_store->get_neighbours(n))
            {
                ...
                if (!is_not_visited(id)) {
                    // std::cout<<"[iterate_to_fixed_point] Already visited id: "<<id<<std::endl;
                    continue;
                }
        ...
        assert(dist_betas.size() == id_scratch.size());

        // Insert <id, dist> pairs into the pool of candidates
        // std::cout<<"[iterate_to_fixed_point]id: ";
        for (size_t m = 0; m < id_scratch.size(); ++m)
        {
            float distance;
            if (dist_scratch[m] < 0)
            {
                distance = (dist_scratch[m] / dist_betas[m]) + dist_pens[m]; // Reduce the magnitude of the negative distance
            }
            else
            {
                distance = (dist_scratch[m] * dist_betas[m]) + dist_pens[m]; // Increase the magnitude of the positive distance
            }
            best_L_nodes.insert(Neighbor(id_scratch[m], distance));
        }
        ...
        if (print_qstats)
        {
            ...
            for (size_t i = 0; i < best_L_nodes.size(); i++)
            {
                auto &x = best_L_nodes[i];
                out << std::setw(10) << x.id << ":" << x.distance << "\t";
            }
            out << std::endl;
            out.close();
        }
                                                                           ...
                                                                           std::any &indices, float *distances)
{
    ...
    {
        auto converted_label = this->get_converted_label(x);
...
//                init_ids.emplace_back(cand);
            ...
            // retval = iterate_to_fixed_point(scratch, L, init_ids, true, filter_vec, true, false, true);
        }
        break;
        case 2:
            num_graphs++;
...
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<float, uint64_t, uint16_t>::search_with_filters<
    uint64_t>(const float *query, const std::vector<uint16_t> &filter_label, const size_t K, const uint32_t L,
              uint64_t *indices, float *distances);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<float, uint64_t, uint16_t>::search_with_filters<
    uint32_t>(const float *query, const std::vector<uint16_t> &filter_label, const size_t K, const uint32_t L,
              uint32_t *indices, float *distances);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<uint8_t, uint64_t, uint16_t>::search_with_filters<
    uint64_t>(const uint8_t *query, const std::vector<uint16_t> &filter_label, const size_t K, const uint32_t L,
              uint64_t *indices, float *distances);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<uint8_t, uint64_t, uint16_t>::search_with_filters<
    uint32_t>(const uint8_t *query, const std::vector<uint16_t> &filter_label, const size_t K, const uint32_t L,
              uint32_t *indices, float *distances);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<int8_t, uint64_t, uint16_t>::search_with_filters<
    uint64_t>(const int8_t *query, const std::vector<uint16_t> &filter_label, const size_t K, const uint32_t L,
              uint64_t *indices, float *distances);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t> Index<int8_t, uint64_t, uint16_t>::search_with_filters<
    uint32_t>(const int8_t *query, const std::vector<uint16_t> &filter_label, const size_t K, const uint32_t L,
              uint32_t *indices, float *distances);
// TagT==uint32_t
