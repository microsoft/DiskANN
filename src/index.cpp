...
Index<T, TagT, LabelT>::Index(
    ...
    : _dist_metric(index_config.metric), _dim(index_config.dimension),
      _max_points(index_config.max_points),
      _num_frozen_pts(index_config.num_frozen_pts),
      _dynamic_index(index_config.dynamic_index),
      _enable_tags(index_config.enable_tags), _indexingMaxC(DEFAULT_MAXC),
      _query_scratch(nullptr), _pq_dist(index_config.pq_dist_build),
      _use_opq(index_config.use_opq),
      _filtered_index(index_config.filtered_index),
      _num_pq_chunks(index_config.num_pq_chunks),
      _delete_set(new tsl::robin_set<uint32_t>),
...
Index<T, TagT, LabelT>::Index(
    ...
    const size_t num_frozen_pts, const bool dynamic_index,
    const bool enable_tags, const bool concurrent_consolidate,
    ...
    : Index(IndexConfigBuilder()
            ...
            IndexFactory::construct_datastore<T>(
                ...
                (max_points == 0 ? (size_t)1 : max_points) +
                    (dynamic_index && num_frozen_pts == 0 ? (size_t)1
                                                          : num_frozen_pts),
                dim, m),
...
void Index<T, TagT, LabelT>::initialize_query_scratch(uint32_t num_threads,
                                                      uint32_t search_l,
                                                      uint32_t indexing_l,
                                                      uint32_t r, uint32_t maxc,
                                                      size_t dim) {
  for (uint32_t i = 0; i < num_threads; i++) {
    auto scratch = new InMemQueryScratch<T>(
        search_l, indexing_l, r, maxc, dim, _data_store->get_aligned_dim(),
...
size_t Index<T, TagT, LabelT>::save_delete_list(const std::string &filename) {
  ...
  return save_bin<uint32_t>(filename, delete_list.get(), _delete_set->size(),
                            1);
}
      ...
      if (_location_to_labels.size() > 0) {
        ...
        for (uint32_t i = 0; i < _nd + _num_frozen_pts; i++) {
          ...
          if (_location_to_labels[i].size() != 0)
            label_writer
                << _location_to_labels[i][_location_to_labels[i].size() - 1];
        ...
        if (compact_before_save && _dynamic_index) {
          _label_map =
              load_label_map(std::string(filename) + "_labels_map.txt");
          ...
          std::ofstream raw_label_writer(std::string(filename) +
                                         "_raw_labels.txt");
          assert(raw_label_writer.is_open());
          for (uint32_t i = 0; i < _nd + _num_frozen_pts; i++) {
            for (uint32_t j = 0; j + 1 < _location_to_labels[i].size(); j++) {
              raw_label_writer
                  << mapped_to_raw_labels[_location_to_labels[i][j]] << ",";
            }
            if (_location_to_labels[i].size() != 0)
              raw_label_writer << mapped_to_raw_labels
                      [_location_to_labels[i]
                                          [_location_to_labels[i].size() - 1]];
...
size_t Index<T, TagT, LabelT>::load_tags(const std::string tag_filename) {
  if (_enable_tags && !file_exists(tag_filename)) {
    diskann::cerr << "Tag file " << tag_filename << " does not exist!"
                  << std::endl;
    throw diskann::ANNException("Tag file " + tag_filename + " does not exist!",
                                -1, __FUNCSIG__, __FILE__, __LINE__);
  }
#endif
  if (!_enable_tags) {
...
void Index<T, TagT, LabelT>::load(const char *filename, uint32_t num_threads,
  ...
  if (_query_scratch.size() == 0) {
    initialize_query_scratch(num_threads, search_l, search_l,
                             (uint32_t)_graph_store->get_max_range_of_graph(),
                             _indexingMaxC, _dim);
  }
}
...
#ifndef EXEC_ENV_OLS
template <typename T, typename TagT, typename LabelT>
size_t Index<T, TagT, LabelT>::get_graph_num_frozen_points(
...
int Index<T, TagT, LabelT>::_get_vector_by_tag(TagType &tag, DataType &vec) {
  try {
    ...
    throw ANNException(
        "Error: bad any cast while performing _get_vector_by_tags() " +
            std::string(e.what()),
        -1);
...
int Index<T, TagT, LabelT>::get_vector_by_tag(TagT &tag, T *vec) {
  ...
  if (_tag_to_location.find(tag) == _tag_to_location.end()) {
    diskann::cout << "Tag " << get_tag_string(tag) << " does not exist"
                  << std::endl;
    return -1;
  }
...
std::vector<uint32_t> Index<T, TagT, LabelT>::get_init_ids() {
  ...
  for (uint32_t frozen = (uint32_t)_max_points;
       frozen < _max_points + _num_frozen_pts; frozen++) {
    if (frozen != _start) {
...
template <typename T, typename TagT, typename LabelT>
bool Index<T, TagT, LabelT>::detect_common_filters(
    uint32_t point_id, bool search_invocation,
    const std::vector<LabelT> &incoming_labels) {
  auto &curr_node_labels = _location_to_labels[point_id];
  std::vector<LabelT> common_filters;
  std::set_intersection(incoming_labels.begin(), incoming_labels.end(),
                        curr_node_labels.begin(), curr_node_labels.end(),
  ...
  if (_use_universal_label) {
    if (!search_invocation) {
      if (std::find(incoming_labels.begin(), incoming_labels.end(),
                    _universal_label) != incoming_labels.end() ||
          std::find(curr_node_labels.begin(), curr_node_labels.end(),
                    _universal_label) != curr_node_labels.end())
        common_filters.push_back(_universal_label);
    } else {
      if (std::find(curr_node_labels.begin(), curr_node_labels.end(),
                    _universal_label) != curr_node_labels.end())
        common_filters.push_back(_universal_label);
    }
  }
...
std::pair<uint32_t, uint32_t> Index<T, TagT, LabelT>::iterate_to_fixed_point(
    InMemQueryScratch<T> *scratch, const uint32_t Lsize,
  ...
  auto is_not_visited = [this, fast_iterate, &inserted_into_pool_bs,
    ...
    return fast_iterate
               ? inserted_into_pool_bs[id] == 0
               : inserted_into_pool_rs.find(id) == inserted_into_pool_rs.end();
  };

  // Lambda to batch compute query<-> node distances in PQ space
  auto compute_dists = [this, scratch,
                        pq_dists](const std::vector<uint32_t> &ids,
                                  std::vector<float> &dists_out) {
    _pq_data_store->get_distance(scratch->aligned_query(), ids, dists_out,
                                 scratch);
  };
  ...
  for (auto id : init_ids) {
    if (id >= _max_points + _num_frozen_pts) {
      diskann::cerr << "Out of range loc found as an edge : " << id
                    << std::endl;
      throw diskann::ANNException(std::string("Wrong loc") + std::to_string(id),
                                  -1, __FUNCSIG__, __FILE__, __LINE__);
    }
...
void Index<T, TagT, LabelT>::search_for_point_and_prune(
    int location, uint32_t Lindex, std::vector<uint32_t> &pruned_list,
    InMemQueryScratch<T> *scratch, bool use_filter, uint32_t filteredLindex) {
  ...
  if (!use_filter) {
    ...
    _data_store->get_vector(location, scratch->aligned_query());
    iterate_to_fixed_point(scratch, filteredLindex, filter_specific_start_nodes,
                           true, _location_to_labels[location], false);
    ...
    for (auto unfiltered_neighbour : scratch->pool()) {
      ...
      if (best_candidate_pool.find(unfiltered_neighbour) ==
          best_candidate_pool.end()) {
...
void Index<T, TagT, LabelT>::occlude_list(
    const uint32_t location, std::vector<Neighbor> &pool, const float alpha,
  ...
  while (cur_alpha <= alpha && result.size() < degree) {
    ...
    for (auto iter = pool.begin(); result.size() < degree && iter != pool.end();
         ++iter) {
      ...
      for (auto iter2 = iter + 1; iter2 != pool.end(); iter2++) {
        ...
        if (_filtered_index) {
          ...
          for (auto &x : _location_to_labels[b]) {
            if (std::find(_location_to_labels[a].begin(),
                          _location_to_labels[a].end(),
                          x) == _location_to_labels[a].end()) {
              prune_allowed = false;
            }
            if (!prune_allowed)
              break;
...
template <typename T, typename TagT, typename LabelT>
void Index<T, TagT, LabelT>::prune_neighbors(const uint32_t location,
                                             std::vector<Neighbor> &pool,
                                             std::vector<uint32_t> &pruned_list,
                                             InMemQueryScratch<T> *scratch) {
  prune_neighbors(location, pool, _indexingRange, _indexingMaxC, _indexingAlpha,
                  pruned_list, scratch);
}

template <typename T, typename TagT, typename LabelT>
void Index<T, TagT, LabelT>::prune_neighbors(
    const uint32_t location, std::vector<Neighbor> &pool, const uint32_t range,
...
template <typename T, typename TagT, typename LabelT>
void Index<T, TagT, LabelT>::inter_insert(uint32_t n,
                                          std::vector<uint32_t> &pruned_list,
  ...
  for (auto des : src_pool) {
    ...
    bool prune_needed = false;
    {
      ...
      if (std::find(des_pool.begin(), des_pool.end(), n) == des_pool.end()) {
        if (des_pool.size() <
            (uint64_t)(defaults::GRAPH_SLACK_FACTOR * range)) {
    ...
    if (prune_needed) {
      ...
      size_t reserveSize =
          (size_t)(std::ceil(1.05 * defaults::GRAPH_SLACK_FACTOR * range));
...
#pragma omp parallel for schedule(dynamic, 2048)
  for (int64_t node_ctr = 0; node_ctr < (int64_t)(visit_order.size());
       node_ctr++) {
...

    {
      ...
      assert(_graph_store->get_neighbours((location_t)node).size() <=
             _indexingRange);
    }
...
#pragma omp parallel for schedule(dynamic, 2048)
  for (int64_t node_ctr = 0; node_ctr < (int64_t)(visit_order.size());
       node_ctr++) {
    auto node = visit_order[node_ctr];
    if (_graph_store->get_neighbours((location_t)node).size() >
        _indexingRange) {
      ScratchStoreManager<InMemQueryScratch<T>> manager(_query_scratch);
      auto scratch = manager.scratch_space();
...
template <typename T, typename TagT, typename LabelT>
void Index<T, TagT, LabelT>::prune_all_neighbors(
    const uint32_t max_degree, const uint32_t max_occlusion_size,
    const float alpha) {
...
#pragma omp parallel for
  for (int64_t node = 0; node < (int64_t)(_max_points + _num_frozen_pts);
       node++) {
    if ((size_t)node < _nd || (size_t)node >= _max_points) {
      if (_graph_store->get_neighbours((location_t)node).size() > range) {
        ...
        for (auto cur_nbr : _graph_store->get_neighbours((location_t)node)) {
          if (dummy_visited.find(cur_nbr) == dummy_visited.end() &&
              cur_nbr != node) {
            float dist = _data_store->get_distance((location_t)node,
                                                   (location_t)cur_nbr);
            dummy_pool.emplace_back(Neighbor(cur_nbr, dist));
            dummy_visited.insert(cur_nbr);
          }
        ...
        prune_neighbors((uint32_t)node, dummy_pool, range, maxc, alpha,
  ...
  for (size_t i = 0; i < _max_points + _num_frozen_pts; i++) {
    if (i < _nd || i >= _max_points) {
      const std::vector<uint32_t> &pool =
          _graph_store->get_neighbours((location_t)i);
...
// REFACTOR
template <typename T, typename TagT, typename LabelT>
void Index<T, TagT, LabelT>::set_start_points(const T *data,
                                              size_t data_count) {
  std::unique_lock<std::shared_timed_mutex> ul(_update_lock);
  std::unique_lock<std::shared_timed_mutex> tl(_tag_lock);
  if (_nd > 0)
  ...
  if (data_count != _num_frozen_pts * _dim)
    throw ANNException("Invalid number of points", -1, __FUNCSIG__, __FILE__,
                       __LINE__);
...
template <typename T, typename TagT, typename LabelT>
void Index<T, TagT, LabelT>::_set_start_points_at_random(DataType radius,
  ...
  try {
    ...
    throw ANNException(
        "Error: bad any cast while performing _set_start_points_at_random() " +
            std::string(e.what()),
        -1);
  } catch (const std::exception &e) {
    throw ANNException("Error: " + std::string(e.what()), -1);
...
template <typename T, typename TagT, typename LabelT>
void Index<T, TagT, LabelT>::set_start_points_at_random(T radius,
                                                        uint32_t random_seed) {
  ...
  for (size_t frozen_point = 0; frozen_point < _num_frozen_pts;
       frozen_point++) {
    double norm_sq = 0.0;
...
void Index<T, TagT, LabelT>::build_with_data_populated(
  ...
  if (_query_scratch.size() == 0) {
    initialize_query_scratch(5 + num_threads_index, index_L, index_L, index_R,
                             maxc, _data_store->get_aligned_dim());
  }
...
template <typename T, typename TagT, typename LabelT>
void Index<T, TagT, LabelT>::_build(const DataType &data,
  ...
  try {
    ...
    throw ANNException("Error: bad any cast in while building index. " +
                           std::string(e.what()),
                       -1);
  } catch (const std::exception &e) {
    throw ANNException("Error" + std::string(e.what()), -1);
...
void Index<T, TagT, LabelT>::build(const char *filename,
  ...
  diskann::cout << "Using only first " << num_points_to_load << " from file.. "
                << std::endl;
...
void Index<T, TagT, LabelT>::build(const char *filename,
  ...
  if (_enable_tags) {
    ...
    if (tag_filename == nullptr) {
      throw ANNException("Tag filename is null, while _enable_tags is set", -1,
                         __FUNCSIG__, __FILE__, __LINE__);
    } else {
...
template <typename T, typename TagT, typename LabelT>
void Index<T, TagT, LabelT>::build(const std::string &data_file,
                                   const size_t num_points_to_load,
                                   IndexFilterParams &filter_params) {
  size_t points_to_load =
      num_points_to_load == 0 ? _max_points : num_points_to_load;
  ...
  if (filter_params.label_file == "") {
    ...
    std::string labels_file_to_use =
        filter_params.save_path_prefix + "_label_formatted.txt";
    std::string mem_labels_int_map_file =
        filter_params.save_path_prefix + "_labels_map.txt";
    convert_labels_string_to_int(filter_params.label_file, labels_file_to_use,
                                 mem_labels_int_map_file,
                                 filter_params.universal_label);
    if (filter_params.universal_label != "") {
      LabelT unv_label_as_num = 0;
      this->set_universal_label(unv_label_as_num);
    }
    this->build_filtered_index(data_file.c_str(), labels_file_to_use,
                               points_to_load);
  }
...
template <typename T, typename TagT, typename LabelT>
LabelT
Index<T, TagT, LabelT>::get_converted_label(const std::string &raw_label) {
...
template <typename T, typename TagT, typename LabelT>
void Index<T, TagT, LabelT>::_set_universal_label(
    const LabelType universal_label) {
  ...
  for (uint32_t point_id = 0; point_id < num_points_to_load; point_id++) {
    for (auto label : _location_to_labels[point_id]) {
      if (label != _universal_label) {
        ...
        for (typename tsl::robin_set<LabelT>::size_type lbl = 0;
             lbl < _labels.size(); lbl++) {
...
Index<T, TagT, LabelT>::_search(const DataType &query, const size_t K,
  ...
  try {
    ...
    if (typeid(uint32_t *) == indices.type()) {
      ...
      throw ANNException(
          "Error: indices type can only be uint64_t or uint32_t.", -1);
    }
  } catch (const std::bad_any_cast &e) {
    throw ANNException(
        "Error: bad any cast while searching. " + std::string(e.what()), -1);
  } catch (const std::exception &e) {
    throw ANNException("Error: " + std::string(e.what()), -1);
  }
}
...
std::pair<uint32_t, uint32_t>
Index<T, TagT, LabelT>::search(const T *query, const size_t K, const uint32_t L,
  ...
  if (L > scratch->get_L()) {
    ...
    diskann::cout << "Resize completed. New scratch->L is " << scratch->get_L()
                  << std::endl;
  }
  ...
  auto retval = iterate_to_fixed_point(scratch, L, init_ids, false,
                                       unused_filter_label, true);
  ...
  if (pos < K) {
    diskann::cerr << "Found pos: " << pos << "fewer than K elements " << K
                  << " for query" << std::endl;
  }
...
std::pair<uint32_t, uint32_t> Index<T, TagT, LabelT>::_search_with_filters(
    const DataType &query, const std::string &raw_label, const size_t K,
    const uint32_t L, std::any &indices, float *distances) {
  ...
  if (typeid(uint64_t *) == indices.type()) {
    ...
    return this->search_with_filters(std::any_cast<T *>(query), converted_label,
                                     K, L, ptr, distances);
...
template <typename T, typename TagT, typename LabelT>
size_t Index<T, TagT, LabelT>::_search_with_tags(
    const DataType &query, const uint64_t K, const uint32_t L,
    const TagType &tags, float *distances, DataVector &res_vectors,
    bool use_filters, const std::string filter_label) {
  try {
    return this->search_with_tags(std::any_cast<const T *>(query), K, L,
                                  std::any_cast<TagT *>(tags), distances,
                                  res_vectors.get<std::vector<T *>>(),
                                  use_filters, filter_label);
  } catch (const std::bad_any_cast &e) {
    throw ANNException(
        "Error: bad any cast while performing _search_with_tags() " +
            std::string(e.what()),
        -1);
  } catch (const std::exception &e) {
    throw ANNException("Error: " + std::string(e.what()), -1);
...
size_t Index<T, TagT, LabelT>::search_with_tags(
    const T *query, const uint64_t K, const uint32_t L, TagT *tags,
  ...
  if (!use_filters) {
    const std::vector<LabelT> unused_filter_label;
    iterate_to_fixed_point(scratch, L, init_ids, false, unused_filter_label,
                           true);
  } else {
    std::vector<LabelT> filter_vec;
...
#else
        distances[pos] =
            _dist_metric == INNER_PRODUCT ? -1 * node.distance : node.distance;
#endif
      }
      pos++;
...
void Index<T, TagT, LabelT>::generate_frozen_point() {
  ...
  if (_num_frozen_pts > 1) {
    throw ANNException(
        "More than one frozen point not supported in generate_frozen_point", -1,
        __FUNCSIG__, __FILE__, __LINE__);
  }
...
inline void Index<T, TagT, LabelT>::process_delete(
    const tsl::robin_set<uint32_t> &old_delete_set, size_t loc,
    ...
    if (expanded_nodes_set.size() <= range) {
      ...
      for (auto &ngh : expanded_nodes_set) {
        expanded_nghrs_vec.emplace_back(
            ngh, _data_store->get_distance((location_t)loc, (location_t)ngh));
      }
      std::sort(expanded_nghrs_vec.begin(), expanded_nghrs_vec.end());
      std::vector<uint32_t> &occlude_list_output =
          scratch->occlude_list_output();
      occlude_list((uint32_t)loc, expanded_nghrs_vec, alpha, range, maxc,
...
consolidation_report Index<T, TagT, LabelT>::consolidate_deletes(
    const IndexWriteParameters &params) {
  if (!_enable_tags)
...

  {
    ...
    if (_location_to_tag.size() + _delete_set->size() != _nd) {
      ...
      return consolidation_report(
          diskann::consolidation_report::status_code::INCONSISTENT_COUNT_ERROR,
          0, 0, 0, 0, 0, 0, 0);
    }
  ...
  if (!cl.try_lock()) {
    diskann::cerr
        ...
        << std::endl;
    return consolidation_report(
        diskann::consolidation_report::status_code::LOCK_FAIL, 0, 0, 0, 0, 0, 0,
        0);
  }

  diskann::cout << "Starting consolidate_deletes... ";

  std::unique_ptr<tsl::robin_set<uint32_t>> old_delete_set(
      new tsl::robin_set<uint32_t>);
  {
    std::unique_lock<std::shared_timed_mutex> dl(_delete_lock);
  ...
  const uint32_t num_threads =
      params.num_threads == 0 ? omp_get_num_procs() : params.num_threads;
...
#pragma omp parallel for num_threads(num_threads) schedule(dynamic, 8192) reduction(+ : num_calls_to_process_delete)
  ...
  for (int64_t loc = _max_points;
       loc < (int64_t)(_max_points + _num_frozen_pts); loc++) {
    ScratchStoreManager<InMemQueryScratch<T>> manager(_query_scratch);
    auto scratch = manager.scratch_space();
    process_delete(*old_delete_set, loc, range, maxc, alpha, scratch);
    num_calls_to_process_delete += 1;
...
void Index<T, TagT, LabelT>::compact_frozen_point() {
  if (_nd < _max_points && _num_frozen_pts > 0) {
    reposition_points((uint32_t)_max_points, (uint32_t)_nd,
                      (uint32_t)_num_frozen_pts);
    _start = (uint32_t)_nd;
    ...
    if (_filtered_index && _dynamic_index) {
      ...
      for (auto &[label, medoid_id] : _label_to_start_id) {
        /*  if (label == _universal_label)
              continue;*/
        _label_to_start_id[label] =
            (uint32_t)_nd + (medoid_id - (uint32_t)_max_points);
      }
    }
  }
}
...
void Index<T, TagT, LabelT>::compact_data() {
  ...
  std::vector<uint32_t> new_location =
      std::vector<uint32_t>(_max_points + _num_frozen_pts, UINT32_MAX);
  ...
  for (uint32_t old = 0; old < _max_points + _num_frozen_pts; ++old) {
    ...
    if ((new_location[old] < _max_points) // If point continues to exist
      ...
      new_adj_list.reserve(
          _graph_store->get_neighbours((location_t)old).size());
  ...
  diskann::cout << "Time taken for compact_data: " << timer.elapsed() / 1000000.
                << "s." << std::endl;
}
...
size_t Index<T, TagT, LabelT>::release_locations(
    const tsl::robin_set<uint32_t> &locations) {
...
template <typename T, typename TagT, typename LabelT>
void Index<T, TagT, LabelT>::reposition_points(uint32_t old_location_start,
  ...
  for (uint32_t i = 0; i < _max_points + _num_frozen_pts; i++) {
    ...
    std::vector<location_t> i_neighbours_copy(i_neighbours.begin(),
                                              i_neighbours.end());
  ...
  if (new_location_start < old_location_start) {
    ...
    for (uint32_t loc_offset = 0; loc_offset < num_locations; loc_offset++) {
      assert(_graph_store->get_neighbours(new_location_start + loc_offset)
                 .empty());
      _graph_store->swap_neighbours(new_location_start + loc_offset,
    ...
    for (uint32_t loc_offset = num_locations; loc_offset > 0; loc_offset--) {
      assert(_graph_store->get_neighbours(new_location_start + loc_offset - 1u)
                 .empty());
...
void Index<T, TagT, LabelT>::reposition_frozen_point_to_end() {
  ...
  reposition_points((uint32_t)_nd, (uint32_t)_max_points,
                    (uint32_t)_num_frozen_pts);
  _start = (uint32_t)_max_points;
  ...
  if (_filtered_index && _dynamic_index) {
    for (auto &[label, medoid_id] : _label_to_start_id) {
      /*if (label == _universal_label)
          continue;*/
      _label_to_start_id[label] =
          (uint32_t)_max_points + (medoid_id - (uint32_t)_nd);
    }
  }
}
...
template <typename T, typename TagT, typename LabelT>
int Index<T, TagT, LabelT>::_insert_point(const DataType &point,
                                          const TagType tag) {
  try {
    return this->insert_point(std::any_cast<const T *>(point),
                              std::any_cast<const TagT>(tag));
  } catch (const std::bad_any_cast &anycast_e) {
    throw new ANNException("Error:Trying to insert invalid data type" +
                               std::string(anycast_e.what()),
                           -1);
  } catch (const std::exception &e) {
  ...
  if (_filtered_index) {
    ...
    for (LabelT label : labels) {
      if (_labels.find(label) == _labels.end()) {
        ...
        auto fz_location =
            (int)(_max_points) + _frozen_pts_used; // as first _fz_point
...
template <typename T, typename TagT, typename LabelT>
void Index<T, TagT, LabelT>::_lazy_delete(TagVector &tags,
                                          TagVector &failed_tags) {
  try {
    this->lazy_delete(tags.get<const std::vector<TagT>>(),
                      failed_tags.get<std::vector<TagT>>());
  } catch (const std::bad_any_cast &e) {
    throw ANNException("Error: bad any cast while performing _lazy_delete() " +
                           std::string(e.what()),
                       -1);
  } catch (const std::exception &e) {
    throw ANNException("Error: " + std::string(e.what()), -1);
...
int Index<T, TagT, LabelT>::lazy_delete(const TagT &tag) {
  ...
  if (_tag_to_location.find(tag) == _tag_to_location.end()) {
    diskann::cerr << "Delete tag not found " << get_tag_string(tag)
                  << std::endl;
    return -1;
  }
...
void Index<T, TagT, LabelT>::_get_active_tags(TagRobinSet &active_tags) {
  try {
    ...
    throw ANNException(
        "Error: bad_any cast while performing _get_active_tags() " +
            std::string(e.what()),
        -1);
  } catch (const std::exception &e) {
    throw ANNException("Error :" + std::string(e.what()), -1);
...
void Index<T, TagT, LabelT>::print_status() {
  ...
  diskann::cout << "Graph size: " << _graph_store->get_total_points()
                << std::endl;
...
void Index<T, TagT, LabelT>::count_nodes_at_bfs_levels() {
  ...
  for (uint32_t i = (uint32_t)_max_points; i < _max_points + _num_frozen_pts;
       ++i) {
    if (i != _start) {
      bfs_sets[0].insert(i);
      visited.set(i);
...
template <typename T, typename TagT, typename LabelT>
void Index<T, TagT,
           LabelT>::optimize_index_layout() { // use after build or load
  if (_dynamic_index) {
    throw diskann::ANNException(
  ...
  _neighbor_len =
      (_graph_store->get_max_observed_degree() + 1) * sizeof(uint32_t);
  ...
  for (uint32_t i = 0; i < _nd; i++) {
    ...
    float cur_norm =
        dist_fast->norm((T *)cur_vec, (uint32_t)_data_store->get_aligned_dim());
    std::memcpy(cur_node_offset, &cur_norm, sizeof(float));
    std::memcpy(cur_node_offset + sizeof(float), cur_vec,
...
template <typename T, typename TagT, typename LabelT>
void Index<T, TagT, LabelT>::_search_with_optimized_layout(
    const DataType &query, size_t K, size_t L, uint32_t *indices) {
  try {
    return this->search_with_optimized_layout(std::any_cast<const T *>(query),
                                              K, L, indices);
  } catch (const std::bad_any_cast &e) {
...
void Index<T, TagT, LabelT>::search_with_optimized_layout(const T *query,
                                                          size_t K, size_t L,
                                                          uint32_t *indices) {
  DistanceFastL2<T> *dist_fast =
      (DistanceFastL2<T> *)(_data_store->get_dist_fn());
  ...
  uint32_t tmp_l = 0;
  uint32_t *neighbors =
      (uint32_t *)(_opt_graph + _node_size * _start + _data_len);
  ...
  for (uint32_t i = 0; i < init_ids.size(); i++) {
    ...
    float norm_x = *x;
    x++;
    float dist = dist_fast->compare(x, query, norm_x,
                                    (uint32_t)_data_store->get_aligned_dim());
  ...
  while (retset.has_unexpanded_node()) {
    ...
    for (uint32_t m = 0; m < MaxM; ++m) {
      ...
      float norm = *data;
      data++;
      float dist = dist_fast->compare(query, data, norm,
                                      (uint32_t)_data_store->get_aligned_dim());
      Neighbor nn(id, dist);
      retset.insert(nn);
    }
...
Index<float, uint64_t, uint32_t>::search_with_filters<uint64_t>(
    ...
    const uint32_t L, uint64_t *indices, float *distances);
template DISKANN_DLLEXPORT std::pair<uint32_t, uint32_t>
