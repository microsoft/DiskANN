...
Index<T, TagT, LabelT>::Index(Metric m, const size_t dim,
    ...
    : _dist_metric(m),
      ...
      _delete_set(new tsl::robin_set<unsigned>),
...
void Index<T, TagT, LabelT>::initialize_query_scratch(uint32_t num_threads,
  ...
  for (uint32_t i = 0; i < num_threads; i++) {
    auto scratch =
        new InMemQueryScratch<T>(search_l, indexing_l, r, maxc, dim, _pq_dist);
...
size_t Index<T, TagT, LabelT>::load_graph(std::string filename,
  ...
  file_frozen_pts = *((_u64 *)(header.get() + sizeof(_u64) + sizeof(unsigned) +
                               sizeof(unsigned)));
#else
...
unsigned Index<T, TagT, LabelT>::calculate_entry_point() {
  ...
  for (size_t j = 0; j < _aligned_dim; j++) center[j] = 0;

  for (size_t i = 0; i < _nd; i++)
    for (size_t j = 0; j < _aligned_dim; j++)
      center[j] += (float)_data[i * _aligned_dim + j];

  for (size_t j = 0; j < _aligned_dim; j++) center[j] /= (float)_nd;
...
std::pair<uint32_t, uint32_t> Index<T, TagT, LabelT>::iterate_to_fixed_point(
    const T *query, const unsigned Lsize, const std::vector<unsigned> &init_ids,
    InMemQueryScratch<T> *scratch, bool use_filter,
    const std::vector<LabelT> &filter_label, bool ret_frozen,
    ...
    if (_pq_dist) {
      ...
      for (size_t m = 0; m < id_scratch.size(); ++m) {
        ...
        dist_scratch.push_back(
            _distance->compare(aligned_query, _data + _aligned_dim * (size_t)id,
...
void Index<T, TagT, LabelT>::link(Parameters &parameters) {
  ...
  if (_num_frozen_pts > 0) visit_order.emplace_back((unsigned)_max_points);
...
void Index<T, TagT, LabelT>::resize(size_t new_max_points) {
  ...
  alloc_aligned((void **)&new_data,
                (new_max_points + 1) * _aligned_dim * sizeof(T), 8 * sizeof(T));
