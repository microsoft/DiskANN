...
namespace diskann {
    ...
    Index<T, TagT>::Index(Metric m, const char *filename,
        ...
        : _num_frozen_pts(num_frozen_pts), _has_built(false), _width(0),
          ...
          _support_eager_delete(support_eager_delete), _store_data(store_data) {
        // data is stored to _nd * aligned_dim matrix with necessary
        // zero-padding
        std::cout << "Number of frozen points = " << _num_frozen_pts
                  << std::endl;
    ...
    void Index<T, TagT>::save(const char *filename) {
        ...
        if (_support_eager_delete)
            if (_eager_done && (!_compacted_order)) {
                if (_nd < _max_points) {
                    assert(_final_graph.size() ==
                           _max_points + _num_frozen_pts);
                    unsigned              active = 0;
                    std::vector<unsigned> new_location =
                        get_new_location(active);
                    std::cout
                        << "Size of new_location = " << new_location.size()
                        << std::endl;
    ...
    void Index<T, TagT>::load(const char *filename, const bool load_tags,
        ...
        if (_final_graph.size() != _nd) {
            std::cout << "ERROR. mismatch in number of points. Graph has "
                      << _final_graph.size()
                      << " points and loaded dataset has " << _nd << " points. "
                      << std::endl;
            return;
        }
        ...
        // SEPARATE FUNCTION FOR LOADING TAGS? NEED TO CHECK IF SIZE MATCHES
        // ETC? USE LOAD_BIN?
        if (load_tags) {
            ...
            if (tag_filename == NULL)
                tag_file =
                    std::ifstream(std::string(filename) + std::string(".tags"));
            else
...
#pragma omp parallel for schedule(static, 65536)
        ...
        while (k < l) {
            ...
            if (best_L_nodes[k].flag) {
                ...
                for (unsigned m = 0; m < _final_graph[n].size(); ++m) {
                    ...
                    if (inserted_into_pool.find(id) ==
                        inserted_into_pool.end()) {
                        inserted_into_pool.insert(id);
                        ...
                        if (dist >= best_L_nodes[l - 1].distance &&
                            (l == Lsize))
                            continue;
    ...
    void Index<T, TagT>::get_expanded_nodes(
        ...
        /*      for (uint32_t i = 0; i < (Lindex - 1); i++) {
            ...
            */
        iterate_to_fixed_point(node_coords, Lindex, INDEXING_BEAM_WIDTH,
    ...
    void Index<T, TagT>::occlude_list(std::vector<Neighbor> &pool,
        ...
        occlude_list(pool, location, alpha, degree, maxc, result,
                     occlude_factor);
    }
    ...
    void Index<T, TagT>::batch_inter_insert(
        ...
        for (auto des : pruned_list) {
            ...
            if (des > _max_points)
                std::cout << "error. " << des << " exceeds max_pts"
                          << std::endl;
            /* des_pool contains the neighbors of the neighbors of n */

            {
                LockGuard guard(_locks[des]);
                if (std::find(_final_graph[des].begin(),
    ...
    void Index<T, TagT>::link(Parameters &parameters) {
        ...
        const unsigned argL =
            parameters.Get<unsigned>("L");  // Search list size
        ...
        for (uint32_t rnd_no = 0; rnd_no < NUM_RNDS; rnd_no++) {
            ...
            size_t round_size =
                DIV_ROUND_UP(_nd, NUM_SYNCS);  // size of each batch
            ...
            for (uint32_t sync_num = 0; sync_num < NUM_SYNCS; sync_num++) {
                ...
                size_t end_id = (std::min)(_nd + _num_frozen_pts,
...
#pragma omp parallel for schedule(dynamic, 64)
                for (_s64 node_ctr = start_id; node_ctr < (_s64) end_id;
                    ...
                    std::vector<unsigned> &pruned_list =
                        pruned_list_vector[node_offset];
                    batch_inter_insert(node, pruned_list, parameters,
                                       need_to_sync);
...
#pragma omp parallel for schedule(dynamic, 64)
    ...
    void Index<T, TagT>::build(Parameters &             parameters,
        ...
        if (_enable_tags) {
            if (tags.size() != _nd) {
                ...
                throw diskann::ANNException("#Tags must be equal to #points",
    ...
    int Index<T, TagT>::generate_random_frozen_points(const char *filename) {
        ...
        if (filename) {  // user defined frozen points
            T *frozen_pts;
            load_aligned_bin<T>(std::string(filename), frozen_pts,
                                _num_frozen_pts, _dim, _aligned_dim);
    ...
    int Index<T, TagT>::eager_delete(const TagT        tag,
        ...
        if (_lazy_done && (!_consolidated_order)) {
            std::cout
                << "Lazy delete reuests issued but data not consolidated, "
        ...
        for (auto it : in_nbr) {
            _final_graph[it].erase(std::remove(_final_graph[it].begin(),
                                               _final_graph[it].end(), id),
                                   _final_graph[it].end());
        }
        ...
        for (unsigned old = 0; old < _max_points + _num_frozen_pts; ++old) {
            if (new_location[old] <
                ...
                for (size_t i = 0; i < _final_graph[old].size(); ++i) {
                    assert(new_location[_final_graph[old][i]] <=
                           _final_graph[old][i]);
                ...
                if (_support_eager_delete)
                    for (size_t i = 0; i < _in_graph[old].size(); ++i) {
                        if (new_location[_in_graph[old][i]] <=
                            _in_graph[old][i])
                ...
                if (new_location[old] != old) {
                    ...
                    memcpy((void *) (_data +
                                     _aligned_dim * (size_t) new_location[old]),
    ...
    void Index<T, TagT>::readjust_data(unsigned _num_frozen_pts) {
        if (_num_frozen_pts > 0) {
            if (_final_graph[_max_points].empty()) {
                std::cout
                    << "Readjusting data to correctly position frozen point"
                    << std::endl;
                ...
                for (unsigned i = 0; i < _num_frozen_pts; i++) {
                    memcpy((void *) (_data +
                                     (size_t) _aligned_dim * (_max_points + i)),
    ...
    int Index<T, TagT>::disable_delete(const Parameters &parameters,
        ...
        if (_eager_done) {
            std::cout << "#Points after eager_delete : "
                      << _nd + _num_frozen_pts << std::endl;
        ...
        if (consolidate) {
            ...
            if (nd >= 0)
                std::cout << "#Points after consolidation: "
                          << nd + _num_frozen_pts << std::endl;
        }
