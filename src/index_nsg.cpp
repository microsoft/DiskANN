...
#define MAX_START_POINTS 100
  ...
  template<typename T>
  void IndexNSG<T>::init_random_graph(size_t num_points, unsigned k,
...
#pragma omp parallel for schedule(static, 1)
      ...
      if (retset[k].flag) {
        ...
        for (size_t m = 0; m < nnbrs; m++) {
          ...
          if (m < (nnbrs - 1)) {
            ...
            const T *vec_next1 =
                _data +
                (size_t) id_next * _dim;  // vec_next1: data of next neighbor
  ...
  int IndexNSG<T>::insert_point(const T *point, const Parameters &parameters,
    ...
    LockGuard guard(_incr_insert_lock);
    if (_nd == _max_points) {
      std::cerr << "Can not insert more than " << _max_points << " points."
                << std::endl;
      return -1;
    } else if (_nd > _max_points) {
      std::cerr << "#points " << _nd
                << " greater than _max_points: " << _max_points << std::endl;
      return -2;
    }

    auto offset_data = _data + (size_t) _dim * _nd;
    ...
    sync_prune(_nd, pool, parameters, visited, cut_graph);

    assert(_final_graph.size() == _max_points);
    _final_graph[_nd].clear();
    _final_graph[_nd].reserve(range);
    assert(!cut_graph.empty());
    for (auto link : cut_graph) {
      _final_graph[_nd].emplace_back(link.id);
      assert(link.id >= 0 && link.id < _nd);
    }
    assert(_final_graph[_nd].size() <= range);
    inter_insert(_nd, cut_graph, parameters);
...
#pragma omp parallel for schedule(static, 65536)
  ...
  void IndexNSG<T>::sync_prune(unsigned q, std::vector<Neighbor> &pool,
     ...
     */
    if (!_final_graph[q].empty())
      for (auto id : _final_graph[q]) {
        if (visited.find(id) != visited.end())
          continue;
        float dist =
            _distance->compare(_data + _dim * (size_t) q,
                               _data + _dim * (size_t) id, (unsigned) _dim);
    ...
    if (alpha > 1.0 && !pool.empty() && result.size() < range) {
      ...
      while (result2.size() < range - result.size() &&
        ...
        for (unsigned t = 0; t < result2.size(); t++) {
          ...
          float djk =
              _distance->compare(_data + _dim * (size_t) result2[t].id,
                                 _data + _dim * (size_t) p.id, (unsigned) _dim);
   ...
   */
  template<typename T>
  void IndexNSG<T>::inter_insert(unsigned n, vecNgh &cut_graph_n,
                                 const Parameters &parameter) {
    float      alpha = parameter.Get<float>("alpha");
   ...
   */
  template<typename T>
  void IndexNSG<T>::link(Parameters &parameters) {
    //    The graph will be updated periodically in NUM_SYNCS batches
    // const uint32_t NUM_SYNCS = _nd > 1 << 20 ? (_nd / (128 * 1024 * 5))
    //                                         : 20 * (_nd / (128 * 1024 * 5));
    ...
    /* ra_ndperm is a vector that is initialized to the entire graph */
    ...
    assert(_final_graph.size() == _max_points);
    // std::vector<std::mutex> locks(_max_points);
    auto cut_graph_ = new vecNgh[_nd];

    for (uint32_t r_ndno = 0; r_ndno < NUM_RNDS; r_ndno++) {
      // Shuffle the dataset
      std::random_shuffle(ra_ndperm.begin(), ra_ndperm.end());
      unsigned progress_counter = 0;

      size_t rou_ndsize = DIV_ROUND_UP(_nd, NUM_SYNCS);  // size of each batch

      for (uint32_t sync_num = 0; sync_num < NUM_SYNCS; sync_num++) {
        if (r_ndno == NUM_RNDS - 1) {
          if (last_rou_ndalpha > 1)
            parameters.Set<unsigned>(
                "L", (unsigned) (std::min)(
                         (int) L, (int) (L -
                                         (L - 30) * ((float) sync_num /
                                                     (float) NUM_SYNCS))));
          parameters.Set<float>("alpha", last_rou_ndalpha);
        }
        size_t start_id = sync_num * rou_ndsize;
        size_t e_ndid = (std::min)(_nd, (sync_num + 1) * rou_ndsize);
        size_t rou_ndsize = e_ndid - start_id;

        size_t PAR_BLOCK_SZ =
            rou_ndsize > 1 << 20 ? 1 << 12 : (rou_ndsize + 256) / 256;
        size_t nblocks = DIV_ROUND_UP(rou_ndsize, PAR_BLOCK_SZ);
...
#pragma omp parallel for schedule(dynamic, 1)
        for (int64_t block = 0; block < nblocks;
          ...
          for (size_t n = start_id + block * PAR_BLOCK_SZ;
               n <
               start_id + (std::min)(rou_ndsize, (block + 1) * PAR_BLOCK_SZ);
               ++n) {
            pool.clear();
            tmp.clear();
            visited.clear();
...
#pragma omp parallel for schedule(static, PAR_BLOCK_SZ)
        ...
        for (int node = start_id; node < e_ndid; ++node) {
...
#pragma omp parallel for schedule(static, PAR_BLOCK_SZ)
  ...
  template<typename T>
  void IndexNSG<T>::build(const T *data, Parameters &parameters) {
    unsigned range = parameters.Get<unsigned>("R");
    _data = data;
  ...
  template<typename T>
  std::pair<int, int> IndexNSG<T>::beam_search(
