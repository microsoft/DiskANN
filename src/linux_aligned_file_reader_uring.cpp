...
void execute_io(IOContext ctx, int fd, std::vector<AlignedRead> &read_reqs, uint64_t n_retries = 0)
{
    ...
    for (uint64_t iter = 0; iter < n_iters; iter++)
    {
        ...
        for (uint64_t j = 0; j < n_ops; j++)
        {
			...
			if (!sqe){
                std::cerr << "io_uring_get_sqe() failed; errno=" << errno;
                std::cout << "ctx: " << ctx << "\n";
                exit(-1);
			}
        ...
        while (n_tries <= n_retries)
        {
            ...
            {
                std::cerr << "io_uring_submit() failed; returned " << ret << ", expected=" << n_ops << ", errno=" << errno
            ...
            else
            {
                ...
                if (ret != 0)
                {
                    std::cerr << "io_uring_wait_cqes() failed; returned " << ret << ", expected=" << n_ops
                              << ", errno=" << errno << "=" << ::strerror(-ret) << ", try #" << n_tries + 1 << std::endl;
                    exit(-1);
                }
                else
                {
