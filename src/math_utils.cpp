...
#pragma omp parallel for schedule(static, 8192)
    for (int64_t n_iter = 0; n_iter < (uint64_t)num_points; n_iter++)
    {
                                      ...
                                      uint32_t *center_index, float *const dist_matrix, size_t k)
{
    ...
    {
#pragma omp parallel for schedule(static, 8192)
        for (int64_t i = 0; i < (uint64_t)num_points; i++)
        {
...
#pragma omp parallel for schedule(static, 1)
        for (int64_t j = cur_blk * PAR_BLOCK_SIZE;
             j < std::min((uint64_t)num_points, (uint64_t)((cur_blk + 1) * PAR_BLOCK_SIZE)); j++)
        {
...
#pragma omp parallel for schedule(static, 1)
    for (int64_t c = 0; c < (uint64_t)num_centers; ++c)
    {
...
#pragma omp parallel for schedule(static, 32)
        for (int64_t chunk = 0; chunk < (uint64_t)nchunks; ++chunk)
