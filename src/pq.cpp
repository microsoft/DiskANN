...
_u32 FixedChunkPQTable::get_num_chunks() { return static_cast<_u32>(n_chunks); }
...
float FixedChunkPQTable::inner_product(const float* query_vec, _u8* base_vec) {
  float res = 0;
  for (_u64 chunk = 0; chunk < n_chunks; chunk++) {
    for (_u64 j = chunk_offsets[chunk]; j < chunk_offsets[chunk + 1]; j++) {
      const float* centers_dim_vec = tables_tr + (256 * j);
...
int generate_opq_pivots(const float* passed_train_data, size_t num_train,
  ...
  for (_u32 d1 = 0; d1 < dim; d1++) *(rotmat_tr.get() + d1 * dim + d1) = 1;
...
void generate_quantized_data(const std::string data_file_to_use,
  ...
  if (!use_opq) {
    ...
    generate_opq_pivots(train_data, train_size, (_u32)train_dim,
                        NUM_PQ_CENTROIDS, (_u32)num_pq_chunks, pq_pivots_path,
                        make_zero_mean);
  }
