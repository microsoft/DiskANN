...
int generate_pq_pivots_simplified(const float *train_data, size_t num_train,
  ...
  for (size_t i = 0; i < num_pq_chunks; i++) {
    ...
    kmeans::run_lloyds(cur_data, num_train, cur_chunk_size, cur_pivot_data,
                       num_centers, KMEANS_ITERS_FOR_PQ, NULL, closest_center);

    for (uint64_t j = 0; j < num_centers; j++) {
      std::memcpy(pivot_data + j * dim + chunk_offset,
...
#pragma omp parallel for schedule(static, 65536)
    ...
    // rotation matrix
    uint32_t errcode = (uint32_t)LAPACKE_sgesdd(
...
int generate_pq_data_from_pivots_simplified(const float *data, const size_t num,
                                            const float *pivot_data,
                                            const size_t pivots_num,
                                            const size_t dim,
  ...
  for (size_t i = 0; i < num_pq_chunks; i++) {
    ...
    for (int j = 0; j < num_centers; j++) {
      std::memcpy(cur_pivot_data + j * chunk_size,
                  pivot_data + j * dim + chunk_offset,
    ...
    math_utils::compute_closest_centers(cur_data, num, chunk_size,
