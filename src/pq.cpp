...
FixedChunkPQTable::~FixedChunkPQTable() {
  ...
  if (tables_tr != nullptr) delete[] tables_tr;
  if (chunk_offsets != nullptr) delete[] chunk_offsets;
  if (centroid != nullptr) delete[] centroid;
  if (rotmat_tr != nullptr) delete[] rotmat_tr;
#endif
}
...
void FixedChunkPQTable::load_pq_centroid_bin(const char *pq_table_file,
  ...
  if (nr != 4 && nr != 5) {
    ...
    throw diskann::ANNException("Error reading pq_pivots file at offsets data.",
  ...
  if ((nr != NUM_PQ_CENTROIDS)) {
    ...
    throw diskann::ANNException("Error reading pq_pivots file at pivots data.",
                                -1, __FUNCSIG__, __FILE__, __LINE__);
  }
  ...
  if ((nr != this->ndims) || (nc != 1)) {
    diskann::cerr << "Error reading centroids from pq_pivots file "
                  ...
                  << ", file_cols = " << nc << " but expecting " << this->ndims
...
void aggregate_coords(const std::vector<uint32_t> &ids,
                      const uint8_t *all_coords, const size_t ndims,
                      uint8_t *out) {
...
// functions above
void aggregate_coords(const uint32_t *ids, const size_t n_ids,
                      const uint8_t *all_coords, const size_t ndims,
                      uint8_t *out) {
...
int generate_pq_pivots(const float *const passed_train_data, size_t num_train,
                       uint32_t dim, uint32_t num_centers,
                       uint32_t num_pq_chunks, uint32_t max_k_means_reps,
  ...
  for (uint32_t d = 0; d < dim; d++) {
    if (dim_to_bin.find(d) != dim_to_bin.end()) continue;
    ...
    if (bin_to_dims[cur_best].size() == high_val) {
      cur_num_high++;
      if (cur_num_high == max_num_high) cur_bin_threshold = low_val;
    }
  }
...
int generate_opq_pivots(const float *passed_train_data, size_t num_train,
                        uint32_t dim, uint32_t num_centers,
                        uint32_t num_pq_chunks, std::string opq_pivots_path,
                        bool make_zero_mean) {
  if (num_pq_chunks > dim) {
    diskann::cout << " Error: number of chunks more than dimension"
                  << std::endl;
  ...
  for (uint32_t d1 = 0; d1 < dim; d1++) *(rotmat_tr.get() + d1 * dim + d1) = 1;
...
int generate_pq_data_from_pivots(const std::string &data_file,
                                 uint32_t num_centers, uint32_t num_pq_chunks,
  ...
  if (!file_exists(pq_pivots_path)) {
    ...
    diskann::load_bin<size_t>(pq_pivots_path.c_str(), file_offset_data, nr, nc,
                              0);
...
void generate_disk_quantized_data(
  ...
  if (disk_pq_dims > train_dim) disk_pq_dims = train_dim;
  ...
  if (compareMetric == diskann::Metric::INNER_PRODUCT)
    generate_pq_data_from_pivots<float>(
        data_file_to_use, 256, (uint32_t)disk_pq_dims, disk_pq_pivots_path,
        disk_pq_compressed_vectors_path);
  else
    generate_pq_data_from_pivots<T>(data_file_to_use, 256,
                                    (uint32_t)disk_pq_dims, disk_pq_pivots_path,
                                    disk_pq_compressed_vectors_path);
...
void generate_quantized_data(const std::string &data_file_to_use,
  ...
  if (!file_exists(codebook_prefix)) {
    ...
    if (compareMetric == diskann::Metric::INNER_PRODUCT) make_zero_mean = false;
    ...
    diskann::cout << "Skip Training with predefined pivots in: "
                  << pq_pivots_path << std::endl;
  }
  generate_pq_data_from_pivots<T>(data_file_to_use, NUM_PQ_CENTROIDS,
