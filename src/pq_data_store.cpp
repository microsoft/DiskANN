...
PQDataStore<data_t>::PQDataStore(
    ...
    : AbstractDataStore<data_t>(num_points, dim), _quantized_data(nullptr),
      _num_chunks(num_pq_chunks), _distance_metric(distance_fn->get_metric()) {
...
size_t PQDataStore<data_t>::save(const std::string &filename,
  ...
  return diskann::save_bin(filename, _quantized_data, this->capacity(),
                           _num_chunks, 0);
}
...
template <typename data_t>
void PQDataStore<data_t>::populate_data(const data_t *vectors,
                                        const location_t num_pts) {
  throw std::logic_error("Not implemented yet");
}

template <typename data_t>
void PQDataStore<data_t>::populate_data(const std::string &filename,
                                        const size_t offset) {
  if (_quantized_data != nullptr) {
    aligned_free(_quantized_data);
  }
  ...
  generate_quantized_data<data_t>(filename, pivots_file, compressed_file,
                                  _distance_metric, p_val, _num_chunks,
  ...
  alloc_aligned(((void **)&_quantized_data),
                file_num_points * _num_chunks * sizeof(uint8_t), 1);
  copy_aligned_data_from_file<uint8_t>(compressed_file.c_str(), _quantized_data,
                                       file_num_points, _num_chunks,
                                       _num_chunks);
#ifdef EXEC_ENV_OLS
...
void PQDataStore<data_t>::get_vector(const location_t i, data_t *target) const {
  ...
  if (i < this->capacity()) {
    ...
    std::stringstream ss;
    ss << "Requested vector " << i << " but only  " << this->capacity()
       << " vectors are present";
    throw diskann::ANNException(ss.str(), -1);
  }
}
template <typename data_t>
void PQDataStore<data_t>::set_vector(const location_t i,
...
void PQDataStore<data_t>::prefetch_vector(const location_t loc) {
  const uint8_t *ptr =
      _quantized_data + ((size_t)loc) * _num_chunks * sizeof(data_t);
...
template <typename data_t>
void PQDataStore<data_t>::copy_vectors(const location_t from_loc,
                                       const location_t to_loc,
                                       const location_t num_points) {
  // REFACTOR TODO: Is the number of bytes correct?
  memcpy(_quantized_data + to_loc * _num_chunks,
         _quantized_data + from_loc * _num_chunks, _num_chunks * num_points);
}

// REFACTOR TODO: Currently, we take aligned_query as parameter, but this
// function should also do the alignment.
template <typename data_t>
void PQDataStore<data_t>::preprocess_query(
    const data_t *aligned_query, AbstractScratch<data_t> *scratch) const {
  if (scratch == nullptr) {
  ...
  if (pq_scratch == nullptr) {
    throw diskann::ANNException(
        "PQScratch space has not been set in the scratch object.", -1);
  }

  _pq_distance_fn->preprocess_query(aligned_query, (location_t)this->get_dims(),
                                    *pq_scratch);
}
...
template <typename data_t>
void PQDataStore<data_t>::get_distance(
    const data_t *preprocessed_query, const location_t *locations,
  ...
  diskann::aggregate_coords(locations, location_count, _quantized_data,
                            this->_num_chunks,
                            pq_scratch->aligned_pq_coord_scratch);
  _pq_distance_fn->preprocessed_distance(*pq_scratch, location_count,
                                         distances);
}
...
void PQDataStore<data_t>::get_distance(
  ...
  diskann::aggregate_coords(ids, _quantized_data, this->_num_chunks,
                            pq_scratch->aligned_pq_coord_scratch);
  _pq_distance_fn->preprocessed_distance(*pq_scratch, (location_t)ids.size(),
                                         distances);
}
...
location_t PQDataStore<data_t>::load_impl(const std::string &file_prefix) {
  ...
  auto quantized_vectors_file =
      _pq_distance_fn->get_quantized_vectors_filename(file_prefix);

  size_t num_points;
  load_aligned_bin(quantized_vectors_file, _quantized_data, num_points,
...
#ifdef EXEC_ENV_OLS
template <typename data_t>
location_t PQDataStore<data_t>::load_impl(AlignedFileReader &reader) {}
#endif
