...
#define OFFSET_TO_NODE(sector_buf, node_id) \
  ((char *)sector_buf +                     \
   (((uint64_t)node_id) % nnodes_per_sector) * max_node_len)
...
PQFlashIndex<T, LabelT>::~PQFlashIndex() {
  ...
  if (centroid_data != nullptr) aligned_free(centroid_data);
...
void PQFlashIndex<T, LabelT>::cache_bfs_levels(uint64_t num_nodes_to_cache,
  ...
  for (auto node : node_set) node_list.push_back(node);
  for (auto node : *cur_level) node_list.push_back(node);
...
void PQFlashIndex<T, LabelT>::get_label_file_metadata(
    std::string map_file, uint32_t &num_pts, uint32_t &num_total_labels) {
...
template <typename T, typename LabelT>
inline bool PQFlashIndex<T, LabelT>::point_has_label(uint32_t point_id,
                                                     uint32_t label_id) {
...
// read index metadata
  ...
  if (this->num_frozen_points == 1) this->frozen_location = file_frozen_id;
...
bool getNextCompletedRequest(const IOContext &ctx, size_t size,
  ...
  do {
    ...
    if (waitsRemaining) {
      WaitOnAddress(&ctx.m_completeCount, &completeCount, sizeof(completeCount),
                    100);
...
void PQFlashIndex<T, LabelT>::cached_beam_search(
  ...
  cached_beam_search(query1, k_search, l_search, indices, distances, beam_width,
                     false, dummy_filter, std::numeric_limits<uint32_t>::max(),
...
void PQFlashIndex<T, LabelT>::cached_beam_search(
  ...
  while (retset.has_unexpanded_node() && num_ios < io_limit) {
    ...
    for (auto &cached_nhood : cached_nhoods) {
      ...
      if (!use_disk_index_pq) {
        cur_expanded_dist = dist_cmp->compare(
            aligned_query_T, node_fp_coords_copy, (uint32_t)aligned_dim);
      } else {
        if (metric == diskann::Metric::INNER_PRODUCT)
      ...
      for (uint64_t m = 0; m < nnbrs; ++m) {
        ...
        if (visited.insert(id).second) {
          if (!use_filter && _dummy_pts.find(id) != _dummy_pts.end()) continue;
    ...
    for (auto &frontier_nhood : frontier_nhoods) {
      ...
      if (data_buf_idx == MAX_N_CMPS) data_buf_idx = 0;
  ...
  for (uint64_t i = 0; i < k_search; i++) {
    ...
    if (distances != nullptr) {
      ...
      if (metric == diskann::Metric::INNER_PRODUCT) {
        ...
        if (max_base_norm != 0) distances[i] *= (max_base_norm * query_norm);
      }
    }
  }
...
uint32_t PQFlashIndex<T, LabelT>::range_search(
  ...
  while (!stop_flag) {
    ...
    for (auto &x : distances) x = std::numeric_limits<float>::max();
    ...
    if (res_count < (uint32_t)(l_search / 2.0)) stop_flag = true;
    l_search = l_search * 2;
    if (l_search > max_l_search) stop_flag = true;
  }
