                                                                      ...
                                                                      std::vector<uint32_t> &node_list)
{
    ...
    if (_filter_to_medoid_ids.size() != 0) {
        ...
        generate_random_labels(random_query_filters, sample_num, nthreads);
    }
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(nthreads)
    for (int64_t i = 0; i < (int64_t)sample_num; i++)
    {
        ...
        cached_beam_search(samples + (i * sample_aligned_dim), 1, l_search, tmp_result_ids_64.data() + (i * 1),
                           tmp_result_dists.data() + (i * 1), beamwidth, filtered_search, label_for_search, false);
    }
...
template <typename T, typename LabelT>
void PQFlashIndex<T, LabelT>::generate_random_labels(std::vector<LabelT> &labels, const uint32_t num_labels, const uint32_t nthreads) {
    std::random_device rd;  
    labels.clear();
    labels.resize(num_labels);

    uint64_t num_total_labels = _pts_to_label_offsets[num_points - 1] + _pts_to_labels[_pts_to_label_offsets[num_points - 1]];
    std::mt19937 gen(rd());
        ...
        if (file_exists(labels_to_medoids))
        {
            ...
            try
            {
                while (std::getline(medoid_stream, line))
                {
                    ...
                    _filter_to_medoid_ids[label] = medoids;
                }
            }
                                                 ...
                                                 QueryStats *stats)
{
    ...
    {
        best_medoid = _filter_to_medoid_ids[filter_label][best_shard]; 
    }
    else
    {
        std::string exception("Cannot find medoid for specified filter.");
        exception += std::to_string(filter_label);
        throw ANNException(exception, -1, __FUNCSIG__, __FILE__, __LINE__);
    }
    }
