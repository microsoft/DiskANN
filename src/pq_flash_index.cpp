...
template <typename T, typename LabelT>
inline bool PQFlashIndex<T, LabelT>::point_has_label(_u32 point_id, LabelT label_id)
{
...
void PQFlashIndex<T, LabelT>::cached_beam_search(const T *query1, const _u64 k_search, const _u64 l_search,
                                                 _u64 *indices, float *distances, const _u64 beam_width,
                                                 const bool use_filter, const std::string &filter_label, const _u32 io_limit,
                                                 const bool use_reorder_data, QueryStats *stats)
{ 
    LabelT label;
    try{
        label = get_converted_label(filter_label);
        ...
        for (auto &frontier_nhood : frontier_nhoods)
        {
            ...
            for (_u64 m = 0; m < nnbrs; ++m)
            {
                ...
                if (visited.insert(id).second)
                {
                    ...
                    if (use_filter && !point_has_label(id, label) && !point_has_label(id, _use_universal_label))
                        continue;
                    cmps++;
                    float dist = dist_scratch[m];
                    if (stats != nullptr)
