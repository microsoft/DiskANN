...
template <typename T, typename LabelT>
void PQFlashIndex<T, LabelT>::cache_bfs_levels(_u64 num_nodes_to_cache,
                                               std::vector<uint32_t> &node_list,
                                               const bool shuffle) {
...
// read index metadata
  ...
  if (this->reorder_data_exists) {
    if (this->use_disk_index_pq == false) {
      throw ANNException(
          "Reordering is designed for used with disk PQ compression option", -1,
...
void PQFlashIndex<T, LabelT>::cached_beam_search(
    ...
    QueryStats *stats) {
  cached_beam_search(query1, k_search, l_search, indices, distances, beam_width,
                     use_filter, filter_label, std::numeric_limits<_u32>::max(),
...
void PQFlashIndex<T, LabelT>::cached_beam_search(
  ...
  cached_beam_search(query1, k_search, l_search, indices, distances, beam_width,
                     false, dummy_filter, std::numeric_limits<_u32>::max(),
                     use_reorder_data, stats);
}
...
template <typename T, typename LabelT>
_u32 PQFlashIndex<T, LabelT>::range_search(const T *query1, const double range,
  ...
  while (!stop_flag) {
    ...
    if (res_count < (_u32)(l_search / 2.0)) stop_flag = true;
    l_search = l_search * 2;
