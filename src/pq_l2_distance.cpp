...
PQL2Distance<data_t>::PQL2Distance(uint32_t num_chunks, bool use_opq)
    : _num_chunks(num_chunks), _is_opq(use_opq) {}
...
std::string PQL2Distance<data_t>::get_quantized_vectors_filename(
  ...
  return diskann::get_quantized_vectors_filename(prefix, _is_opq,
                                                 (uint32_t)_num_chunks);
}
template <typename data_t>
std::string
PQL2Distance<data_t>::get_pivot_data_filename(const std::string &prefix) const {
  if (_num_chunks == 0) {
    throw diskann::ANNException(
        "Must set num_chunks before calling get_pivot_data_filename", -1,
        __FUNCSIG__, __FILE__, __LINE__);
  }
  return diskann::get_pivot_data_filename(prefix, _is_opq,
                                          (uint32_t)_num_chunks);
}
template <typename data_t>
std::string PQL2Distance<data_t>::get_rotation_matrix_suffix(
    const std::string &pq_pivots_filename) const {
  return diskann::get_rotation_matrix_suffix(pq_pivots_filename);
}

#ifdef EXEC_ENV_OLS
template <typename data_t>
void PQL2Distance<data_t>::load_pivot_data(MemoryMappedFiles &files,
...
void PQL2Distance<data_t>::load_pivot_data(const std::string &pq_table_file,
  ...
  diskann::load_bin<float>(files, pq_table_file, centroid, nr, nc,
                           file_offset_data[1]);
#else
  diskann::load_bin<float>(pq_table_file, _centroid, nr, nc,
                           file_offset_data[1]);
#endif
  ...
  if ((nr != this->_ndims) || (nc != 1)) {
    diskann::cerr << "Error reading centroids from pq_pivots file "
                  ...
                  << ", file_cols = " << nc << " but expecting " << this->_ndims
...
#else
  diskann::load_bin<uint32_t>(pq_table_file, _chunk_offsets, nr, nc,
                              file_offset_data[chunk_offsets_index]);
#endif
...
template <typename data_t>
void PQL2Distance<data_t>::preprocess_query(const data_t *aligned_query,
                                            uint32_t dim,
                                            PQScratch<data_t> &scratch) {
...
template <typename data_t>
float PQL2Distance<data_t>::brute_force_distance(const float *query_vec,
                                                 uint8_t *base_vec) {
  float res = 0;
...
template <typename data_t>
void PQL2Distance<data_t>::prepopulate_chunkwise_distances(
    const float *query_vec, float *dist_vec) {
