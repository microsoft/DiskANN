...
const char* DISTANCE_FUNCTION_DESCRIPTION = "distance function <l2/mips/fast_l2/cosine>.  'fast l2' and 'mips' only support data_type float";
const char* INDEX_PATH_PREFIX_DESCRIPTION = "Path prefix to the index, e.ge '/mnt/data/my_ann_index'";
const char* RESULT_PATH_DESCRIPTION = "Path prefix for saving results of the queries, e.g. '/mnt/data/query_file_X.bin'";
...
const char* FILTERS_FILE_DESCRIPTION ="Filter file for Queries for Filtered Search.  File format is text with one filter per line.  File must "
                          "have exactly one filter OR the same number of filters as there are queries in the 'query_file'.";
const char* LABEL_TYPE_DESCRIPTION ="Storage type of Labels <uint/ushort>, default value is uint which will consume memory 4 bytes per filter";
