...
pplx::task<void> Server::open() { return _listener->open(); }
pplx::task<void> Server::close() { return _listener->close(); }
...
void Server::handle_post(web::http::http_request message) {
  message.extract_string(true)
      ...
      .then(
          [=](std::pair<short unsigned int, web::json::value> response_status) {
            try {
              message.reply(response_status.first, response_status.second)
                  .wait();
            } catch (const std::exception& ex) {
