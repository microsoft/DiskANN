...
//
template <typename T>
InMemQueryScratch<T>::InMemQueryScratch(uint32_t search_l, uint32_t indexing_l,
  ...
  alloc_aligned(((void **)&this->_aligned_query_T), aligned_dim * sizeof(T),
                alignment_factor * sizeof(T));
  memset(this->_aligned_query_T, 0, aligned_dim * sizeof(T));

  if (init_pq_scratch)
    this->_pq_scratch =
        new PQScratch<T>(defaults::MAX_GRAPH_DEGREE, aligned_dim);
  else
  ...
  _id_scratch.reserve(
      (size_t)std::ceil(1.5 * defaults::GRAPH_SLACK_FACTOR * _R));
  _dist_scratch.reserve(
      (size_t)std::ceil(1.5 * defaults::GRAPH_SLACK_FACTOR * _R));
...
template <typename T> void SSDThreadData<T>::clear() { scratch.reset(); }
