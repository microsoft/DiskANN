...
//
template <typename T>
InMemQueryScratch<T>::InMemQueryScratch(MemoryManager &memory_manager, uint32_t search_l, uint32_t indexing_l,
                                        uint32_t r, uint32_t maxc, size_t dim, size_t aligned_dim,
                                        size_t alignment_factor, bool init_pq_scratch)
    : _memory_manager(memory_manager), _L(0), _R(r), _maxc(maxc),
      _occlude_list_output(memory_manager.create_allocator<uint32_t>())
{
    ...
    _memory_manager.alloc_aligned(((void **)&_aligned_query), aligned_dim * sizeof(T), alignment_factor * sizeof(T));
    memset(_aligned_query, 0, aligned_dim * sizeof(T));

    if (init_pq_scratch)
        _pq_scratch = new PQScratch<T>(_memory_manager, MAX_GRAPH_DEGREE, aligned_dim);
    else
        _pq_scratch = nullptr;
...
template <typename T> InMemQueryScratch<T>::~InMemQueryScratch()
{
    ...
    {
        _memory_manager.aligned_free(_aligned_query);
    }
...
template <typename T>
SSDQueryScratch<T>::SSDQueryScratch(MemoryManager &memory_manager, size_t aligned_dim, size_t visited_reserve)
    : _memory_manager(memory_manager)
{
    size_t coord_alloc_size = ROUND_UP(MAX_N_CMPS * aligned_dim, 256);

    _memory_manager.alloc_aligned((void **)&coord_scratch, coord_alloc_size, 256);
    _memory_manager.alloc_aligned((void **)&sector_scratch, (size_t)MAX_N_SECTOR_READS * (size_t)SECTOR_LEN,
                                  SECTOR_LEN);
    _memory_manager.alloc_aligned((void **)&aligned_query_T, aligned_dim * sizeof(T), 8 * sizeof(T));
...
{
    _memory_manager.aligned_free((void *)coord_scratch);
    _memory_manager.aligned_free((void *)sector_scratch);

    delete[] _pq_scratch;
}

template <typename T>
SSDThreadData<T>::SSDThreadData(MemoryManager &memory_manager, size_t aligned_dim, size_t visited_reserve)
    : scratch(memory_manager, aligned_dim, visited_reserve)
{
}
