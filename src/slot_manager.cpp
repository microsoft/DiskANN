...
namespace diskann {
  ...
  SlotManager<tag_t>::SlotManager(const location_t capacity,
                                  const location_t num_frozen_points)
      : _capacity(capacity),
        _num_frozen_points(num_frozen_points),
        _next_available_slot(0) 
  {
  ...
  template <typename tag_t>
  location_t SlotManager<tag_t>::capacity() const 
  {
    return _capacity;
  }
  template <typename tag_t>
  location_t SlotManager<tag_t>::num_frozen_points() const 
  {
    return _num_frozen_points;
  }

  template <typename tag_t>
  location_t SlotManager<tag_t>::num_used_slots() const 
  {
    return number_of_active_points() + number_of_deleted_points();
  }

  template <typename tag_t>
  location_t SlotManager<tag_t>::num_deleted_points() const 
  {
    return _delete_set.size();
  }

  template <typename tag_t>
  location_t SlotManager<tag_t>::num_available_slots() const 
  {
    return _capacity - number_of_used_slots() + _empty_slots.size();
  }

  // This is the number of slots occupied by active points, i.e., points that are
  // not soft deleted.
  template <typename tag_t>
  location_t SlotManager<tag_t>::num_active_points() const 
  {
    return _tag_to_location.size();
  }

  template <typename tag_t>
  location_t SlotManager<tag_t>::load(const std::string &filename) {
    // We must load the delete set first because during save we don't remove
    // deleted points. Any point that is present in tags file but is also in the
    // delete set will be discarded.
  ...
  template <typename tag_t>
  void SlotManager<tag_t>::load_tags(const std::string &tag_filename) {
    ...
    // REFACTOR: We used to put zeros for num_frozen_points at the end of the tag
    // file
    //  during save and ignore them during load. Now we'll avoid it, so that way
    //  the slot manager need not know about num_frozen_points.
  ...
  template <typename tag_t>
  void SlotManager<tag_t>::save_tags(const std::string &tags_filename) {
    if (!_enable_tags) {
    ...
    // REFACTOR: We used to put zeros for num_frozen_points at the end of the tag
    // file and remove them during load. Avoiding it now. if (_num_frozen_pts > 0)
    //{
    ...
    try {
      // REFACTOR
      // tag_bytes_written = save_bin<TagT>(tags_filename, tag_data, _nd +
      // _num_frozen_pts, 1);
  ...
  template <typename tag_t>
  void SlotManager<tag_t>::load_delete_set(const std::string &filename) {
  ...
  template <typename tag_t>
  void SlotManager<tag_t>::save_delete_set(const std::string &filename) {
  ...
  template <typename tag_t>
  location_t SlotManager<tag_t>::resize(const location_t new_num_points) {
    if (new_num_points > _tag_to_location.size()) {
      _location_to_tag.reserve(new_num_points);
      _tag_to_location.reserve(new_num_points);
    }
    // REFACTOR TODO: It is not clear if we should support shrink as well, but
    // currently, we will not.
    return _tag_to_location.size();
  }

  template <typename tag_t>
  location_t SlotManager<tag_t>::get_location_for_tag(const tag_t &tag) {
    return _tag_to_location[tag];
  }

  template <typename tag_t>
  tag_t SlotManager<tag_t>::get_tag_at_location(const location_t slot) {
  ...
  template <typename tag_t>
  SlotManagerErrorCode SlotManager<tag_t>::add_tag(const tag_t &tag,
                                                   location_t &location);
  ...
  template <typename tag_t>
  SlotManagerErrorCode SlotManager<tag_t>::delete_tag(const tag_t &tag,
                                                      location_t &location);

  template <typename tag_t>
  bool SlotManager<tag_t>::exists(const tag_t &tag);

  template <typename tag_t>
  void SlotManager<tag_t>::compact(std::vector<location_t> &new_locations);

  // TODO: these are intrusive methods, but keeping them to make the port easier.
  // Must revisit later.
  template <typename tag_t>
  void SlotManager<tag_t>::get_delete_set(
      std::vector<location_t> &copy_of_delete_set);
  template <typename tag_t>
  void SlotManager<tag_t>::clear_delete_set();
