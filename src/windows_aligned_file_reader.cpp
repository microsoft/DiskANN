...
void WindowsAlignedFileReader::register_thread() {
  ...
  if (ctx.fhandle == INVALID_HANDLE_VALUE) {
    ...
    if (wcstombs_s(&actual_len, filePath, c_max_filepath_len,
                   m_filename.c_str(), m_filename.length()) == 0) {
      diskann::cout << "Error opening " << filePath
                    << " -- error=" << GetLastError() << std::endl;
    } else {
      diskann::cout << "Error converting wchar to char -- error="
