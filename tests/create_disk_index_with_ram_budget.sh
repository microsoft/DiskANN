...
{
    echo ""
    echo "Usage: $0 -t data_type -i input_bin_file -o index_prefix -L Lvalue -R Rvalue -B PQ_vector_bytes -M ram_budget(GB) -S sampling_rate"
    ...
    echo -e "\t-M max ram usage during indexing (GB)"
...
if [ -z "$TYPE" ] || [ -z "$DATA" ] || [ -z "$OUTPUT_PREFIX" ] || [ -z "$L" ] || [ -z "$R" ] || [ -z "$B" ] || [ -z "$RAM_BUDGET" ] || [ -z "$RATE" ]
then
...
echo "Building $TYPE disk-index on $DATA by partitioning with $RAM_BUDGET GB budget on DRAM with replication factor of 2, with L=$L, R=$R, B=$B and storing output files in prefix $OUTPUT_PREFIX"
...
#partitions the data using k-means into $RAM_BUDGET pieces, by placing each point in 2 closest clusters to obtain overlapping clusters. k-means is run on sampled data using $RATE sampling rate
#`rm ${OUTPUT_PREFIX}_subshard-*`
${BUILD_PATH}/tests/partition_with_ram_budget $TYPE $DATA $OUTPUT_PREFIX $RATE $RAM_BUDGET $SHARD_R 2
