...
std::tuple<float, float, float> search_index(
    ...
    std::vector<std::pair<_u64, unsigned>>& unatomic_counter) {
  ...
  std::vector<std::pair<_u64, std::atomic<unsigned>>> node_visit_counter(
      num_points);
  unatomic_counter.resize(num_points);

  for (_u64 i = 0; i < node_visit_counter.size(); i++) {
    node_visit_counter[i].first = i;
    node_visit_counter[i].second = 0;
  }
...
#pragma omp  parallel for schedule(dynamic, 1) num_threads(16)
  for (_s64 i = 0; i < query_num; i++)
    _pFlashIndex->cached_beam_search(
        ...
        node_visit_counter, stats + i);

  for (_u64 i = 0; i < node_visit_counter.size(); i++) {
    unatomic_counter[i].first = node_visit_counter[i].first;
    unatomic_counter[i].second = node_visit_counter[i].second;
  }
...
int aux_main(int argc, char** argv) {
  ...
  std::vector<_u64> node_id_cache(0);
  std::vector<std::pair<_u64, unsigned>> unatomic_counter(0);
  ...
  // for query search
  {
    ...
    q_stats =
        search_index(_pFlashIndex, (const char*) query, query_num, recall_at,
                     query_dists, query_res, curL, unatomic_counter);

    std::sort(unatomic_counter.begin(), unatomic_counter.end(),
              custom_sort_func);

    node_id_cache.resize(num_cache_nodes);
    for (uint32_t node_idx = 0; node_idx < num_cache_nodes; node_idx++) {
      node_id_cache[node_idx] = unatomic_counter[node_idx].first;
    }

    std::cout << "Saving visit ctr file" << std::endl;
    std::string node_cache_path = std::string(argv[2]) + "_visit_ctr.bin";
    NSG::save_bin<_u64>(node_cache_path.c_str(), node_id_cache.data(),
                        num_cache_nodes, 1);
