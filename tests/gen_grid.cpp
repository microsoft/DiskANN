...
void add_point(std::ofstream& out, const int dim,
               std::vector<float>& fixed_vals, const std::vector<float>& vals,
               bool rand = 1) {
...
int main(int argc, char** argv) {
  if (argc != 6) {
    std::cout << "Usage: gen_grid <dim> <min> <max> <points/dim> <output.fvecs>"
              << std::endl;
    return -1;
  }
