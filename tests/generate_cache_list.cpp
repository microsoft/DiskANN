...
int generate_cache_list(int argc, char** argv) {
  ...
  std::string warmup_bin(argv[6]);
  _u64        Lsearch = std::atoi(argv[7]);
  _u32        beam_width = std::atoi(argv[8]);
  _u64        num_cache_nodes = std::atoi(argv[9]);
  std::string cache_list_bin(argv[10]);

  _u32 num_threads = 16;
  _u32 cache_nlevels = 2;
  ...
  if (argc != 11) {
    std::cout
        ...
        << " <index_type[float/int8/uint8]>  <pq_centroids_bin> "
           ...
           "<warmup_bin>  "
           "<Lsearch> <beam_width> <num_nodes_to_cache> <output_cache_list_bin>"
        << std::endl;
    exit(-1);
  }
