...
void thread_fn(uint64_t size, uint64_t qlen, uint64_t filesize, uint64_t niters,
               double &time) {
...
int main(int argc, char **argv) {
  if (argc != 6) {
    std::cout
        << argv[0]
        << " filename nthreads read_size niters_per_thread qlen_per_thread"
        << std::endl;
    exit(-1);
  }
  ...
  for (unsigned i = 0; i < nthreads; i++) {
    std::thread th(thread_fn, read_size, qlen, filesize, niters,
                   std::ref(times[i]));
