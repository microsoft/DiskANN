...
_u64 get_max_degree(const std::vector<std::vector<unsigned>> &graph){
...
void read_nsg_blk(std::ifstream &reader, std::vector<std::vector<unsigned>> &block, _u64 blk_size){
  // width=100;
  _u64 read = 0;
  while (!reader.eof() && read++ < blk_size) {
    unsigned k;
    reader.read((char *) &k, sizeof(unsigned));
    if (reader.eof())
      break;
    std::vector<unsigned> tmp(k);
    reader.read((char *) tmp.data(), k * sizeof(unsigned));
    block.push_back(tmp);
  }
}

template<typename InType, typename OutType>
void optimize_and_write(char *                                    out_filename, 
                        char* nsg_filename, std::vector<std::vector<unsigned>> &nsg, 
  ...
  std::ofstream       writer(out_filename, std::ios::binary | std::ios::out);
  
  // compute max_degree
  _u64 max_degree = get_max_degree(nsg);
  ...
  // write out header
  write_header<OutType>(writer, nsg, npts, ndims, medoid, scale_factor);
  _u64 total_offset = 23625728;

  // delete nsg
  nsg.clear();

  // read in blocks of nsg
  std::vector<std::vector<unsigned>> nsg_blk;
  std::ifstream nsg_reader(nsg_filename, std::ios::binary);
  nsg_reader.seekg(2*sizeof(unsigned), std::ios::beg);
  ...
  for (_u64 i = 0; i < n_blks; i++) {
    ...
    // read in nsg block
    read_nsg_blk(nsg_reader, nsg_blk, blk_size);
    std::cout << "Block #" << i << ", # nodes: " << nsg_blk.size() << std::endl;

    // fill write_buf for block
    for (_u64 j = blk_start; j < blk_end; j++) {
      _u64     nnbrs = nsg_blk[j-blk_start].size();
      ...
      memcpy(write_buf + blk_write_size + node_write_size,
             (char *) nsg_blk[j-blk_start].data(), nnbrs * sizeof(unsigned));
      node_write_size += nnbrs * sizeof(unsigned);

      // write coords of each nbr
      for (_u64 k = 0; k < nnbrs; k++) {
        _u64          nbr_id = nsg_blk[j-blk_start][k];
...
void templated_optimize_and_write(char *filename, char* nsg_filename,
  ...
  if (in_type == "int8" && out_type == "int8") {
    optimize_and_write<int8_t, int8_t>(filename, nsg_filename, nsg, (int8_t *) data, medoid,
                                       npts, ndims, scale);
  } else if (in_type == "float" && out_type == "int8") {
    optimize_and_write<float, int8_t>(filename, nsg_filename, nsg, (float *) data, medoid,
                                      npts, ndims, scale);
  } else if (in_type == "uint8" && out_type == "uint8") {
    optimize_and_write<uint8_t, uint8_t>(filename, nsg_filename, nsg, (uint8_t *) data,
...
int main(int argc, char **argv) {
  ...
  templated_optimize_and_write(argv[3], argv[2], nsg, data, npts, ndims, medoid, scale,
                               in_type, out_type);
