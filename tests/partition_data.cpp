...
int main(int argc, char** argv) {
  ...
  const size_t      sample_size = (size_t) atoi(argv[4]);
  ...
  if (std::string(argv[1]) == std::string("float"))
    partition<float>(data_path, sample_size, num_partitions, max_reps,
                     prefix_path, k_index);
  else if (std::string(argv[1]) == std::string("int8"))
    partition<int8_t>(data_path, sample_size, num_partitions, max_reps,
                      prefix_path, k_index);
  else if (std::string(argv[1]) == std::string("uint8"))
    partition<uint8_t>(data_path, sample_size, num_partitions, max_reps,
                       prefix_path, k_index);
  else
