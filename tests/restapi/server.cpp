    ...
    message.extract_string(true)
        .then([=](utility::string_t body) {
            ...
            try
            {
                ...
                response[utility::conversions::to_string_t(INDICES_KEY)] = idsToJsonArray(result);
                response[utility::conversions::to_string_t(DISTANCES_KEY)] = distancesToJsonArray(result);
                if (result.tags_enabled())
                    response[utility::conversions::to_string_t(TAGS_KEY)] = tagsToJsonArray(result);
                if (result.partitions_enabled())
                    response[utility::conversions::to_string_t(PARTITION_KEY)] = partitionsToJsonArray(result);

                response[utility::conversions::to_string_t(TIME_TAKEN_KEY)] =
            ...
            catch (const std::exception &ex)
            {
                ...
                web::json::value response = prepareResponse(queryId, K);
                response[utility::conversions::to_string_t(ERROR_MESSAGE_KEY)] = web::json::value::value(utility::conversions::to_string_t(ex.what()));
            ...
            catch (...)
            {
                ...
                web::json::value response = prepareResponse(queryId, K);
                response[utility::conversions::to_string_t(ERROR_MESSAGE_KEY)] =
                    web::json::value::value(utility::conversions::to_string_t(UNKNOWN_ERROR));
                return std::make_pair(web::http::status_codes::InternalError, response);
            }
        })
...
web::json::value Server::prepareResponse(const int64_t &queryId, const int k)
{
    ...
    response[utility::conversions::to_string_t(QUERY_ID_KEY)] = queryId;
    response[utility::conversions::to_string_t(K_KEY)] = k;
                       ...
                       unsigned int &dimensions, unsigned &Ls)
{
    ...
    web::json::array queryArr = val.at(utility::conversions::to_string_t(VECTOR_KEY)).as_array();
    queryId = val.has_field(utility::conversions::to_string_t(QUERY_ID_KEY))
                  ? val.at(utility::conversions::to_string_t(QUERY_ID_KEY)).as_number().to_int64()
                  : -1;
    Ls = val.has_field(utility::conversions::to_string_t(L_KEY))
             ? val.at(utility::conversions::to_string_t(L_KEY)).as_number().to_uint32()
             : DEFAULT_L;
    k = val.at(utility::conversions::to_string_t(K_KEY)).as_integer();
...
web::json::value Server::tagsToJsonArray(const diskann::SearchResult &result)
{
    ...
    {
        tagArray[i] = web::json::value::value(utility::conversions::to_string_t(tags[i]));
    }
    return tagArray;
}
