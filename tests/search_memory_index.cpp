...
int search_memory_index(diskann::Metric &metric, const std::string &index_path,
  ...
  const size_t num_frozen_pts =
      IndexType::get_graph_num_frozen_points(index_path);
  IndexType index(metric, query_dim, 0, dynamic, tags, concurrent,
                  pq_dist_build, num_pq_chunks, use_opq, num_frozen_pts);
  ...
  if (metric == diskann::FAST_L2) index.optimize_index_layout();
...
#pragma omp parallel for schedule(dynamic, 1)
    ...
    if (show_qps_per_thread) displayed_qps /= num_threads;
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    desc.add_options()("search_list,L",
                       po::value<std::vector<uint32_t>>(&Lvec)->multitoken(),
