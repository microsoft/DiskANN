...
int search_memory_index(diskann::Metric& metric, const std::string& index_path,
  ...
  diskann::Index<T, uint32_t> index(metric, query_dim, 0, true, true,
                                 false);
  ...
  // uint32_t*     query_result_tags = new uint32_t[recall_at * query_num*Lvec.size()];
...
#pragma omp parallel for schedule(dynamic, 1)
    for (int64_t i = 0; i < (int64_t) query_num; i++) {
      auto qs = std::chrono::high_resolution_clock::now();
        cmp_stats[i] =
            index
                .search_with_tags(query + i * query_aligned_dim, recall_at, L, query_result_tags+i*recall_at,
                        nullptr, res);
