...
void load_data(char* filename, int*& data, unsigned long long& num,
  ...
  unsigned long long int data_size =
      (unsigned long long) num * (unsigned long long) dim;
...
void load_fdata(char* filename, float*& data, unsigned long long& num,
                unsigned& dim) {  // load data with sift10K pattern
  std::ifstream in(filename, std::ios::binary);
  if (!in.is_open()) {
    std::cout << "open file error" << std::endl;
...
int main(int argc, char** argv) {
  if (argc != 7 && argc != 8) {
    std::cout << argv[0]
              << "ground_truth_ivecs our_ivecs base_file query_file r R  (for "
                 "recall r (gt) at R (or)) our_dist_file (optional)"
              << std::endl;
    exit(-1);
  }
  int*     gold_std = NULL;
  ...
  if ((dim_or < dim_gs) || (!(r < dim_gs)) || (!(R < dim_or))) {
    std::cout << "ground truth has size " << dim_gs << " and our set has only "
              << dim_or << " points, and r and R are " << r << " and " << R
  ...
  for (ull i = 0; i < points_num; i++) {
    ...
    for (ull j1 = 0; j1 < r; j1++)
      for (ull j2 = 0; j2 < R; j2++)
        if (gold_std[i * (ull) dim_gs + j1] ==
            our_results[i * (ull) dim_or + j2]) {
