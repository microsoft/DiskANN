...
void build_incremental_index(
    ...
    const float alpha, const uint32_t thread_count, size_t points_to_skip,
    size_t max_points_to_insert, size_t beginning_index_size,
    float start_point_norm, uint32_t num_start_pts,
  ...
  diskann::Index<T, TagT> index(diskann::L2, dim, max_points_to_insert, true,
                                params, L, thread_count, enable_tags,
                                concurrent);
    ...
    for (size_t start = current_point_offset; start < last_point_threshold;
      ...
      if (!delete_launched && end >= start_deletes_after &&
        ...
        delete_task = std::async(std::launch::async, [&]() {
          delete_from_beginning(index, delete_params, points_to_skip,
                                points_to_delete_from_beginning);
        });
      }
    }
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    desc.add_options()(
        "num_start_points",
        po::value<uint32_t>(&num_start_pts)->default_value(0),
  ...
  try {
    if (data_type == std::string("int8"))
      build_incremental_index<int8_t>(
          ...
          num_start_pts, points_per_checkpoint, checkpoints_per_snapshot,
