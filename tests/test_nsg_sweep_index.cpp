...
int main(int argc, char** argv) {
  if (argc != 14) {
    std::cout << "Correct usage\n"
              ...
              << "num_hier<1>  num_syncs<10> num_pass<1> innerL innerR innerC"
              << std::endl;
    exit(-1);
  }
  ...
  unsigned    num_syncs = (float) std::atof(argv[9]);
  unsigned    num_rnds = (unsigned) std::atoi(argv[10]);
  unsigned    innerL = (unsigned) atoi(argv[11]);
  unsigned    innerR = (unsigned) atoi(argv[12]);
