...
#pragma omp atomic
    ...
    auto big_ret =
        big_index.BeamSearch(query_load + i * dim, data_load, K, big_params,
...
#pragma omp atomic
  ...
  std::cout << "Average small hops: " << (float) small_hops / (float) query_num
            << std::endl
            << "Average small cmps: " << (float) small_cmps / (float) query_num
            << std::endl;

  std::cout << "Average big hops: " << (float) big_hops / (float) query_num
            << std::endl
            << "Average big cmps: " << (float) big_cmps / (float) query_num
            << std::endl;
