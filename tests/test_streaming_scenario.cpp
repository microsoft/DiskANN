                             ...
                             const float start_point_norm, uint32_t num_start_pts, const std::string &save_path)
{
    ...
    if (num_points < max_points_to_insert)
        throw diskann::ANNException("num_points " + std::to_string(num_points) + " < max_points_to_insert " + std::to_string(max_points_to_insert), -1, __FUNCSIG__, __FILE__, __LINE__);
