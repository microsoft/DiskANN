...
template <typename T>
void build_incremental_index(const std::string &data_path, const uint32_t L,
                             const uint32_t R, const float alpha,
                             const uint32_t insert_threads,
                             const uint32_t consolidate_threads,
  ...
  diskann::Index<T, TagT, LabelT> index(
      ...
      L, insert_threads, enable_tags, true);
