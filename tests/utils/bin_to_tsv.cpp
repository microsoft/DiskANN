...
int main(int argc, char** argv) {
  if (argc != 4) {
    std::cout << argv[0] << " <float/uint32/int8/uint8> input_bin output_tsv" << std::endl;
    exit(-1);
  }
  std::string type_string(argv[1]);
  if ((type_string != std::string("float")) &&
      (type_string != std::string("uint32")) &&
      (type_string != std::string("int8")) &&
      (type_string != std::string("uin8"))) {
    std::cerr << "Error: type not supported. Use float/uint32/int8/uint8" << std::endl;
  }
  ...
  for (_u64 i = 0; i < nblks; i++) {
    ...
    else if (type_string == std::string("uint32"))
      block_convert<uint32_t>(writer, reader, (uint32_t*) read_buf, cblk_size, ndims);
