...
int main(int argc, char** argv) {
  ...
  if (argc != 8) {
    std::cout << argv[0] << " format: data type <int8/uint8/float> base_set train_set "
                            "num_clusters "
  ...
  if (std::string(argv[1]) == std::string("float"))
	  ...
	  partition<int8_t>(argv[2], argv[3], num_clusters, max_reps, argv[6], k_base);
  else if (std::string(argv[1]) == std::string("uint8"))
	  partition<uint8_t>(argv[2], argv[3], num_clusters, max_reps, argv[6], k_base);
  else
